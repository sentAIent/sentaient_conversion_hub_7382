...
import { CymaticsCore } from './CymaticsCore_v4.js?v=fix4';
...
static CYMATIC_PATTERNS = [
        ...
        this.matrixCyberStreams.forEach((stream, streamIndex) => {
            ...
            for (let i = 0; i < visibleLength && i < stream.chars.length; i += step) {
                ...
                if (this.cyberRainbowMode) {
                    ...
                    if (streamIndex === 0 && i === 0 && Math.random() < 0.01) console.log('[DEBUG] renderCyberCyber: drawing rainbow (hue:', hue, ')');
                } else {
                    ctx.fillStyle = stream.color || this.cyberColor;
                    if (streamIndex === 0 && i === 0 && Math.random() < 0.01) console.log('[DEBUG] renderCyberCyber: drawing solid (color:', ctx.fillStyle, ')');
                }
    ...
    createSnowflakeTexture() {
        ...
        // Soft radial glow halo behind the crystal (Removed based on user feedback)
    ...
    updateUIPanels() {
        ...
        // Force synchronous shader compilation for the current scene to prevent in-frame FPS stutters
    ...
    render(analyserL, analyserR) {
        ...
        try {
            ...
            const vNormTreble = (normHighs || 0) * vFactor;
            ...
            if (this._fpsRingBuffer) {
                ...
                if (this._fpsRingCount === 60) {
                    ...
                    // Add 8 second grace period after boot to ignore shader compilation frame drops
                    if (!this._appStartTime) this._appStartTime = performance.now();
                    if (currentFPS < 35 && (nowTime - (this._lastLodDegradation || 0) > 3000) && (nowTime - this._appStartTime > 8000)) {
                        this.degradeLOD();
            ...
            if (nowTime - this.lastFrameRenderTime >= targetFrameInterval) {
                ...
                // Audio level for PostProcessing and Swarms
                const audioLevel = (vNormBass + ((normMids || 0) * vFactor) + vNormTreble) / 3.0;
                const dt = nowTime - this.lastFrameRenderTime;

                // Update Swarm
                if (this.particleSwarm && this.activeModes.has('cymatics')) {
                    this.particleSwarm.update(Math.min(dt * 0.001, 0.1), audioLevel);
                }

                // Draw 3D Scene
                if (this.postProcessor && this.currentLodLevel !== 'low') {
                    this.postProcessor.render(this.scene, this.camera, Math.min(dt * 0.001, 0.1), audioLevel);
                } else {
                    this.renderer.render(this.scene, this.camera);
                }
    ...
    setMatrixColor(hexColor) {
        ...
        if (this.cyberMaterial && this.cyberMaterial.uniforms.uColor) {
            ...
            if (this.cyberMaterial.uniforms.uHeadColor) {
    ...
    setGlobalBrightness(brightness) {
        ...
        if (this.particleMaterial && this.particleMaterial.uniforms && this.particleMaterial.uniforms.uIntensity) {
            this.particleMaterial.uniforms.uIntensity.value = brightness;
        }
        if (this.petalMaterial && this.petalMaterial.uniforms && this.petalMaterial.uniforms.uIntensity) {
            this.petalMaterial.uniforms.uIntensity.value = brightness * 0.8; // Maintain original base opacity ratio
        }
    }
    ...
    setMatrixRainbow(isRainbow) {
        console.log('[DEBUG] setMatrixRainbow called:', isRainbow);
        ...
        this.cyberRainbowMode = isRainbow;
        if (!isRainbow) {
            const hex = this.matrixConfig.color || this.cyberColor || '#00FF41';
            console.log('[DEBUG] setMatrixRainbow (OFF) - Reverting to color:', hex);
            if (this.matrixCyberStreams) {
                this.matrixCyberStreams.forEach(stream => { stream.color = hex; });
            }
        } else {
    ...
    // Duplicate setMatrixColor removed
    ...
    updateLogoTexture() {
        ...
        if (!this.logoMesh) {
            ...
            if (this.camera && !this.dimmerPlane) {
                ...
                const dw = 2000;
                const dh = 2000;
                const dimVerts = new Float32Array([
                    -dw/2, -dh/2, 0,
                     dw * 1.5, -dh/2, 0,
                    -dw/2,  dh * 1.5, 0
                ]);
                dimGeom.setAttribute('position', new THREE.BufferAttribute(dimVerts, 3));
...
window.setCymaticPattern = function(classId, variationId) {
    if (classId === 0) {
        console.warn("Class 0 is invalid. Routing to Sri Yantra (Class 22, Var 8).");
        classId = 22;
        variationId = 8;
    }
    ...
    if (viz3D && viz3D.cymaticsCore) {
        ...
        if (!viz3D.activeModes.has('cymatics')) {
            if (window.switchRightTab) window.switchRightTab('cymatics', document.querySelector('.tab-pill[title="Cymatics"]'));
...
// --- Quantum Double-Slit Logic ---
let observerAnimFrame = null;
let quantumAudioCtx = null;
let quantumOscillator = null;
let quantumGain = null;
let quantumHumEnabled = true;

window.toggleQuantumHum = function(isEnabled) {
    quantumHumEnabled = isEnabled;
    if (quantumGain && quantumAudioCtx) {
        const now = quantumAudioCtx.currentTime;
        quantumGain.gain.cancelScheduledValues(now);
        quantumGain.gain.setValueAtTime(quantumGain.gain.value, now);
        
        if (!isEnabled) {
            quantumGain.gain.linearRampToValueAtTime(0.0, now + 0.5);
        } else {
            // Restore volume based on observer state
            const isObserved = document.getElementById('observerToggle') ? document.getElementById('observerToggle').checked : false;
            const targetVol = isObserved ? 0.3 : 0.15;
            quantumGain.gain.linearRampToValueAtTime(targetVol, now + 0.5);
        }
    }
};

window.toggleObserverEffect = function(isObserved) {
    if (!viz3D || !viz3D.cymaticsCore) return;
    
    // Switch to Quantum class if not active
    if (viz3D.cymaticsCore.activeClassId !== 25) {
        window.setCymaticPattern(25, 0);
    }
    
    const targetObserver = isObserved ? 1.0 : 0.0;
    const material = viz3D.cymaticsCore.materials[25];
    
    if (!material) return;
    
    // Audio Synthesis for Quantum Hum
    try {
        if (!quantumAudioCtx) {
            const AudioContext = window.AudioContext || window.webkitAudioContext;
            quantumAudioCtx = new AudioContext();
        }
        ...
        if (!quantumOscillator) {
            quantumOscillator = quantumAudioCtx.createOscillator();
            quantumGain = quantumAudioCtx.createGain();
            
            quantumOscillator.type = 'triangle';
            quantumOscillator.frequency.value = 108.0; // Deep hum
            
            quantumGain.gain.value = 0.0; // Start silent
            
            quantumOscillator.connect(quantumGain);
            quantumGain.connect(quantumAudioCtx.destination);
            quantumOscillator.start();
        }
        
        // Smoothly adjust audio params
        const now = quantumAudioCtx.currentTime;
        quantumGain.gain.cancelScheduledValues(now);
        quantumOscillator.frequency.cancelScheduledValues(now);
        
        // CRITICAL FIX: WebKit/Safari requires an anchor value before ramping, otherwise it crashes the audio thread or the whole tab.
        quantumGain.gain.setValueAtTime(quantumGain.gain.value, now);
        quantumOscillator.frequency.setValueAtTime(quantumOscillator.frequency.value, now);
        
        if (isObserved) {
            // Collapse: Higher pitch, slightly louder
            if (quantumHumEnabled) quantumGain.gain.linearRampToValueAtTime(0.3, now + 1.0);
            quantumOscillator.frequency.exponentialRampToValueAtTime(216.0, now + 1.0);
        } else {
            // Wave: Deep, low hum
            if (quantumHumEnabled) quantumGain.gain.linearRampToValueAtTime(0.15, now + 1.0);
            quantumOscillator.frequency.exponentialRampToValueAtTime(108.0, now + 1.0);
        }
    } catch (e) {
        console.warn("Web Audio API not supported or blocked for Quantum Hum:", e);
    }

    // Visual Interpolation
    if (observerAnimFrame) cancelAnimationFrame(observerAnimFrame);
    
    const animateObserver = () => {
        let current = material.uniforms.uObserver.value;
        const diff = targetObserver - current;
        
        if (Math.abs(diff) < 0.01) {
            material.uniforms.uObserver.value = targetObserver;
            return;
        }
        
        material.uniforms.uObserver.value = current + diff * 0.05;
        observerAnimFrame = requestAnimationFrame(animateObserver);
    };
