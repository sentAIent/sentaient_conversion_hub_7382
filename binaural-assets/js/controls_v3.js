...
window.controls = {
    ...
    toggleHyperGamma: function() {
        if (!state.isPlaying) return;
        const modal = document.getElementById('hyperGammaModal');
        if (modal) {
    ...
    toggleGlobalMenu: function(forceClose = false) {
        if (typeof window.toggleGlobalMenu === 'function') {
            window.toggleGlobalMenu(forceClose);
        } else {
            console.error('[Menu] window.toggleGlobalMenu not initialized');
        }
    },
    setupGlobalMenuSystem: function() {
        if (typeof setupGlobalMenuSystem === 'function') {
            setupGlobalMenuSystem();
        } else {
    ...
    switchLeftTab: function(targetId, btn) {
        ...
        if (!leftPanel) {
            console.error('[LeftMenu] Left panel not found');
            console.groupEnd();
            return;
        }
...
export function updateDockScaling() {
    ...
    let footer = document.getElementById('bottomControlBar') || document.getElementById('unifiedBottomDock');
    if (!footer) return;
...
/**
 * TELEPORTATION ENGINE: Moves mixer sections between sidebar (desktop) 
 * and bottom dock (mobile) to ensure full functionality on small screens.
 */
export function syncLayoutSections() {
    const width = window.innerWidth;
    const isMobile = width < 768;
    const rightPanel = document.getElementById('rightPanel');
    const mobileAnchor = document.getElementById('mobileMixerAnchor');
    const hub = document.getElementById('studioHub');
    const focusPane = document.getElementById('studioFocusPane');
    
    if (!rightPanel || !mobileAnchor) return;

    // Sections to move
    const sectionIds = ['journeys', 'djpads', 'atmosphere', 'stories', 'uploads', 'classical'];
    
    if (!isMobile) {
        // DESKTOP: Move everything back to right panel
        const mixerContent = rightPanel.querySelector('.flex-1.overflow-y-auto');
        if (mixerContent) {
            sectionIds.forEach(id => {
                const section = document.querySelector(`[data-section="${id}"]`);
                if (section && section.parentElement !== mixerContent) {
                    mixerContent.appendChild(section);
                    section.classList.remove('hidden'); // Ensure visible on desktop
                }
            });
        }
        // Ensure hub/focus are hidden if we somehow resized
        if (hub) hub.classList.add('hidden');
        if (focusPane) focusPane.classList.add('hidden');
    } else {
        // MOBILE: Sections stay hidden until a category is opened
        // We only move sections if we are NOT currently in a focused view
        if (focusPane && focusPane.classList.contains('hidden')) {
            sectionIds.forEach(id => {
                const section = document.querySelector(`[data-section="${id}"]`);
                if (section) section.classList.add('hidden');
            });
        }
        updateStudioHubStatus(); // Update dots on load/sync
    }

    console.log(`[Layout Sync] Engine optimized for ${isMobile ? 'MOBILE HUB' : 'DESKTOP MIXER'}`);
}
window.syncLayoutSections = syncLayoutSections;

/**
 * Opens a focused mixer category on mobile
 */
window.openStudioCategory = function(id) {
    const hub = document.getElementById('studioHub');
    const focusPane = document.getElementById('studioFocusPane');
    const anchor = document.getElementById('mobileMixerAnchor');
    const section = document.querySelector(`[data-section="${id}"]`);

    if (!hub || !focusPane || !anchor || !section) return;

    // Haptic feedback
    if (window.haptics && window.haptics.impact) window.haptics.impact('medium');

    // Teleport section to anchor
    ...
    // Scroll to top of dock
    const panes = document.getElementById('dockPanes');
    if (panes) panes.scrollTo({ top: 0, behavior: 'smooth' });
};

/**
 * Returns to the Studio Hub grid
 */
window.closeStudioCategory = function() {
    const hub = document.getElementById('studioHub');
    const focusPane = document.getElementById('studioFocusPane');
    if (!hub || !focusPane) return;

    hub.classList.remove('hidden');
    focusPane.classList.add('hidden');
    
    updateStudioHubStatus();
};

/**
 * Checks all categories for active sounds and updates Hub dots
 */
window.updateStudioHubStatus = function() {
    const sectionIds = ['journeys', 'djpads', 'atmosphere', 'stories', 'uploads', 'classical'];
    sectionIds.forEach(id => {
        ...
        // Check for any active sliders or toggles
        const sliders = section.querySelectorAll('input[type="range"]');
        let isActive = false;
        
        sliders.forEach(s => {
            // Ignore master sliders if they are just headers, check individual channel volumes
            if (parseFloat(s.value) > 0 && !s.id.includes('Master')) {
                isActive = true;
            }
        });
...
export function setupUI() {
    ...
    // MOBILE UI ENHANCEMENTS - FUSED HUD ELEMENTS
    els.miniMasterSliders = document.querySelectorAll('[id^="miniMasterSlider_"]');
    els.miniNowPlayingLabels = document.querySelectorAll('[id^="miniNowPlaying_"]');
    els.openLibraryMobileBtn = document.getElementById('openLibraryMobileBtn');
    ...
    els.lowPowerToggle = document.getElementById('lowPowerToggle');
    els.batterySaverToggle = document.getElementById('batterySaverToggle');
    els.lockInterfaceToggle = document.getElementById('lockInterfaceToggle');

    // Engine Controls - Elements for Sliders/Inputs (Exhaustive Binding)
    els.cyberTextInput = document.getElementById('cyberTextInput');
    els.cyberSpeedSlider = document.getElementById('cyberSpeedSlider');
    els.cyberLengthSlider = document.getElementById('cyberLengthSlider');
    els.matrixTextInput = document.getElementById('matrixTextInput');
    els.matrixSpeedSlider = document.getElementById('matrixSpeedSlider');

    // Mixer Sync Elements
    els.mixerBaseSlider = document.getElementById('mixerBaseSlider');
    els.mixerBeatSlider = document.getElementById('mixerBeatSlider');
    els.mixerVolSlider = document.getElementById('mixerVolSlider');

    // --- DOCK TAB SWITCHING (Collapsible) ---
    const dockTabBtns = document.querySelectorAll('.dock-tab-btn[data-tab]');
    const dockPanes = document.querySelectorAll('.dock-pane');
    const dockPanesContainer = document.getElementById('dockPanes');
    let activeDockTab = null; // null = collapsed

    function getDockMaxHeight() {
        const isLandscape = window.innerWidth > window.innerHeight;
        if (isLandscape) return '75vh'; // 3/4 screen height (Optimized for user request)
        return '45vh';
    }

    function collapseDock() {
        activeDockTab = null;
        if (dockPanesContainer) {
            dockPanesContainer.style.maxHeight = '0';
            dockPanesContainer.style.paddingTop = '0';
            dockPanesContainer.style.paddingBottom = '0';
            dockPanesContainer.style.overflowY = 'hidden';
            dockPanesContainer.style.borderTop = '0px solid transparent';
        }
        dockTabBtns.forEach(b => {
            b.classList.remove('active', 'bg-white/10', 'text-white', 'shadow-inner');
            b.classList.add('text-white/50');
        });
        dockPanes.forEach(pane => {
            pane.classList.add('hidden');
            pane.classList.remove('flex');
        });
    }

    function expandDock(targetId) {
        activeDockTab = targetId;
        // Update button styles
        dockTabBtns.forEach(b => {
            if (b.getAttribute('data-tab') === targetId) {
                b.classList.add('active', 'bg-white/10', 'text-white', 'shadow-inner');
        ...
        dockPanes.forEach(pane => {
            if (pane.id === targetId) {
                ...
                // Trigger dynamic content updates for relevant tabs
                if (targetId === 'configTab' || targetId === 'visualsTab') {
                    updateConfigPanel();
                }
            } else {
                pane.classList.add('hidden');
        ...
        // Expand container
        if (dockPanesContainer) {
            dockPanesContainer.style.maxHeight = getDockMaxHeight();
            dockPanesContainer.style.paddingTop = '1rem';
            dockPanesContainer.style.paddingBottom = '1rem';
            dockPanesContainer.style.overflowY = 'auto';
        }
    }
    ...
    dockTabBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (activeDockTab === targetId) {
                collapseDock(); // Toggle off
            } else {
                expandDock(targetId); // Switch or expand
            }
            // Haptic feedback
            import('../utils/haptics.js').then(m => {
                if (m.hapticSelection) m.hapticSelection();
            }).catch(() => {});
        });
    });

    window.switchDockTab = function(targetId) {
        expandDock(targetId);
    };
    ...
    if (dockDragHandle) {
        ...
        dockDragHandle.addEventListener('touchend', (e) => {
    ...
    // Tap on the visualizer/background area collapses dock
    const tapZone = document.getElementById('tapZone');
    if (tapZone) {
        tapZone.addEventListener('click', () => {
            if (activeDockTab) collapseDock();
        });
    }

    // Update max-height on orientation change
    window.addEventListener('resize', () => {
        if (activeDockTab && dockPanesContainer) {
            dockPanesContainer.style.maxHeight = getDockMaxHeight();
        }
        updateDockScaling();
        syncLayoutSections();
    ...
    function updateConfigPanel() {
        const viz = getVisualizer();
        const activeModes = viz ? Array.from(viz.activeModes) : [];
        
        // Hide all specific panels first
        const panels = ['matrixSettingsPanel', 'cyberSettingsPanel', 'galaxySettingsPanel'];
        panels.forEach(p => {
            const el = document.getElementById(p);
            if (el) el.classList.add('hidden');
        });

        // Show the most relevant one
        if (activeModes.includes('matrix')) {
            document.getElementById('matrixSettingsPanel')?.classList.remove('hidden');
        } else if (activeModes.includes('cyber')) {
            document.getElementById('cyberSettingsPanel')?.classList.remove('hidden');
        } else if (activeModes.includes('galaxy')) {
            document.getElementById('galaxySettingsPanel')?.classList.remove('hidden');
        } else {
            // Default or info message could go here
            console.log('[Config] No configurable visual active');
        }
    }
    window.updateConfigPanel = updateConfigPanel;
    console.log(`[Controls] Dock tabs initialized (${dockTabBtns.length} tabs, ${dockPanes.length} panes)`);
    ...
    if (restoredPrefs) {
        ...
        if (restoredPrefs.visuals) {
            ...
            if (restoredPrefs.visuals.lowPowerMode !== undefined) {
                state.lowPowerMode = restoredPrefs.visuals.lowPowerMode;
                if (els.lowPowerToggle) els.lowPowerToggle.checked = state.lowPowerMode;
            }
        }
    ...
    // MINI-MASTER SYNCHRONIZATION (Multi-HUD Support)
    if (els.miniMasterSliders.length > 0 && els.masterVolSlider) {
        els.miniMasterSliders.forEach(slider => {
            slider.addEventListener('input', (e) => {
                const val = parseFloat(e.target.value);
                updateMasterVolume(val);
                els.masterVolSlider.value = val;
                if (els.masterVolValue) els.masterVolValue.textContent = Math.round(val * 100) + '%';
                
                // Sync ALL other mini-sliders
                els.miniMasterSliders.forEach(s => { if (s !== e.target) s.value = val; });
            });
        });
        
        els.masterVolSlider.addEventListener('input', (e) => {
            const val = e.target.value;
            els.miniMasterSliders.forEach(s => s.value = val);
        });
    }

    // MOBILE LIBRARY TRIGGER
    if (els.openLibraryMobileBtn && els.libraryPanel) {
        els.openLibraryMobileBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            els.libraryPanel.classList.toggle('translate-x-full');
            // If opening, render the list
    ...
    // SAVE PRESET TRIGGER (Mobile/Refreshed)
    if (els.savePresetBtn) {
        els.savePresetBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            if (els.saveModal) {
                els.saveModal.classList.remove('hidden');
                els.saveModal.classList.add('flex');
            } else {
                // Fallback to legacy save
                if (typeof showSaveModal === 'function') showSaveModal();
            }
        });
    }
    ...
    const bindDualListener = (id, event, handler) => {
        const el1 = document.getElementById(id);
        const el2 = document.getElementById(id + '-dock');
        if (el1) el1.addEventListener(event, handler);
        if (el2) el2.addEventListener(event, handler);
    };

    bindDualListener('playBtn', 'click', handlePlayClick);
    bindDualListener('audioPlayBtn', 'click', handleAudioToggle);
    bindDualListener('randomColorBtn', 'click', () => {
        const viz = getVisualizer();
        if (viz && viz.setVisualColor) {
            const randomColor = '#' + Math.floor(Math.random()*16777215).toString(16).padStart(6, '0');
            viz.setVisualColor(randomColor);
            setVisualColor(randomColor);
        }
    });

    // Timer controls
    bindDualListener('timerMinus', 'click', () => {
        if (window.subtractTime) window.subtractTime(300);
    });
    bindDualListener('timerPlus', 'click', () => {
        if (window.addTime) window.addTime(300);
    });

    // Special handling for Color Picker (input matches change)
    const syncColorPickers = (e) => {
        const color = e.target.value;
        const p1 = document.getElementById('visualColorPicker');
        const p2 = document.getElementById('visualColorPicker-dock');
        if (p1) p1.value = color;
        if (p2) p2.value = color;
        setVisualColor(color);
    };
    const cp1 = document.getElementById('visualColorPicker');
    const cp2 = document.getElementById('visualColorPicker-dock');
    if (cp1) cp1.addEventListener('input', syncColorPickers);
    if (cp2) cp2.addEventListener('input', syncColorPickers);
    ...
    // STUDIO MIXER SYNC (Bidirectional)
    const syncSliders = () => {
        // Base
        if (els.baseSlider && els.mixerBaseSlider) {
            els.mixerBaseSlider.addEventListener('input', (e) => {
                els.baseSlider.value = e.target.value;
                state.baseFrequency = parseFloat(e.target.value);
                if (els.baseValue) els.baseValue.textContent = state.baseFrequency + ' Hz';
                updateFrequencies();
            });
            els.baseSlider.addEventListener('input', (e) => {
                els.mixerBaseSlider.value = e.target.value;
            });
        }
        // Beat
        if (els.beatSlider && els.mixerBeatSlider) {
            els.mixerBeatSlider.addEventListener('input', (e) => {
                els.beatSlider.value = e.target.value;
                state.beatFrequency = parseFloat(e.target.value);
                if (els.beatValue) els.beatValue.textContent = state.beatFrequency + ' Hz';
                updateFrequencies();
            });
            els.beatSlider.addEventListener('input', (e) => {
                els.mixerBeatSlider.value = e.target.value;
            });
        }
        // Volume
        if (els.volSlider && els.mixerVolSlider) {
            els.mixerVolSlider.addEventListener('input', (e) => {
                els.volSlider.value = e.target.value;
                state.binauralVolume = parseFloat(e.target.value);
                if (els.volValue) els.volValue.textContent = Math.round(state.binauralVolume * 100) + '%';
                updateBeatsVolume(state.binauralVolume);
            });
            els.volSlider.addEventListener('input', (e) => {
                els.mixerVolSlider.value = e.target.value;
            });
        }
    };
    syncSliders();
    ...
    // Low Power Mode Toggle - NEW
    if (els.lowPowerToggle) {
        els.lowPowerToggle.addEventListener('change', (e) => {
            state.lowPowerMode = e.target.checked;
            console.log('[LowPower] Mode toggled:', state.lowPowerMode);
            
            // Notify visualizer
            const viz = getVisualizer();
            if (viz && typeof viz.setLowPowerMode === 'function') {
                viz.setLowPowerMode(state.lowPowerMode);
            }
            
            saveUserPreferences();
            
            if (window.showToast) {
                window.showToast(state.lowPowerMode ? 'Low Power Mode: ON (Capped FPS)' : 'Low Power Mode: OFF (Full Performance)', 'success');
            }
        });
    }
    ...
    if (els.leftToggle) els.leftToggle.addEventListener('click', () => {
        ...
        if (isMobile) {
            if (window.switchDockTab) window.switchDockTab('sessionsTab');
            return;
        }
        if (els.leftPanel) {
    ...
    if (els.rightToggle) els.rightToggle.addEventListener('click', () => {
        const isMobile = window.innerWidth < 768;
        if (isMobile) {
            if (window.switchDockTab) window.switchDockTab('audioTab');
            return;
        }
        if (els.rightPanel) {
            ...
            if (typeof enforceLightThemeStyles === 'function') enforceLightThemeStyles();
        }
    });
    ...
    const footer = document.getElementById('bottomControlBar') || document.getElementById('unifiedBottomDock');
    if (footer) {
    ...
    // --- UNIVERSAL MODAL DISMISS (Tap Backdrop to Close) ---
    // This catches any modal that uses the .modal class pattern
    document.querySelectorAll('.modal').forEach(modal => {
        modal.addEventListener('click', (e) => {
            // Only dismiss if clicking the backdrop (the modal container itself), not its content
            if (e.target === modal) {
                // Try animated close first (theme modal has special animation)
                const card = modal.querySelector('[id$="ModalCard"]');
                if (card) {
    ...
    // Also add Escape key to close any visible modal
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
...
export async function applyComboPreset(comboId, btnElement) {
    ...
    // Toggle Logic - Stop if clicking same active combo preset
    ...
    try {
        ...
        resumeVisuals();
        syncAllButtons();
        startSessionTracking(combo.preset);
        console.log("[Controls] Combo preset fully active - beats + ambience");
    } catch (e) {
        console.error("[Controls] Failed to start combo audio:", e);
    }
...
function setupMatrixControls() {
    ...
    if (matrixTextInput) {
        matrixTextInput.addEventListener('input', (e) => {
            const viz = getVisualizer();
    ...
    modeBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const viz = getVisualizer();
            if (viz && viz.setMatrixLogicMode) {
                const text = matrixTextInput ? matrixTextInput.value : '';
    ...
    // --- Auto Color Cycle Controls ---
    const matrixCycleToggle = document.getElementById('matrixAutoCycleToggle');
    const matrixCycleSlider = document.getElementById('matrixCycleTimerSlider');
    const matrixCycleVal = document.getElementById('matrixCycleTimerVal');

    if (matrixCycleToggle) {
        matrixCycleToggle.addEventListener('change', (e) => {
            const viz = getVisualizer();
            const seconds = matrixCycleSlider ? parseFloat(matrixCycleSlider.value) : 5;
            if (viz && viz.setAutoColorCycle) viz.setAutoColorCycle(e.target.checked, seconds);
            // Sync cyber toggle too (shared engine)
            const cyberToggle = document.getElementById('cyberAutoCycleToggle');
            if (cyberToggle) cyberToggle.checked = e.target.checked;
        });
    }
    if (matrixCycleSlider) {
        matrixCycleSlider.addEventListener('input', (e) => {
            const val = parseFloat(e.target.value);
            if (matrixCycleVal) matrixCycleVal.textContent = `${val}s`;
            const viz = getVisualizer();
            if (viz && viz.setAutoColorCycle && matrixCycleToggle?.checked) {
                viz.setAutoColorCycle(true, val);
            }
            // Sync cyber slider too
            const cyberSlider = document.getElementById('cyberCycleTimerSlider');
            const cyberVal = document.getElementById('cyberCycleTimerVal');
            if (cyberSlider) cyberSlider.value = val;
            if (cyberVal) cyberVal.textContent = `${val}s`;
        });
    }
}
...
function setupIdleTimer() {
    ...
    // Dock-specific: ensure touching the faded dock instantly reveals it
    const dock = document.getElementById('unifiedBottomDock');
    if (dock) {
        ['touchstart', 'mousedown'].forEach(evt => {
            dock.addEventListener(evt, (e) => {
                // If dock is faded, reveal it immediately
                if (dock.classList.contains('idle-hidden')) {
                    e.stopPropagation();
                    showUI();
                    resetIdleTimer();
                } else {
...
function setupCyberControls() {
    ...
    // Custom Text
    const cyberTextInput = document.getElementById('cyberTextInput');
    if (cyberTextInput) {
        cyberTextInput.addEventListener('input', (e) => {
            const viz = getVisualizer();
            if (viz && viz.setCyberLogicMode) {
    ...
    // --- Auto Color Cycle Controls ---
    const cyberCycleToggle = document.getElementById('cyberAutoCycleToggle');
    const cyberCycleSlider = document.getElementById('cyberCycleTimerSlider');
    const cyberCycleVal = document.getElementById('cyberCycleTimerVal');

    if (cyberCycleToggle) {
        cyberCycleToggle.addEventListener('change', (e) => {
            const viz = getVisualizer();
            const seconds = cyberCycleSlider ? parseFloat(cyberCycleSlider.value) : 5;
            if (viz && viz.setAutoColorCycle) viz.setAutoColorCycle(e.target.checked, seconds);
            // Sync matrix toggle too (shared engine)
            const matrixToggle = document.getElementById('matrixAutoCycleToggle');
            if (matrixToggle) matrixToggle.checked = e.target.checked;
        });
    }
    if (cyberCycleSlider) {
        cyberCycleSlider.addEventListener('input', (e) => {
            const val = parseFloat(e.target.value);
            if (cyberCycleVal) cyberCycleVal.textContent = `${val}s`;
            const viz = getVisualizer();
            if (viz && viz.setAutoColorCycle && cyberCycleToggle?.checked) {
                viz.setAutoColorCycle(true, val);
            }
            // Sync matrix slider too
            const matrixSlider = document.getElementById('matrixCycleTimerSlider');
            const matrixVal = document.getElementById('matrixCycleTimerVal');
            if (matrixSlider) matrixSlider.value = val;
            if (matrixVal) matrixVal.textContent = `${val}s`;
        });
    }
}
...
/**
 * Mobile Mixer Navigation: Scrolls the dock panes to target sections
 * @param {string} direction - 'first', 'last', 'next', or 'prev'
 */
window.scrollToMixerSection = function(direction) {
    const container = document.getElementById('dockPanes');
    const anchor = document.getElementById('mobileMixerAnchor');
    
    if (!container || !anchor) {
        console.warn('[Mixer Nav] Container or Anchor not found');
        return;
    }
    ...
    const sections = Array.from(anchor.querySelectorAll('.mixer-section'));
    ...
    if (direction === 'next') {
        // Find the first section that is NOT yet at the top
        target = sections.find(sec => {
            const rect = sec.getBoundingClientRect();
            // If the section top is more than 50px below the container top, it's the "next"
            return rect.top > containerTop + 50;
        });
    } else if (direction === 'prev') {
        // Find the first section that is ABOVE the container top
        const reversed = [...sections].reverse();
        target = reversed.find(sec => {
            const rect = sec.getBoundingClientRect();
            // If the section top is more than 50px above the container top, it's the "prev"
            return rect.top < containerTop - 50;
        });
    }

    if (target) {
        // We use scrollIntoView on the target section
        target.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
        // If no target (reached extreme), provide bounce or feedback
        if (direction === 'next') container.scrollTo({ top: container.scrollHeight, behavior: 'smooth' });
        if (direction === 'prev') container.scrollTo({ top: 0, behavior: 'smooth' });
    }
};

// =============================================================================
// GLOBAL MENU SYSTEM (The "Perfected" Web App Menu)
// =============================================================================
window.toggleGlobalMenu = function(forceClose = false) {
    if (!els.rightPanel) {
        console.error('[Menu] rightPanel element not found in els');
        els.rightPanel = document.getElementById('rightPanel');
        if (!els.rightPanel) return;
    }
    
    const isClosed = els.rightPanel.classList.contains('translate-x-full');
    const overlay = document.getElementById('mobileMenuOverlay');
    
    // If we want to force close, or if it's currently open
    if (forceClose || !isClosed) {
        console.log('[Menu] Closing Global Menu...');
        els.rightPanel.classList.add('translate-x-full');
        if (overlay) overlay.classList.remove('active');
    } else {
        console.log('[Menu] Opening Global Menu...');
        els.rightPanel.classList.remove('translate-x-full');
        if (overlay) overlay.classList.add('active');
        
        // Ensure the default tab is active if none selected
        const activeTab = Array.from(document.querySelectorAll('.global-tab-btn')).find(b => b.classList.contains('bg-[var(--accent)]'));
        if (!activeTab) {
            const firstTab = document.querySelector('.global-tab-btn');
            if (firstTab) firstTab.click();
        }
    }
};

export function setupGlobalMenuSystem() {
    console.log('[Controls] Initializing Robust Tab System...');
    
    const tabContainer = document.getElementById('globalMenuTabs') || document.getElementById('mixerTabs');
    const contentContainer = document.getElementById('globalMenuContent') || document.getElementById('globalMenuContent-hub');
    
    if (!tabContainer || !contentContainer) {
        console.warn('[Menu] UI containers missing. Running in Hub Mode or restricted layout.');
        // Don't recurse infinitely, just bind what we can find
    }

    // Dedicated Close Buttons
    const closeBtns = ['#closeGlobalMenuBtn', '#closeRightBtn', '#mobileMenuOverlay', '#closeLeftBtn'];
    closeBtns.forEach(sel => {
        const el = document.querySelector(sel);
        if (el) {
            el.onclick = () => {
                if (window.toggleGlobalMenu) window.toggleGlobalMenu(true);
                // Also handle left panel if it's the left close button
                if (sel === '#closeLeftBtn') {
                    const leftPanel = document.getElementById('leftPanel');
...
function setupSettingsHandlers() {
    console.log('[Controls] setupSettingsHandlers CALLED');
    
    const lockToggle = document.getElementById('lockInterfaceToggle');
    if (lockToggle) {
        lockToggle.checked = state.uiLocked;
        lockToggle.addEventListener('change', (e) => {
            state.uiLocked = e.target.checked;
            updateLockUI();
        });
    }
    ...
    if (batteryToggle) {
        batteryToggle.checked = state.lowPowerMode;
        batteryToggle.addEventListener('change', (e) => {
            state.lowPowerMode = e.target.checked;
            const viz = getVisualizer();
            if (viz && viz.setLowPowerMode) viz.setLowPowerMode(state.lowPowerMode);
            // Also sync traditional lowPowerToggle if it exists
            const lp2 = document.getElementById('lowPowerToggle');
            if (lp2) lp2.checked = state.lowPowerMode;
        });
    }
    ...
    if (resetPanelsBtn) {
        resetPanelsBtn.addEventListener('click', () => {
            import('./resize-panels.js').then(m => m.resetPanelWidths());
        });
    }
}
...
function saveCustomMixPrompt() {
    ...
    const newMix = {
        id: 'custom-' + Date.now(),
        name: mixName,
        baseFreq: parseFloat(els.baseSlider.value),
        beatFreq: parseFloat(els.beatSlider.value),
        vol: parseFloat(els.volSlider.value),
        activeVisuals: Array.from(window.state?.visualizer?.activeModes || []),
        cymaticClass: window.state?.visualizer?.currentCymaticClass || 0,
        cymaticVariation: window.state?.visualizer?.currentCymaticVariation || 0,
        activeSoundscapes: { ...window.state?.activeSoundscapes },
...
function loadCustomMix(id) {
    ...
    // Set audio values
    if (window.updateValue) {
        els.baseSlider.value = mix.baseFreq;
        els.beatSlider.value = mix.beatFreq;
        els.volSlider.value = mix.vol;
        window.updateValue(els.baseSlider, 'baseValue', 'Hz');
        window.updateValue(els.beatSlider, 'beatValue', 'Hz');
        window.updateValue(els.volSlider, 'volValue', '%', true);
        
        if (window.updateAudioParams) window.updateAudioParams();
    }
    
    // Handle Audio Mode
    if (mix.audioMode && window.setAudioMode) {
        window.setAudioMode(mix.audioMode);
    }
    
    // Set Visuals
    if (window.state && window.state.visualizer) {
        // clear current visuals
        Array.from(window.state.visualizer.activeModes).forEach(mode => {
            window.state.visualizer.toggleMode(mode);
        });
        ...
        if (mix.activeVisuals.includes('cymatics')) {
            window.state.visualizer.currentCymaticClass = mix.cymaticClass;
            window.state.visualizer.currentCymaticVariation = mix.cymaticVariation;
            if (window.state.visualizer.updateCymaticsUniforms) {
                window.state.visualizer.updateCymaticsUniforms();
            }
        }
    }
    
    // Set Soundscapes
    if (window.toggleSoundscape && mix.activeSoundscapes) {
        // Stop all current
        Object.keys(window.state.activeSoundscapes).forEach(sid => {
            if(window.state.activeSoundscapes[sid]) {
                window.toggleSoundscape(sid, false);
                const btn = document.querySelector(".soundscape-btn[data-soundscape='" + sid + "']");
                if (btn) btn.classList.remove('active', 'border-[var(--accent)]');
            }
        });
        
        // Start saved
        Object.keys(mix.activeSoundscapes).forEach(sid => {
            if (mix.activeSoundscapes[sid]) {
                window.toggleSoundscape(sid, true);
                const btn = document.querySelector(".soundscape-btn[data-soundscape='" + sid + "']");
                if (btn) btn.classList.add('active', 'border-[var(--accent)]');
            }
        });
    }
