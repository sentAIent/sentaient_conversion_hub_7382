...
// Handle sun cursor SVG injection based on user preference
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    const cursorToggleBtn = document.getElementById('cursorToggleBtn');
    
    // Check saved preference
    const savedCursorPref = localStorage.getItem('mindwave_cursor_pref');
    if (savedCursorPref === 'custom') {
        document.body.classList.add('custom-sun-cursor');
        if (cursorToggleBtn) {
            cursorToggleBtn.classList.add('active', 'text-white');
            cursorToggleBtn.classList.remove('text-[var(--text-muted)]');
        }
    }

    if (cursorToggleBtn) {
        cursorToggleBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent modal closures if any
            
            const isCustom = document.body.classList.toggle('custom-sun-cursor');
            
            if (isCustom) {
                localStorage.setItem('mindwave_cursor_pref', 'custom');
                window.showToast?.('Tribal Sun Cursor Activated ☀️', 'success');
                cursorToggleBtn.classList.add('active', 'text-white');
                cursorToggleBtn.classList.remove('text-[var(--text-muted)]');
            } else {
                localStorage.setItem('mindwave_cursor_pref', 'default');
                window.showToast?.('Default Cursor Restored', 'info');
                cursorToggleBtn.classList.remove('active', 'text-white');
                cursorToggleBtn.classList.add('text-[var(--text-muted)]');
            }
        });
    }
});
