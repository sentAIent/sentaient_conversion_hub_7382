...
class EntityRenderer {
    ...
    draw(ctx, time) {
        ...
        this.renderBackgroundSpacecraft(ctx, time);
    }
    ...
    renderInfluenceGrid(ctx, time) {
        ...
        ctx.strokeStyle = `rgba(0, 243, 255, ${influence * 0.1})`;
        ctx.lineWidth = 1;
    ...
    renderBackgroundSpacecraft(ctx, time) {
        if (!this.game.spacecraft || this.game.spacecraft.length === 0) return;
        const { camera, canvas } = this.game;
        const zoom = camera.zoom || 1;
        
        ctx.save();
        this.game.spacecraft.forEach(s => {
            const parallax = (s.z / 4000) || 0.1;
            // Add slight drift
            s.x += s.vx * 0.05;
            s.y += s.vy * 0.05;

            const sx = (((s.x || 0) * zoom + (camera.x * parallax)) % canvas.width + canvas.width) % canvas.width;
            const sy = (((s.y || 0) * zoom + (camera.y * parallax)) % canvas.height + canvas.height) % canvas.height;
            ...
            if (s.type === 'organic') {
                ...
                for(let i=0; i<8; i++) {
                    ...
                    const rad = size * (0.6 + Math.sin(time * 0.002 + i) * 0.2);
                    ctx.lineTo(Math.cos(ang) * rad, Math.sin(ang) * rad);
                }
                ctx.closePath();
                ctx.fill();
            } else {
                // Sleek scout silhouette
                ctx.beginPath();
                ctx.moveTo(size, 0);
            ...
            // Engines glow (Pulsing)
            const pulse = (Math.sin(time * 0.01) + 1) / 2;
            const glow = ctx.createRadialGradient(-size * 0.4, 0, 0, -size * 0.4, 0, size * 0.6);
            this.rm.safeAddColorStop(glow, 0, '#ffffff');
            this.rm.safeAddColorStop(glow, 0.5, s.color);
            this.rm.safeAddColorStop(glow, 1, 'transparent');
            ctx.fillStyle = glow;
            ctx.globalAlpha = 0.4 + pulse * 0.4;
            ctx.beginPath();
            ctx.arc(-size * 0.4, 0, size * 0.6, 0, Math.PI * 2);
            ctx.fill();
