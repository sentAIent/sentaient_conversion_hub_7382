...
`,ot=f=>f===1?`
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                // Remove the flat front face entirely, leaving only the back slope (the perfect wave)
                float envelope = 0.0;
                if (waveX < 0.0) {
                    envelope = exp(-pow(waveX * 0.04, 2.0)); // The perfect back slope
                }
                ...
                if (pos.z > curlStartHeight) {
                    ...
                    // Match the curl width exactly to the razor-steep front face so it doesn't widen!
                    float curlEnvelope = exp(-pow(waveX * 0.12, 2.0)); 
                    
                    // Wrap the curl all the way around (270 degrees / 1.5 PI) to completely CLOSE the barrel gap
                    float bendAngle = pow(normH, 1.2) * 3.14159 * 1.5 * normalizedCurl * curlEnvelope; 
                    ...
                    // Fake 3D Volume/Thickness: Spiral the radius inward as it curls. 
                    // This forces the paper-thin mesh to roll up into a dense, solid cylinder of water!
                    float radius = h * (1.0 - (bendAngle / 6.28) * 0.6);
        ...
        `,a=`
            ...
            void main() {
                // Physically delete the flat front wall so you only see the perfect back slope
                if (vWaveX >= 0.0) {
                    discard;
                }
...
//# sourceMappingURL=visualizer_v4-DeZKaIwL.js.map
