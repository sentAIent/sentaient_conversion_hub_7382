...
export class Visualizer3D {
    ...
    initCymatics() {
        ...
        if (this.cymaticMesh) {
            console.log('[Visualizer] Removing stale cymaticMesh from scene');
        ...
        if (this.cymaticsGroup) {
            console.log('[Visualizer] Removing existing cymaticsGroup layers');
        ...
        // Use a single high-definition geometry plane. 
        // True cymatics rely on infinite mathematically calculated resolution, which a Fragment Shader provides perfectly.
        const geometry = new THREE.PlaneGeometry(35, 35, 2, 2);
        this.cymaticMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uTime: { value: 0 },
                uIntensity: { value: 0 },
                uBeatFreq: { value: 10.0 }, // Connection to Audio Engine
                uColor: { value: this.customColor ? new THREE.Color(this.customColor) : new THREE.Color(0x2dd4bf) }
            },
            vertexShader: `
                varying vec2 vUv;
                void main() {
            ...
            fragmentShader: `
                ...
                float fractalResonance(vec2 uv, float n, float m, float beat) {
                    float pi = 3.14159265;
                    float total = 0.0;
                    vec2 p = uv;
                    for(int i = 0; i < 2; i++) {
                        float t = uTime * 0.4 + float(i) * 2.0;
                        p.x += 0.22 * sin(p.y * 2.5 + t + beat * 0.015);
                        p.y += 0.22 * cos(p.x * 2.5 - t * 0.6 + beat * 0.015);
                        float ang = t * 0.1;
                ...
                void main() {
                    vec2 uv = (vUv - 0.5) * 2.5;
                    float vib = fractalResonance(uv, uN, uM, uBeatFreq);
                    float thr = 0.05 + uIntensity * 0.3;
                    float sandP = smoothstep(thr + 0.45, thr - 0.05, vib);
                    float finalSand = step(hash(uv * 1000.0), sandP);
                    vec3 baseCol = mix(uColor, vec3(1.0, 0.9, 0.7), uIntensity);
                    vec3 col = mix(vec3(0.01, 0.01, 0.03), baseCol * 2.5, finalSand);
                    col += baseCol * smoothstep(0.3, 0.0, vib) * 0.5;
        ...
        // The mesh representing the cymatic physics plate
        const mesh = new THREE.Mesh(geometry, this.cymaticMaterial);
        
        // Position it closer to the camera to ensure it's in front of all backdrops
        mesh.rotation.x = 0; 
        mesh.position.y = 0;
        mesh.position.z = -5; // Moved forward from -15

        this.scene.add(mesh);
        this.cymaticMesh = mesh; // Track for deletion
        console.log('[Visualizer] Cymatics Mesh added DIRECTLY to scene at Z: -5');

        // Backdrop Ambient Aura corresponding to frequency energy
        const canvas = document.createElement('canvas');
        ...
        const glowMat = new THREE.MeshBasicMaterial({
            color: this.customColor ? new THREE.Color(this.customColor) : new THREE.Color(0x2dd4bf),
            transparent: true,
            opacity: 0.3,
            blending: THREE.AdditiveBlending,
            depthWrite: false,
        ...
        glowMesh.position.set(0, 0, -10); // Glow slightly behind the plate
