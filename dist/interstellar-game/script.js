...
class AetherEngine {
    ...
    toggleUniverseKnowledge() {
        const panel = document.getElementById('universeKnowledgePanel');
    ...
    initGemsSectionResize() {
        ...
        window.addEventListener('mousemove', (e) => {
            ...
            const newWidth = Math.max(200, Math.min(2500, startWidth + deltaX));
    ...
    generateMines() {
        ...
        for (let i = 0; i < count; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 1500 + Math.random() * (this.worldSize - 1500);
            this.mines.push({
                x: Math.cos(angle) * dist,
                y: Math.sin(angle) * dist,
                ...
                rotation: Math.random() * Math.PI * 2 // For spinning symbol
            });
        }
    }

    renderMines(ctx, time) {
        if (!this.flightMode) return;
        this.mines.forEach(mine => {
            if (!mine.active) return;
            if (!mine.radius || isNaN(mine.radius) || mine.radius <= 0) return; // Safety check
            // Simple visual culling
            if (this.playerShip && Math.abs(mine.x - this.playerShip.x) > 3000) return;

            ctx.save();
            ctx.translate(mine.x, mine.y);
            const pulse = Math.sin(time * 0.005 + mine.pulseOffset);
            const size = mine.radius;

            // Outer Radiation Glow (pulsing)
            const glowSize = size * (1.8 + pulse * 0.2);
            if (!isFinite(glowSize) || glowSize <= 0) { // Safety check
                ctx.restore();
                return;
            }
            const grad = ctx.createRadialGradient(0, 0, size * 0.3, 0, 0, glowSize);
            grad.addColorStop(0, 'rgba(204, 255, 0, 0.4)');
            ...
            // 3D Bomb Body - Cylindrical with shading
            const bombHeight = size * 1.4;
            const bombWidth = size * 0.8;

            // Main body gradient (metallic)
            const bodyGrad = ctx.createLinearGradient(-bombWidth, 0, bombWidth, 0);
            bodyGrad.addColorStop(0, '#1a1a1a');
            bodyGrad.addColorStop(0.3, '#444');
            bodyGrad.addColorStop(0.5, '#666');
            bodyGrad.addColorStop(0.7, '#444');
            bodyGrad.addColorStop(1, '#1a1a1a');

            // Bomb cylinder
            ctx.fillStyle = bodyGrad;
            ctx.fillRect(-bombWidth / 2, -bombHeight / 2, bombWidth, bombHeight);

            // Top cap (rounded)
            ctx.beginPath();
            ctx.ellipse(0, -bombHeight / 2, bombWidth / 2, bombWidth / 4, 0, 0, Math.PI * 2);
            ctx.fill();
            ...
            for (let i = 0; i < 4; i++) {
                ...
                ctx.rotate(angle);
                ctx.beginPath();
                ctx.moveTo(bombWidth / 2, bombHeight / 3);
                ctx.lineTo(bombWidth * 1.2, bombHeight / 2);
                ctx.lineTo(bombWidth / 2, bombHeight / 2);
                ctx.closePath();
                ctx.fill();
                ctx.strokeStyle = '#111';
                ctx.lineWidth = 1;
            ...
            const stripeWidth = bombWidth * 0.9;
            const stripeHeight = bombHeight * 0.15;
            ...
            for (let i = -3; i < 4; i++) {
                ctx.save();
                ctx.translate(i * stripeWidth / 6, 0);
                ctx.rotate(Math.PI / 4);
                ctx.fillRect(-stripeWidth / 12, -stripeHeight, stripeWidth / 12, stripeHeight * 2);
                ctx.restore();
            }

            // Radiation Symbol on warning stripe
            ctx.fillStyle = '#000';
            const symbolSize = stripeHeight * 0.6;

            // Center circle
            ctx.beginPath();
            ctx.arc(0, 0, symbolSize * 0.2, 0, Math.PI * 2);
            ctx.fill();
            ...
            for (let i = 0; i < 3; i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(0, 0);
                ctx.arc(0, 0, symbolSize, angle - 0.3, angle + 0.3);
                ctx.lineTo(0, 0);
                ctx.fill();
            }

            // Blinking red light on top
            const blinkAlpha = Math.abs(Math.sin(time * 0.003));
            ctx.fillStyle = `rgba(255, 0, 0, ${blinkAlpha})`;
            ctx.beginPath();
            ctx.arc(0, -bombHeight / 2, bombWidth / 8, 0, Math.PI * 2);
            ctx.fill();

            // Light glow
            if (blinkAlpha > 0.5) {
                const lightGrad = ctx.createRadialGradient(0, -bombHeight / 2, 0, 0, -bombHeight / 2, bombWidth / 3);
                lightGrad.addColorStop(0, 'rgba(255, 0, 0, 0.5)');
                lightGrad.addColorStop(1, 'transparent');
                ctx.fillStyle = lightGrad;
                ctx.beginPath();
                ctx.arc(0, -bombHeight / 2, bombWidth / 3, 0, Math.PI * 2);
                ctx.fill();
            }
    ...
    checkCollisions() {
        if (!this.playerShip || this.isDead) return;
        const shipRadius = 40;

        // Check Mines
        this.mines.forEach(mine => {
            if (!mine.active) return;
            const dx = mine.x - this.playerShip.x;
            const dy = mine.y - this.playerShip.y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < mine.radius + shipRadius) {
                this.triggerDeath('mine', mine);
            }
        });
        ...
        if (this.blackHoles) {
            this.blackHoles.forEach(bh => {
                ...
                const dist = Math.sqrt(dx * dx + dy * dy);
                // BH radius - event horizon approx 100
                if (dist < 100) {
                    this.triggerDeath('blackhole', bh);
                }
            });
        }
    }

    triggerDeath(sourceType, sourceObj) {
        if (this.isDead) return;
        this.isDead = true;
        this.deathStartTime = Date.now();
        this.deathEndTime = Date.now() + 30000;

        const hud = document.getElementById('flightHUD');
        if (hud) hud.classList.add('hidden');

        if (sourceType === 'mine') {
            this.supernova = { x: sourceObj.x, y: sourceObj.y, startTime: Date.now() };
        } else {
            this.supernova = null;
        }
    }
    ...
    renderDeathSequence() {
        ...
        ctx.fillRect(0, 0, canvas.width, canvas.height); // Black nothingness

        if (this.supernova) {
            const snElapsed = Date.now() - this.supernova.startTime;
            if (snElapsed < 5000) {
                const progress = snElapsed / 5000;
                // Supernova Flash
                const maxRad = Math.max(canvas.width, canvas.height);
                ...
                const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, maxRad * progress * 2);
                grad.addColorStop(0, '#ffffff');       // White Hot
                grad.addColorStop(0.2, '#ccff00');    // Nuclear Flash
                grad.addColorStop(0.4, '#39ff14');    // Toxic Green wave
                grad.addColorStop(0.6, '#ff5500');    // Fire
                grad.addColorStop(1, 'transparent');

                ctx.fillStyle = grad;
                ctx.beginPath();
                ctx.arc(cx, cy, maxRad * progress * 2, 0, Math.PI * 2);
                ctx.fill();
                ctx.globalAlpha = 1;
            }
        } else {
            // BLACK HOLE TRAVERSAL - Immersive Journey Through Singularity
            const cx = canvas.width / 2;
            const cy = canvas.height / 2;
            const totalDuration = 30000;
            const progress = Math.min(elapsed / totalDuration, 1);

            // Phase 1: Initial Pull (0-3s) - Being sucked in
            if (elapsed < 3000) {
                const pullProgress = elapsed / 3000;

                // Gravitational lens distortion
                ctx.save();
                const distortScale = 1 + pullProgress * 0.3;
                ctx.translate(cx, cy);
                ctx.scale(distortScale, distortScale);
                ctx.translate(-cx, -cy);

                // Swirling stars being pulled in
                for (let i = 0; i < 200; i++) {
                    const angle = (i * 137.5 + elapsed * 0.01) % 360;
                    const rad = angle * Math.PI / 180;
                    const dist = (canvas.width / 2) * (1 - pullProgress * 0.8) + (i % 10) * 20;
                    const spiral = elapsed * 0.003 * (1 + i % 3);

                    const x = cx + Math.cos(rad + spiral) * dist;
                    const y = cy + Math.sin(rad + spiral) * dist;

                    const brightness = 200 - Math.floor(pullProgress * 150);
                    const alpha = (1 - pullProgress) * 0.8;
                ...
                const vignetteGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, canvas.width);
                vignetteGrad.addColorStop(0, 'transparent');
                vignetteGrad.addColorStop(1 - pullProgress * 0.5, 'rgba(0, 0, 0, 0)');
                vignetteGrad.addColorStop(1, `rgba(0, 0, 0, ${pullProgress})`);
                ctx.fillStyle = vignetteGrad;
                ctx.fillRect(0, 0, canvas.width, canvas.height);
            }

            // Phase 2: Tunnel Travel (3-27s) - Inside the event horizon
            else if (elapsed < 27000) {
                const tunnelProgress = (elapsed - 3000) / 24000;

                // TUNNEL EFFECT - Concentric rings moving outward (like warp speed inverted)
                const numRings = 40;
                for (let i = 0; i < numRings; i++) {
                    const ringDelay = i * 0.03;
                    const ringTime = (elapsed * 0.001 + ringDelay) % 2;
                    const ringRadius = ringTime * canvas.width * 0.6;

                    if (ringRadius < canvas.width * 0.6) {
                        const thickness = 3 + Math.sin(i * 0.5) * 2;
                        const alpha = (1 - ringTime / 2) * 0.4;

                        // Color shift through spectrum as we travel
                        const hue = (tunnelProgress * 360 + i * 30) % 360;
                        const colorR = Math.floor(100 + Math.sin(hue * Math.PI / 180) * 100);
                        const colorG = Math.floor(100 + Math.sin((hue + 120) * Math.PI / 180) * 100);
                        const colorB = Math.floor(150 + Math.sin((hue + 240) * Math.PI / 180) * 105);
                ...
                // Particle streams flowing past
                for (let i = 0; i < 300; i++) {
                    const streamAngle = (i * 23) % 360 * Math.PI / 180;
                    const streamSpeed = 2 + (i % 5);
                    const streamDist = ((elapsed * streamSpeed + i * 50) % (canvas.width * 0.7));

                    const x = cx + Math.cos(streamAngle) * streamDist;
                    const y = cy + Math.sin(streamAngle) * streamDist;

                    const distFromCenter = streamDist / (canvas.width * 0.7);
                    const alpha = (1 - distFromCenter) * 0.6;
                    const size = 1 + distFromCenter * 3;

                    // Color based on depth
                    const r = Math.floor(50 + tunnelProgress * 150);
                    const g = Math.floor(100 + Math.sin(tunnelProgress * Math.PI * 2) * 100);
                    const b = Math.floor(200 - tunnelProgress * 100);

                    ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${alpha})`;
                    ctx.fillRect(x - size / 2, y - size / 2, size, size);
                }

                // Spaghettification effect - vertical stretching
                const stretchFactor = 1 + Math.sin(elapsed * 0.002) * 0.05;
                ctx.save();
                ctx.translate(cx, cy);
                ...
                ctx.strokeStyle = `rgba(100, 150, 255, ${0.1 + tunnelProgress * 0.1})`;
                ctx.lineWidth = 1;
                const gridSpacing = 50;
                for (let x = 0; x < canvas.width; x += gridSpacing) {
                    ctx.beginPath();
                    for (let y = 0; y < canvas.height; y += 5) {
                        const distort = Math.sin(y * 0.05 + elapsed * 0.005) * 10;
                        ctx.lineTo(x + distort, y);
                    }
                    ctx.stroke();
                }
                ctx.restore();

                // Time dilation text effect
                if (Math.floor(elapsed / 2000) % 2 === 0) {
                    ctx.fillStyle = `rgba(150, 200, 255, ${0.3 * (1 - tunnelProgress)})`;
                    ctx.font = "italic 16px monospace";
                    ctx.textAlign = "center";
                    ctx.fillText("TRAVERSING SINGULARITY", cx, cy - 30);
                    ctx.fillText("TIME DILATION IN EFFECT", cx, cy + 30);
                }
            }

            // Phase 3: Emergence (27-30s) - Slowing down, white flash
            else {
                const emergeProgress = (elapsed - 27000) / 3000;

                // White flash as we exit
                const flashAlpha = emergeProgress * 0.8;
                ctx.fillStyle = `rgba(255, 255, 255, ${flashAlpha})`;
                ctx.fillRect(0, 0, canvas.width, canvas.height);

                // Scattered stars reappearing
                for (let i = 0; i < 100; i++) {
                    const x = (i * 137.5) % canvas.width;
                    const y = (i * 239.5) % canvas.height;
                    const alpha = emergeProgress * (Math.random() * 0.5 + 0.5);
        ...
        if (Date.now() > this.deathEndTime) {
            this.resetGame();
        } else {
            const remaining = Math.ceil((this.deathEndTime - Date.now()) / 1000);
            if (elapsed > 4000 || !this.supernova) {
                ctx.fillStyle = "#333";
                ctx.font = "20px monospace";
                ctx.textAlign = "center";
                ctx.fillText(`CRITICAL FAILURE`, canvas.width / 2, canvas.height / 2 - 20);
                ctx.fillText(`REBOOT IN ${remaining}s`, canvas.width / 2, canvas.height / 2 + 20);
            }
        }
    }
    ...
    openImageLightbox(src, caption) {
        ...
        const caps = document.getElementById('lightboxCaption');
        if (modal && img) {
            img.src = src;
    ...
    resetGame() {
        ...
        const hud = document.getElementById('flightHUD');
        if (hud && this.flightMode) hud.classList.remove('hidden');
    }
}
