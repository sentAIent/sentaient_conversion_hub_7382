...
for orig_x in range(60, -60, -2):
    ...
    # 1.0 on the front face, smooth exponential decay on the back face
    crestMask = 1.0 if orig_x >= 0 else math.exp(-math.pow(orig_x * 0.03, 2.0))
    ...
    # To create an "O" shape, we need the very tip to tuck back slightly in X.
    # But only if it doesn't cause a knot!
    # Instead of tucking back X, what if it just drops straight down?
