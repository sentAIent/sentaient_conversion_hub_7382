import"./modulepreload-polyfill-B5Qt9EMX.js";import{s as g,i as h,a as C,p as x}from"./controls_v3-By9psPOH.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";const c={sun:{name:"Sun",icon:"☀️",create:e=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="32" height="32">
            <g>
        ...
        </svg>`},sun2:{name:"Sun 2",icon:"🌞",create:()=>null},default:{name:"Default",icon:"↖",create:()=>null}};let n="lotus",s=null,d=1;function b(){g.currentTheme=localStorage.getItem("mindwave_theme")||"default";const e=localStorage.getItem("mindwave_cursor_shape");e&&c[e]?n=e:(n="default",localStorage.setItem("mindwave_cursor_shape","default"));const t=localStorage.getItem("mindwave_cursor_color");t&&(s=t),l(),window.addEventListener("themeChanged",r=>{g.currentTheme=r.detail.name,l()})}function u(){return s||getComputedStyle(document.documentElement).getPropertyValue("--accent").trim()||"#60a9ff"}function S(){return getComputedStyle(document.documentElement).getPropertyValue("--accent-secondary").trim()||"#fb7185"}function l(){let e=document.getElementById("dynamic-cursor-styles");if(e||(e=document.createElement("style"),e.id="dynamic-cursor-styles",document.head.appendChild(e)),n==="default"){e.textContent="";return}const t=u(),r=S();if(n==="sun2"){e.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./tribal-sun-cursor.png') 16 16, auto !important; }`;return}if(n==="mindwave"){e.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./mindwave-cursor.png') 31 31, auto !important; }`;return}const a=c[n].create(t,r);if(!a){e.textContent="";return}let i=a;d!==1&&(i=i.replace("<svg ",`<svg opacity="${d}" `));const o=`url("data:image/svg+xml,${encodeURIComponent(i)}") 16 16, auto`;e.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: ${o} !important; }`}function f(e){c[e]&&(n=e,localStorage.setItem("mindwave_cursor_shape",e),l(),document.querySelectorAll(".cursor-option").forEach(t=>{const r=t.dataset.shape===e;t.classList.toggle("active",r),r?(t.classList.add("bg-[var(--accent)]/20","border-[var(--accent)]"),t.classList.remove("bg-white/5","border-white/10")):(t.classList.remove("bg-[var(--accent)]/20","border-[var(--accent)]"),t.classList.add("bg-white/5","border-white/10"))}))}function $(e){d=e,l()}function m(e){s=e,e?localStorage.setItem("mindwave_cursor_color",e):localStorage.removeItem("mindwave_cursor_color"),l();const t=document.getElementById("cursorColorPicker");t&&e&&(t.value=e);const r=document.getElementById("cursorColorPreview");r&&(r.style.backgroundColor=u())}function y(){m(null)}function E(){console.log("[Cursor] Rendering UI");const e=document.getElementById("themeGrid");if(!e)return;const t=document.getElementById("cursorSection");t&&t.remove();const r=document.createElement("div");r.id="cursorSection",r.className="mt-6 pt-6 border-t border-white/10",r.innerHTML=`
        ...
        <div class="grid grid-cols-3 sm:grid-cols-5 gap-2" id="cursorShapeGrid">
            ${Object.entries(c).map(([o,p])=>{const w={sun:"☀️",moon:"🌙",heart:"❤️",mindwave:'<img src="./mindwave-cursor.png" width="24" height="24" style="display:inline-block;">',sun2:"🌞",default:"🖱️"}[o]||"🖱️";return`<button class="cursor-option p-3 rounded-xl text-center transition-all border ${o===n?"active bg-[var(--accent)]/20 border-[var(--accent)]":"bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20"}" data-shape="${o}" title="${p.name}"><span class="text-2xl block">${w}</span><div class="text-[10px] mt-1 font-semibold" style="color: var(--text-main);">${p.name}</div></button>`}).join("")}
        </div>`,e.parentElement.appendChild(r),r.querySelectorAll(".cursor-option").forEach(o=>o.addEventListener("click",()=>f(o.dataset.shape)));const a=r.querySelector("#cursorColorPicker");a&&a.addEventListener("input",o=>m(o.target.value));const i=r.querySelector("#resetCursorColor");i&&i.addEventListener("click",()=>{y();const o=getComputedStyle(document.documentElement).getPropertyValue("--accent").trim();r.querySelector("#cursorColorPreview")&&(r.querySelector("#cursorColorPreview").style.backgroundColor=o),a&&(a.value=o)})}window.setCursorShape=f;window.setCursorColor=m;window.setCursorOpacity=$;window.resetCursorColor=y;window.createCursorUIInThemeModal=E;window.NUCLEAR_MAIN_LOADED="V138_STABLE";const v=async()=>{console.log("[Main] Starting V138 Definitive Boot Sequence...");try{await h(),C(),x(),b();const e=document.getElementById("loadingScreen");e&&(e.style.opacity="0",setTimeout(()=>{e.remove(),console.log("[Main] System Operational.")},800))}catch(e){console.error("[Main] Critical Boot Failure:",e);const t=document.getElementById("loadingScreen");t&&t.remove()}};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v):v();
//# sourceMappingURL=mindwave-BDlK7_Zr.js.map
