...
// Find where class 17 starts and where class 19 (or else) starts.
const class17Regex = /\} else if \(classId === 17\) \{[\s\S]*?\} else if \(classId === 18\) \{[\s\S]*?\} else \{/;

const newShaders = `} else if (classId === 17) {
        return \`
        uniform float uTime;
        ...
        void main() {
            ...
            if(dO < MAX_DIST) {
                ...
                // Ultra-modern glowing material (no 1980s directional light)
        ...
        // UNROLLED Mandelbulb to avoid WebGL 1.0 loop crashes
        ...
        float map(vec3 p, int var, float t) {
            ...
            if (var == 0) {
                ...
                // Advanced Fractal Heart (Unrolled)
                p.xy *= rot(t*0.5); p.yz *= rot(t*0.3);
                ...
                // Recursive Crystal Lattice (Unrolled)
                p.xz *= rot(t*0.2);
        ...
        void main() {
            ...
            // Safe unrolled or bounded raymarching
            for(int i=0; i<MAX_STEPS; i++) {
                p = ro + rd * dO;
                float dS = map(p, var, t);
            ...
            if(dO < MAX_DIST) {
                vec3 n = getNormal(p, var, t);
                float fresnel = pow(1.0 - max(dot(n, -rd), 0.0), 1.5); // Modern edge glow
                vec3 baseCol = mix(uColor1, uColor2, clamp(p.y*0.5+0.5, 0.0, 1.0));
                
                col = baseCol * fresnel * 3.0; // Emissive core
                col += mix(uColor1, uColor2, n.x*0.5+0.5) * 0.8; // Iridescent skin
            } 
            
            // Abstract volumetric light fields
...
content = content.replace(class17Regex, newShaders);
