...
`,Ke=f=>f===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class Je{constructor(e){this.group=e,this.meshes=[],this.activeClassId=17,this.activeVariationId=9,this.materials={};const t=new Fe(new Uint8Array([0,0,0,0]),1,1,Re);t.needsUpdate=!0,this.dummyTexture=t;const i=new Q(2,2);for(let a=1;a<=25;a++){let o=Ke(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        uniform float uAudioHigh;`),o=o.replace("float intMod = max(0.1, uIntensity);","float intMod = max(0.1, uIntensity * (1.0 + uAudioLow * 3.0 + uAudioMid * 1.5));")),!o.includes("uniform float uMouseSync;")){let p="";/uniform\s+vec2\s+uMouse\s*;/.test(o)||(p+=`uniform vec2 uMouse;
        `),p+=`uniform float uMouseSync;
        void main() {`,o=o.replace("void main() {",p)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            `,transparent:!0,blending:T,depthWrite:!1}),this.particles=new H(t,this.particleMaterial),this.particleGroup.add(this.particles),this.particleGroup.visible=!1}initBox(){var h;this.boxOuter=new z;const e=new le(new ce(3,3,3)),t=new te({color:16777215,transparent:!0,opacity:.9,blending:T}),i=new te({color:3900150,transparent:!0,opacity:.5,blending:T});this.boxOuter.add(new ae(e,t));for(let l=1;l<=3;l++){const x=new ae(e,i);x.scale.setScalar(1+l*.012),this.boxOuter.add(x)}const a=new le(new ce(2,2,2)),s=new te({color:14742270,transparent:!0,opacity:.8,blending:T}),o=new te({color:6333946,transparent:!0,opacity:.4,blending:T});this.boxInner=new z,this.boxInner.add(new ae(a,s));for(let l=1;l<=2;l++){const x=new ae(a,o);x.scale.setScalar(1+l*.015),this.boxInner.add(x)}const r=new le(new ce(3.05,3.05,3.05)),n=new te({color:9684477,transparent:!0,opacity:.8,blending:T});this.boxEdges=new ae(r,n),this.boxGroup.add(this.boxOuter),this.boxGroup.add(this.boxInner),this.boxGroup.add(this.boxEdges),this.boxGroup.visible=!1;const p=((h=this.customColors)==null?void 0:h.box)||this.customColor;p&&(this.boxOuter.children.forEach(l=>l.material.color.copy(p)),this.boxInner.children.forEach(l=>l.material.color.copy(p)),this.boxEdges&&this.boxEdges.material&&this.boxEdges.material.color.copy(p))}initDragon(){var x;this.dragonDummy=new Ee,this.dragonLength=80;const e=new re(.8,1),t=new R({color:15680580,wireframe:!0,transparent:!0,opacity:.8,blending:T}),i=new re(.5,1),a=new R({color:16096779,wireframe:!1,transparent:!0,opacity:.6,blending:T});this.dragonBodyInstanced=new ye(e,t,this.dragonLength),this.dragonGlowInstanced=new ye(i,a,this.dragonLength);const s=new fe(1.5,3.5,5);s.rotateX(Math.PI/2);const o=new R({color:16638023,wireframe:!0,transparent:!0,opacity:.9,blending:T});this.dragonHead=new P(s,o),this.dragonPearlGroup=new z;const r=new ne(1,16,16),n=new R({color:3718648,wireframe:!1,transparent:!0,opacity:.9,blending:T}),p=new ne(1.3,16,16),h=new R({color:8246268,wireframe:!0,transparent:!0,opacity:.5,blending:T});this.dragonPearl=new P(r,n),this.dragonPearlHalo=new P(p,h),this.dragonPearlGroup.add(this.dragonPearl),this.dragonPearlGroup.add(this.dragonPearlHalo),this.dragonGroup.add(this.dragonBodyInstanced),this.dragonGroup.add(this.dragonGlowInstanced),this.dragonGroup.add(this.dragonHead),this.dragonGroup.add(this.dragonPearlGroup),this.dragonGroup.visible=!1;const l=((x=this.customColors)==null?void 0:x.dragon)||this.customColor;l&&this.updateDragonColor(l)}updateGalaxyColor(e){if(this.galaxySunMesh&&this.galaxySunMesh.traverse(t=>{t.isMesh&&t.material&&t.material.color.copy(e)}),this.galaxyStars&&this.galaxyStars.geometry){const t={};e.getHSL(t);const i=(t.h+.5)%1,a=this.galaxyStars.geometry.getAttribute("color");if(a){const s=a.count,o=this._tempColor;for(let r=0;r<s;r++){const n=r/s,p=n<.2?.8:n<.5?.6:.4+Math.random()*.15,h=.6+Math.random()*.3;o.setHSL(i,h,p),a.setXYZ(r,o.r,o.g,o.b)}a.needsUpdate=!0}}}updateDragonColor(e){if(!this.dragonBodyInstanced)return;this.dragonBodyInstanced.material.color.copy(e),this.dragonHead.material.color.copy(e);const t={};e.getHSL(t);const i=(t.h+.5)%1,a=new G().setHSL(i,t.s,t.l);this.dragonGlowInstanced.material.color.copy(a)}initGalaxy(){const e=this.batterySaver?500:1500,t=new W,i=[],a=[],s=[];for(let n=0;n<e;n++){const p=n/e*Math.PI*10,h=2.5+n/e*20+Math.random()*2,l=n%4*(Math.PI*2/4),x=Math.max(.5,n/e*4),c=Math.cos(p+l)*h+(Math.random()-.5)*x,u=(Math.random()-.5)*1.5,v=Math.sin(p+l)*h+(Math.random()-.5)*x;i.push(c,u,v);const A=n/e;A<.2?a.push(1,.95,.7):A<.5?a.push(.7+Math.random()*.3,.8,1):a.push(.4+Math.random()*.2,.3+Math.random()*.3,.8+Math.random()*.2),s.push(Math.random()<.05?.4+Math.random()*.3:.08+Math.random()*.15)}t.setAttribute("position",new X(i,3)),t.setAttribute("color",new X(a,3)),t.setAttribute("size",new X(s,1));const o=this.createStarTexture(),r=new ue({size:.25,vertexColors:!0,map:o,transparent:!0,opacity:.9,blending:T,depthWrite:!1,sizeAttenuation:!0});this.galaxyStars=new H(t,r),this.galaxyGroup.add(this.galaxyStars),this.galaxySunStyle=this.galaxySunStyle||"sun",this.createGalaxySun(this.galaxySunStyle),this.galaxyGroup.visible=!1}createGalaxySun(e){this.galaxySunMesh&&(this.galaxyGroup.remove(this.galaxySunMesh),this.galaxySunMesh=null),this.galaxySunMesh=new z;const t=new R({color:4892415,transparent:!0,opacity:.85,blending:T,depthWrite:!1,side:Z}),i=new z;if(e==="sun2"){const a=t.clone();a.side=ke;const s=new be(1.5,.25,16,64);i.add(new P(s,a));const o=8,r=new fe(.4,2.7,4);r.translate(0,2.7/2,0);const n=new fe(.2,1.7,4);n.translate(0,1.7/2,0);for(let p=0;p<o;p++){const h=p/o*Math.PI*2,l=new P(r,a);l.rotation.z=-h,l.position.set(Math.sin(h)*1.5,Math.cos(h)*1.5,0),i.add(l);const x=h+Math.PI/o,c=new P(n,a);c.rotation.z=-x,c.position.set(Math.sin(x)*1.5,Math.cos(x)*1.5,0),i.add(c)}}else if(e==="sun3"){const a={uTime:{value:0},uColor:{value:t.color},uBassIntt:{value:0}};this.galaxySunUniforms=a;const s=`
                varying vec2 vUv;
            ...
            `,r=new O({vertexShader:s,fragmentShader:o,uniforms:a,transparent:!0,blending:T,depthWrite:!1,side:Z}),n=new ne(2,64,64),p=new P(n,r);i.add(p);const h=new R({color:t.color,transparent:!0,opacity:.15,blending:T,depthWrite:!1,side:Be}),l=new ne(3,32,32);i.add(new P(l,h))}else{const a=new be(1.5,.12,8,64);i.add(new P(a,t));const s=8;for(let o=0;o<s;o++){const r=o/s*Math.PI*2,n=new we;n.moveTo(-.4,0),n.lineTo(.4,0),n.lineTo(0,2.7),n.lineTo(-.4,0);const p=new Ce(n),h=new P(p,t);h.rotation.z=-r,h.position.set(Math.sin(r)*1.5,Math.cos(r)*1.5,0),i.add(h);const l=r+Math.PI/s,x=new we;x.moveTo(-.2,0),x.lineTo(.2,0),x.lineTo(0,1.7),x.lineTo(-.2,0);const c=new Ce(x),u=new P(c,t);u.rotation.z=-l,u.position.set(Math.sin(l)*1.5,Math.cos(l)*1.5,0),i.add(u)}}this.galaxySunMesh.add(i),this.galaxySunMesh.position.set(0,0,.1),this.galaxyGroup.add(this.galaxySunMesh)}setGalaxySunStyle(e){e!=="sun"&&e!=="sun2"&&e!=="sun3"||(this.galaxySunStyle=e,this.galaxyGroup&&this.galaxyGroup.children.length>0&&this.createGalaxySun(e))}setGalaxySunRotation(e,t){e==="x"&&(this.sunRotationSpeedX=parseFloat(t)),e==="y"&&(this.sunRotationSpeedY=parseFloat(t)),e==="z"&&(this.sunRotationSpeedZ=parseFloat(t))}createStarTexture(){if(this.textures.star)return this.textures.star;const e=64,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e/2);return o.addColorStop(0,"rgba(255, 255, 255, 1.0)"),o.addColorStop(.1,"rgba(255, 255, 240, 0.8)"),o.addColorStop(.25,"rgba(255, 255, 200, 0.3)"),o.addColorStop(.5,"rgba(200, 200, 255, 0.1)"),o.addColorStop(1,"rgba(100, 100, 255, 0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.fillStyle="rgba(255, 255, 255, 0.6)",i.beginPath(),i.moveTo(0,s-1),i.lineTo(a,s-.5),i.lineTo(e,s-1),i.lineTo(e,s+1),i.lineTo(a,s+.5),i.lineTo(0,s+1),i.closePath(),i.fill(),i.beginPath(),i.moveTo(a-1,0),i.lineTo(a-.5,s),i.lineTo(a-1,e),i.lineTo(a+1,e),i.lineTo(a+.5,s),i.lineTo(a+1,0),i.closePath(),i.fill(),i.beginPath(),i.arc(a,s,2,0,Math.PI*2),i.fillStyle="rgba(255, 255, 255, 1.0)",i.fill(),this.textures.star=new ie(t),this.textures.star}initMandala(){var s;this.mandalaRings=[];const e=[2450411,15357964,3900150,16347926,12573694];for(let o=0;o<5;o++){const r=1.2+o*.8,n=6+o*6,p=new Ne(r-.05,r+.05,n),h=new R({color:e[o],side:Z,transparent:!0,opacity:.4-o*.05,blending:T}),l=new P(p,h);l.userData={speed:(.01+o*.005)*(o%2===0?1:-1),segments:n},this.mandalaRings.push(l),this.mandalaGroup.add(l)}const t=new We(.3,32),i=new R({color:16347926,transparent:!0,opacity:.6,blending:T});this.mandalaCenter=new P(t,i),this.mandalaGroup.add(this.mandalaCenter),this.mandalaGroup.visible=!1;const a=(s=this.customColors)==null?void 0:s.mandala;a&&(this.mandalaRings.forEach(o=>o.material.color.copy(a)),this.mandalaCenter.material.color.copy(a))}createSnowflakeTexture(){if(this.textures.snowflake)return this.textures.snowflake;const e=128,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e*.5);o.addColorStop(0,"rgba(200,230,255,0.5)"),o.addColorStop(.4,"rgba(180,220,255,0.15)"),o.addColorStop(1,"rgba(150,200,255,0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.strokeStyle="rgba(220,240,255,1.0)",i.lineCap="round";for(let r=0;r<6;r++){const n=r/6*Math.PI*2;i.save(),i.translate(a,s),i.rotate(n),i.lineWidth=2.5,i.beginPath(),i.moveTo(0,0),i.lineTo(0,-52),i.stroke();const p=[{d:18,len:14,angle:Math.PI/4},{d:32,len:20,angle:Math.PI/4},{d:46,len:10,angle:Math.PI/5}];i.lineWidth=1.5,p.forEach(({d:h,len:l,angle:x})=>{[1,-1].forEach(c=>{i.beginPath(),i.moveTo(0,-h),i.lineTo(c*l*Math.cos(Math.PI/2-x),-h-l*Math.sin(Math.PI/2-x)),i.stroke()})}),i.restore()}i.beginPath();for(let r=0;r<6;r++){const n=r/6*Math.PI*2-Math.PI/6,p=a+Math.cos(n)*4,h=s+Math.sin(n)*4;r===0?i.moveTo(p,h):i.lineTo(p,h)}return i.closePath(),i.fillStyle="rgba(255, 255, 255, 0.8)",i.fill(),this.textures.snowflake=new ie(t),this.textures.snowflake}initSnowflake(){for(;this.snowflakeGroup.children.length>0;){const c=this.snowflakeGroup.children[0];this.snowflakeGroup.remove(c),c.geometry&&c.geometry.dispose(),c.material&&c.material.dispose()}this._snowData=null;const e=700,t=new Float32Array(e*3),i=new Float32Array(e),a=new Float32Array(e),s=new Float32Array(e),o=new Float32Array(e),r=new Float32Array(e),n=new Float32Array(e);for(let c=0;c<e;c++){const u=c*3;t[u]=(Math.random()-.5)*80,t[u+1]=(Math.random()-.5)*60,t[u+2]=-40+Math.random()*35;const v=(t[u+2]+40)/35;i[c]=1.5+v*8,a[c]=.2+v*.6,s[c]=Math.random()*Math.PI*2,o[c]=.015+Math.random()*.04+v*.03,r[c]=.01+Math.random()*.02,n[c]=.4+Math.random()*.8}const p=new W;p.setAttribute("position",new B(t,3)),p.setAttribute("aSize",new B(i,1)),p.setAttribute("aOpacity",new B(a,1));const h=this.createSnowflakeTexture(),l=new O({uniforms:{uTexture:{value:h},uColor:{value:this.customColor?this.customColor.clone():new G(10875883)},uIntensity:{value:0},uSizeMultiplier:{value:1},uGlowAmount:{value:.5}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),x=new H(p,l);this.snowflakeGroup.add(x),this._snowData={count:e,positions:t,phases:s,speeds:o,drifts:r,driftFreqs:n,points:x,material:l,spinMeshes:[],spinSpeeds:[]}}setSnowSize(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uSizeMultiplier.value=Math.max(.2,Math.min(4,e)))}setSnowGlow(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uGlowAmount.value=Math.max(0,Math.min(1,e)))}initLava(){var o;this.lavaBlobs=[];const e=16;this.lavaUniforms={uBlobs:{value:[]},uColor:{value:new G(((o=g.visualColors)==null?void 0:o.lava)||16737792)},uSecondaryColor:{value:new G(16755200)},uTime:{value:0},uIntensity:{value:0},uResolution:{value:new de(window.innerWidth,window.innerHeight)}};for(let r=0;r<e;r++)this.lavaUniforms.uBlobs.value.push(new He(0,-100,0,0));const t=new O({uniforms:this.lavaUniforms,vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
            ...
            `,transparent:!0,blending:T,depthWrite:!1,side:Z}),i=new Q(100,100),a=new P(i,t);a.position.z=-5,[{count:5,minSize:1,maxSize:1.5},{count:7,minSize:1.5,maxSize:2.2},{count:4,minSize:2.2,maxSize:3.5}].forEach(r=>{for(let n=0;n<r.count;n++){const p=r.minSize+Math.random()*(r.maxSize-r.minSize),h=["heating","rising","cooling","falling"],l=h[Math.floor(Math.random()*h.length)],x=-18+Math.random()*4,c=18+Math.random()*4;let u=0,v=.5;l==="heating"?(u=x,v=Math.random()*.5):l==="rising"?(u=x+Math.random()*(c-x),v=.8+Math.random()*.2):l==="cooling"?(u=c,v=1-Math.random()*.3):l==="falling"&&(u=c-Math.random()*(c-x),v=.2+Math.random()*.3),this.lavaBlobs.push({position:new Xe((Math.random()-.5)*12,u,(Math.random()-.5)*6),material:{uniforms:{uTemp:{value:0},uIntensity:{value:0},uTime:{value:0}}},userData:{baseSize:p,state:l,temperature:v,floatMin:x,floatMax:c,heatRate:.15+Math.random()*.3,coolRate:.15+Math.random()*.3,riseSpeed:(.04+Math.random()*.05)/(p*.5),fallSpeed:(.05+Math.random()*.05)/(p*.5),driftPhase:Math.random()*Math.PI*2,driftSpeed:.01+Math.random()*.02}})}}),this.lavaGroup.add(a),this.lavaGroup.visible=!1}initFireplace(){const i=new Q(100,80);this.fireMaterial=this.createFireShader(),this.fireMesh=new P(i,this.fireMaterial),this.fireMesh.position.set(0,0,-15),this.fireMesh.renderOrder=10,this.fireplaceGroup.add(this.fireMesh),this.fireLight=new je(16737792,5,200),this.fireLight.position.set(0,0,30),this.fireplaceGroup.add(this.fireLight);const a=650,s=new W,o=[];for(let r=0;r<a;r++)o.push((Math.random()-.5)*100,-20+Math.random()*40,-15+(Math.random()-.5)*20);s.setAttribute("position",new X(o,3)),this.emberMat=new ue({color:16763904,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.8,blending:T,depthWrite:!1}),this.embers=new H(s,this.emberMat),this.fireplaceGroup.add(this.embers),this.emberVelocities=new Float32Array(a);for(let r=0;r<a;r++)this.emberVelocities[r]=.02+Math.random()*.05;this.fireplaceGroup.position.set(0,0,0),this.fireplaceGroup.visible=!1}createFireShader(){return new O({uniforms:{uTime:{value:0},uColor:{value:new G(16729088)},uSpeed:{value:1},uIntensity:{value:1}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            ...
            `,transparent:!0,side:Z,blending:T,depthWrite:!1})}initRainforest(){const e=this.batterySaver?400:1e3,t=new W,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*40,i[o+2]=(Math.random()-.5)*40,a[o]=Math.random(),a[o+1]=Math.random(),a[o+2]=.08+Math.random()*.12}t.setAttribute("position",new B(i,3)),t.setAttribute("aRandom",new B(a,3)),this.rainMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new G(8965375)},uIntensity:{value:.6}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),this.raindrops=new H(t,this.rainMaterial),this.rainforestGroup.add(this.raindrops),this.rainforestGroup.visible=!1}initZenGarden(){const e=this.batterySaver?200:500,t=new W,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let o=0;o<e;o++){const r=o*3;i[r]=(Math.random()-.5)*40,i[r+1]=(Math.random()-.5)*20,i[r+2]=(Math.random()-.5)*40,a[r]=Math.random(),a[r+1]=Math.random(),a[r+2]=Math.random()*Math.PI*2}t.setAttribute("position",new B(i,3)),t.setAttribute("aRandom",new B(a,3)),this.petalMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new G(16758725)},uIntensity:{value:.8}},vertexShader:`
                attribute vec3 aRandom;
                uniform float uTime, uSpeed;
                varying float vAlpha;
                void main() {
            ...
            `,wireframe:!0,transparent:!0,side:Z})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new W,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new X(a,3));const s=new ue({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new H(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let p="🪷MINDWAVE";const h=e.logicMode,l=e.customText;h==="custom"||h==="txt"?p="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(h==="random"||h==="rnd"||h==="matrix"||h==="int"||h==="interstellar")&&(p="");const x=h==="matrix"||h==="int"||h==="interstellar"?[]:["LOGO",...p],c=x.length;for(let v=0;v<64;v++){const A=v%8,q=Math.floor(v/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(A*r,q*n,r,n),a.save(),a.translate(A*r+r/2,q*n+n/2);let I="",S=!1;if(v<c){const _=x[v];_==="LOGO"?S=!0:I=_}else{const w="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";I=w.charAt(Math.floor(Math.random()*w.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(I,0,0),a.restore(),I="")}if(I||S)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",S)if(this.logoImage){const L=document.createElement("canvas");L.width=100,L.height=100;const V=L.getContext("2d");V.imageSmoothingEnabled=!0,V.imageSmoothingQuality="high",V.drawImage(this.logoImage,0,0,100,100);const K=V.getImageData(0,0,100,100),F=K.data;for(let U=0;U<F.length;U+=4){const J=180+(F[U]+F[U+1]+F[U+2])/3/255*75;F[U]=J,F[U+1]=J,F[U+2]=J}V.putImageData(K,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(L,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new Ye().load("./mindwave-logo-icon.png",w=>{if(this.logoImage=w,this.logoLoading=!1,this.cyberMaterial){const L=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=L}},void 0,w=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(I,0,0);a.restore()}const u=new ie(i);return u.magFilter=Me,u.minFilter=Me,u}createCyberShader(e,t=this.matrixConfig){return new O({uniforms:{uTexture:{value:e},uColor:{value:new G(t.color||65345)},uHeadColor:{value:new G(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
