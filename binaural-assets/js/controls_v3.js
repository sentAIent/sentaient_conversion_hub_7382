...
function setupGalaxyControls() {
    ...
    const updateSun = () => {
        const viz = getVisualizer();
        const vx = rx ? parseFloat(rx.value) : 0;
        const vy = ry ? parseFloat(ry.value) : 0.5;
        const vz = rz ? parseFloat(rz.value) : 0;

        // Update Labels
        if (valX) valX.textContent = vx.toFixed(1);
        if (valY) valY.textContent = vy.toFixed(1);
...
function updateTimerUI(data) {
    // Update countdown text (Panel & Dock)
    const t1 = document.getElementById('timeRemaining');
    const t2 = document.getElementById('sessionTimer-dock');
    if (t1) t1.textContent = data.formatted;
    if (t2) t2.textContent = data.formatted;

    // Update progress ring (Panel & Dock)
    ...
    const pr1 = document.getElementById('timerProgress');
    const pr2 = document.getElementById('timerProgress-dock');
    if (pr1) pr1.style.strokeDashoffset = offset;
    if (pr2) pr2.style.strokeDashoffset = offset;
}
...
// --- NOW PLAYING HELPER ---
// --- NOW PLAYING HELPER (Multi-HUD Support) ---
window.updateMiniNowPlaying = function(text) {
    const labels = document.querySelectorAll('[id^="miniNowPlaying_"]');
    labels.forEach(el => {
        el.textContent = text || 'Standing By';
        el.style.opacity = '0';
...
export function setVisualMode(mode, forceState = null, isManual = false) {
    ...
    if (viz) {
        ...
        // RESTORE STACKING: Clicks simply toggle their individual mode.
        // Array modes (presets) clear existing state, but individual clicks STACK.
        ...
        modes.forEach(m => {
            if (forceState !== null) {
                ...
                // Individual clicks toggle on/off without affecting others
                viz.toggleMode(m);
            }
        ...
        // Sync config panel if currently viewing the Config tab
    ...
    els.profileBtn.addEventListener('click', () => {
        ...
        import('../services/firebase.js').then(({ getAuth }) => {
            ...
            if (auth && auth.currentUser) {
                // User is logged in -> Show Profile Modal with Stats
                if (window.openProfile) {
                    window.openProfile();
                } else if (els.profileModal) {
                    els.profileModal.classList.add('active', 'flex');
...
if (els.statsBtn) {
    els.statsBtn.addEventListener('click', () => {
        if (els.statsModal) {
            els.statsModal.classList.add('active', 'flex');
            els.statsModal.classList.remove('hidden');
            // Populate stats if function exists
