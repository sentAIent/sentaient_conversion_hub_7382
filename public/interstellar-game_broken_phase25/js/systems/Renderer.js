...
class RenderManager {
    ...
    drawBackgroundElements() { this.celestial.drawBackground(this.game.ctx, Date.now()); }
    renderNebulaHazards(ctx, time) { this.celestial.renderNebulaHazards(ctx, time); }
    drawWarpTunnel(ctx, time, intensity) { this.celestial.drawWarpTunnel(ctx, time, intensity); }
    ...
    draw(time) {
        ...
        // 1. CLEAR - Absolute Black Void
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        
        // 2. CELESTIAL BACKGROUND (RESTORED PHASE 24)
        try {
            this.celestial.draw(ctx, time);
        } catch (e) {
            console.error("❌ [Renderer] Celestial failure:", e);
        }
        ...
        try { this.entity.draw(ctx, time); } catch (e) { console.error("❌ [Renderer] Entity failure:", e); }
        try { this.combat.draw(ctx, time); } catch (e) { console.error("❌ [Renderer] Combat failure:", e); }
        try { this.hazard.draw(ctx, time); } catch (e) { console.error("❌ [Renderer] Hazard failure:", e); }
    ...
    renderLegacyEntities(ctx, time, rotCenterX, rotCenterY, ship, viewDist) {
        ...
        if (this.game.spaceMines) {
            this.game.spaceMines.forEach(mine => {
                ...
                ctx.beginPath();
                ctx.arc(0, 0, (mine.size || 5) * rotated.scale / this.game.camera.zoom, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            });
        }
