...
class InfinityGate extends AncientGateway {
    ...
    triggerJump() {
        ...
        const legacyPoints = this.game.legacyData?.legacyPoints || 0;
        if (legacyPoints < 10) {
...
class WarpGateManager {
    ...
    getNetwork() {
        return this.gates.map((g, index) => ({
            index, name: g.name, x: g.x, y: g.y, type: g.type
        }));
    }
...
if (typeof window !== 'undefined') {
    ...
    window.DysonSwarm = DysonSwarm;
    window.RingWorld = RingWorld;
    window.GreatBeacon = GreatBeacon;
    window.AncientGateway = AncientGateway;
    window.DeepSpaceOutpost = DeepSpaceOutpost;
    window.InfinityGate = InfinityGate;
    window.WarpGateManager = WarpGateManager;
    window.MegaStructures = WarpGateManager; // Alias
}
