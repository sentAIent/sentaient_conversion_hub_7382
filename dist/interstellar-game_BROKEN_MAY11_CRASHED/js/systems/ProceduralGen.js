class ProceduralManager {
    constructor(game) {
        this.game = game;
        console.log('[Procedural] Manager Instantiated successfully.');
    }
    ...
    generateBackground() {
        ...
        this.game.backgroundStars = bgStars;
        return bgStars;
    }
    ...
    updateUniverse() {
        ...
        const sector = this.getSectorCoords(this.game.playerShip.x, this.game.playerShip.y);
        ...
        // Cleanup sectors that are too far away
        ...
        for (const [key] of this.game.loadedSectors.entries()) {
            ...
            if (dist > cleanupRadius) {
                // O(1) cleanup: simply delete the sector data.
                // Entities are now contained within the sector and will be GC'd.
    ...
    generateSingleStyle(style) {
        ...
        // Standardize style names (e.g. 'nebula-blue' -> 'nebula')
        ...
        switch (baseStyle) {
            ...
            case 'matrix':
                ...
                break;
            default:
                // Fallback to deep space for unknown styles
                this.generateDeepSpaceStyle();
        }
    }
    ...
    generateDeepSpaceStyle() {
        ...
        // 2. Galaxies: Large, rotating spiral and elliptical clusters
        const galaxyCount = 5 + Math.floor(Math.random() * 5);
        for (let i = 0; i < galaxyCount; i++) {
            ...
            this.game.galaxies.push({
                ...
                color: Math.random() > 0.5 ? '#60A9FF' : '#FF60A9',
                type: Math.random() > 0.3 ? 'spiral' : 'elliptical',
                alpha: 0.3 + Math.random() * 0.4
            });
        }

        // 3. Black Holes: Ambient gravitational lenses (rare)
        if (Math.random() < 0.3) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 5000 + Math.random() * 3000;
        ...
        // 4. Distant Nebulae: Subtle volumetric clouds
        const nebCount = 3 + Math.floor(Math.random() * 4);
        for (let i = 0; i < nebCount; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 4000 + Math.random() * 5000;
            this.game.nebulae.push({
                x: Math.cos(angle) * dist,
                y: Math.sin(angle) * dist,
                z: (Math.random() - 0.5) * 3000,
                size: 1500 + Math.random() * 2000,
                color: Math.random() > 0.5 ? '#200040' : '#002040',
                alpha: 0.15 + Math.random() * 0.1
            });
        }

        // 5. Ambient Planets: Distant, non-interactive worlds
        const planetCount = 2 + Math.floor(Math.random() * 3);
        for (let i = 0; i < planetCount; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 2500 + Math.random() * 3500;
            const type = ['gas-giant', 'ice-giant', 'terrestrial'][Math.floor(Math.random() * 3)];
            this.game.planets.push({
                ...
                atmosphereColor: type === 'ice-giant' ? '#80ffff' : '#87CEEB',
                textureSeed: Math.random() * 1000
            });
        }

        // 6. Shooting Stars: Pre-populate a few active ones
    ...
    generateNebulaStyle() {
        ...
        // 1. Nebulae: Dense volumetric clusters
        const nebCount = 12 + Math.floor(Math.random() * 8);
        const colors = ['#0033ff', '#3300ff', '#0099ff', '#6600ff'];
        for (let i = 0; i < nebCount; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = Math.random() * 8000;
            this.game.nebulae.push({
                x: Math.cos(angle) * dist,
                y: Math.sin(angle) * dist,
                z: (Math.random() - 0.5) * 4000,
    ...
    generateAlienStyle() {
        ...
        // Alien Fleets: Ambient, non-interactive background traffic
        const fleetCount = 8 + Math.floor(Math.random() * 8);
