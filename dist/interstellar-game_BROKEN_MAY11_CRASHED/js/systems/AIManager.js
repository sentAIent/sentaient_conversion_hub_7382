...
class AIManager {
    constructor(game) {
        ...
        this.outposts = [];
        this.ships = this.game.enemyShips; // Global Sync
        this.projectiles = this.game.enemyBullets; // Global Sync to enemyBullets
    ...
    getGlobalDifficulty() {
        ...
        // 1. Sector Difficulty (Base)
        if (this.game.sectorManager) {
            const sector = this.game.sectorManager.getCurrentSectorData();
            if (sector) diff *= sector.difficulty;
        }

        // 2. Player Progression (Ship Tier)
        ...
        if (ship) {
            ...
            diff *= (shipTiers[ship.type] || 1.0);
        }

        // 3. Economic Progression
        const wealth = (this.game.credits || 0) + (this.game.playerGems || 0) * 100;
        if (wealth > 100000) diff *= 1.5;
        else if (wealth > 50000) diff *= 1.25;
    ...
    seedInitialFactions() {
        ...
        if (planets.length < 2) return; // Lowered from 5 to 2 for better early pop
        ...
        if (player) {
            for (let i = this.ships.length - 1; i >= 0; i--) {
                ...
                if (ship.health !== undefined && ship.health <= 0) {
                    if (ship.shipClass === 'freighter') this.dropConvoyLoot(ship);
                ...
                if (dist < 2500) {
                    ...
                    if (ship.shipClass === 'freighter' && ship.targetOutpost) {
    ...
    updateFormations(dt) {
        ...
        factions.forEach(fKey => {
            const fShips = this.ships.filter(s => s.faction === fKey && s.shipClass === 'scout');
    ...
    executeAIFireLogic(ship, player, dist) {
        const diff = this.getGlobalDifficulty();
        const baseCooldown = 1200;
        const cooldown = baseCooldown / Math.max(1, diff * 0.8);
        
        if (!ship.lastShot || Date.now() - ship.lastShot > cooldown) {
            // Pick a weapon variant
            const variant = Math.random() < (0.3 * diff) ? 'pulsar' : 'void';
    ...
    maintainTradeRoutes() {
        const freighters = this.ships.filter(s => s.shipClass === 'freighter');
    ...
    maintainRivalFleets() {
        this.outposts.forEach(outpost => {
            const factionShips = this.ships.filter(s => s.faction === outpost.faction && s.shipClass !== 'freighter');
    ...
    spawnRivalShip(outpost) {
        ...
        this.ships.push(ship);
        console.log(`[AI] ${outpost.faction} deployed ${ship.shipClass} near ${outpost.name} (Diff: ${diff.toFixed(1)}x)`);
    }
    ...
    dropConvoyLoot(ship) {
        ...
        const sector = this.game.proceduralManager.getSectorCoords(ship.x, ship.y);
        const sectorKey = `${sector.x},${sector.y}`;
        const sectorData = this.game.loadedSectors.get(sectorKey);
        ...
        for (let i = 0; i < Math.min(bonusCount, 8); i++) {
            ...
            if (sectorData) {
                sectorData.minerals.push(m);
            } else {
                // Fallback to global if sector not loaded (unlikely)
                if (!this.game.minerals) this.game.minerals = [];
                this.game.minerals.push(m);
            }
        }
    }
