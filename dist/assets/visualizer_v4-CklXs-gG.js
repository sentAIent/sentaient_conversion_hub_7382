...
`,ot=m=>m===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class st{constructor(e){this.group=e,this.meshes=[],this.activeClassId=17,this.activeVariationId=9,this.materials={};const t=new ke(new Uint8Array([0,0,0,0]),1,1,Ie);t.needsUpdate=!0,this.dummyTexture=t;const i=new O,a=new Float32Array([-1,-1,.5,3,-1,.5,-1,3,.5]),s=new Float32Array([0,0,2,0,0,2]);i.setAttribute("position",new D(a,3)),i.setAttribute("uv",new D(s,2));for(let o=1;o<=25;o++){let n=ot(o);if(o<=16&&(n=n.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        uniform float uAudioHigh;`),n=n.replace("float intMod = max(0.1, uIntensity);","float intMod = max(0.1, uIntensity * (1.0 + uAudioLow * 3.0 + uAudioMid * 1.5));")),!n.includes("uniform float uMouseSync;")){let l="";/uniform\s+vec2\s+uMouse\s*;/.test(n)||(l+=`uniform vec2 uMouse;
        `),l+=`uniform float uMouseSync;
        void main() {`,n=n.replace("void main() {",l)}o<=21&&(n.includes("vec2 p = (vUv - 0.5) * 2.0;")?n=n.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            p -= uMouse * uMouseSync * 0.5;`):n.includes("vec2 uv = (vUv - 0.5) * 2.0;")?n=n.replace("vec2 uv = (vUv - 0.5) * 2.0;",`vec2 uv = (vUv - 0.5) * 2.0;
            uv -= uMouse * uMouseSync * 0.5;`):n.includes("vec2 up = (vUv - 0.5) * 2.0;")&&(n=n.replace("vec2 up = (vUv - 0.5) * 2.0;",`vec2 up = (vUv - 0.5) * 2.0;
            up -= uMouse * uMouseSync * 0.5;`)));const h=`
            ...
            `,depthWrite:!1,depthTest:!1,transparent:!0}),this.mesh.material=this.material,this.scene=new _e,this.scene.add(this.mesh)}resize(e,t){this.renderTarget.setSize(e,t),this.material.uniforms.uResolution.value.set(e,t)}render(e,t,i,a){this.renderer.setRenderTarget(this.renderTarget),this.renderer.clear(),this.renderer.render(e,t),this.material.uniforms.uTime.value+=i;const s=Math.max(0,a-.7)*3;this.material.uniforms.uAberration.value=5*s,this.material.uniforms.uBloomIntensity.value=.5+a*1.5,this.renderer.setRenderTarget(null),this.renderer.clear(),this.renderer.render(this.scene,this.camera)}dispose(){this.renderTarget.dispose(),this.geometry.dispose(),this.material.dispose()}}class nt{constructor(e,t=5e3){this.scene=e,this.count=t,this.geometry=new O,this.positions=new Float32Array(this.count*3),this.velocities=new Float32Array(this.count*3),this.colors=new Float32Array(this.count*3),this.sizes=new Float32Array(this.count);const i=new _(3718648),a=new _(11032055);for(let h=0;h<this.count;h++){const f=h*3,l=20*Math.cbrt(Math.random()),g=Math.random()*2*Math.PI,c=Math.acos(2*Math.random()-1);this.positions[f]=l*Math.sin(c)*Math.cos(g),this.positions[f+1]=l*Math.sin(c)*Math.sin(g),this.positions[f+2]=l*Math.cos(c),this.velocities[f]=-this.positions[f+2]*.05+(Math.random()-.5),this.velocities[f+1]=(Math.random()-.5)*.5,this.velocities[f+2]=this.positions[f]*.05+(Math.random()-.5);const u=i.clone().lerp(a,Math.random());this.colors[f]=u.r,this.colors[f+1]=u.g,this.colors[f+2]=u.b,this.sizes[h]=Math.random()*2+.5}this.geometry.setAttribute("position",new D(this.positions,3)),this.geometry.setAttribute("color",new D(this.colors,3)),this.geometry.setAttribute("size",new D(this.sizes,1));const s=document.createElement("canvas");s.width=64,s.height=64;const o=s.getContext("2d"),r=o.createRadialGradient(32,32,0,32,32,32);r.addColorStop(0,"rgba(255,255,255,1)"),r.addColorStop(.2,"rgba(255,255,255,0.8)"),r.addColorStop(.6,"rgba(255,255,255,0.2)"),r.addColorStop(1,"rgba(0,0,0,0)"),o.fillStyle=r,o.fillRect(0,0,64,64);const n=new ee(s);this.material=new k({uniforms:{uTime:{value:0},uAudioLevel:{value:0},tSprite:{value:n}},vertexShader:`
                uniform float uTime;
                ...
                void main() {
                    ...
                    // Particles pulse size with audio
                    float audioPulse = 1.0 + (uAudioLevel * 2.0);
                    gl_PointSize = size * audioPulse * (300.0 / -mvPosition.z);
            ...
            `,fragmentShader:`
                ...
                varying vec3 vColor;
                void main() {
                    vec4 texColor = texture2D(tSprite, gl_PointCoord);
                    if (texColor.a < 0.1) discard;
                    gl_FragColor = vec4(vColor * texColor.rgb, texColor.a * 0.8);
                }
            `,transparent:!0,blending:T,depthWrite:!1}),this.mesh=new H(this.geometry,this.material),this.scene.add(this.mesh)}update(e,t){this.material.uniforms.uTime.value+=e,this.material.uniforms.uAudioLevel.value=t;const i=this.geometry.attributes.position.array,a=.5+t*5;for(let s=0;s<this.count;s++){const o=s*3,r=i[o],n=i[o+1],h=i[o+2],f=r*r+n*n+h*h,l=Math.sqrt(f)+.1,g=-(r/l)*a,c=-(n/l)*a,u=-(h/l)*a,v=t>.6?(Math.random()-.5)*2:0;this.velocities[o]+=(g+v)*e,this.velocities[o+1]+=(c+v)*e,this.velocities[o+2]+=(u+v)*e,this.velocities[o]*=.98,this.velocities[o+1]*=.98,this.velocities[o+2]*=.98,i[o]+=this.velocities[o]*e*10,i[o+1]+=this.velocities[o+1]*e*10,i[o+2]+=this.velocities[o+2]*e*10}this.geometry.attributes.position.needsUpdate=!0,this.mesh.rotation.y+=e*.2}dispose(){this.scene.remove(this.mesh),this.geometry.dispose(),this.material.dispose(),this.material.uniforms.tSprite.value.dispose()}}let p=null;class ze{constructor(e,t={}){this.canvas=e,this.activeModes=t.activeModes||new Set,this.mode=t.mode||"none",this.mindWaveMode=t.mindWaveMode!==void 0?t.mindWaveMode:!0,this.galaxySunStyle=t.galaxySunStyle||"sun",this.currentCymaticData={name:"Fundamental Zenith",n:1,m:1,energy:.2},this.cymaticsHistory=[],this.cymaticsHistoryIndex=-1,this._cymaticDriftOffset=Math.random()*1e3,this.cyberConfig={logicMode:t.cyberLogicMode||"matrix",customText:t.cyberCustomText||"WELCOME",angle:t.cyberAngle||0,speed:t.cyberSpeedMultiplier||1,length:1,color:"#00FF41",rainbow:!0},this.matrixConfig={logicMode:t.matrixLogicMode||"interstellar",customText:t.matrixCustomText||"MINDWAVE",angle:0,speed:1,length:1,color:"#00FF41",rainbow:!1},this.initialized=!1,this._rainbowEnabled=t.rainbowEnabled||!1,this.isVisualizer3D=!0,this.overlayCanvas=document.getElementById("cyberCanvas"),this.overlayCtx=this.overlayCanvas?this.overlayCanvas.getContext("2d",{alpha:!0}):null,this.overlayCanvas&&(this.resizeOverlayCanvas(),window.addEventListener("resize",()=>this.resizeOverlayCanvas())),this.matrixCyberStreams=[],this.currentCyberAngle=t.cyberAngle!==void 0?t.cyberAngle:0,this.cyberColorCustomized=!1;try{const a=localStorage.getItem("cyberThemeHistory");this.themeHistory=a?JSON.parse(a):[],this.lastCyberFamily=localStorage.getItem("cyberLastFamily")||""}catch{this.themeHistory=[],this.lastCyberFamily=""}this.currentLogoOpacity=.8,this.targetLogoOpacity=.8,p=this,this.themeType=document.body.dataset.themeType||"dark";const i=/Mobile|Android|iPhone|iPad/i.test(navigator.userAgent);this.batterySaver=t.batterySaver!==void 0?t.batterySaver:i,this.isLowPower=this.batterySaver||localStorage.getItem("mindwave_battery_saver")==="true"||document.body.classList.contains("system-stability-mode"),this.lastFrameRenderTime=0,this.targetFPS=this.isLowPower?30:60,this.vibrationEnabled=typeof x<"u"&&x.visualVibration!==void 0?x.visualVibration:t.visualVibration!==void 0?t.visualVibration:!1,this._fpsRingBuffer=new Float64Array(60),this._fpsRingIndex=0,this._fpsRingCount=0,this.lastLodDegradation=0,this.currentLodLevel="high",this._tempColor=new _,this._logoRenderCanvas=null,this.sphereGroup=new z,this.particleGroup=new z,this.lightspeedGroup=new z,this.lavaGroup=new z,this.fireplaceGroup=new z,this.rainforestGroup=new z,this.zenGardenGroup=new z,this.oceanGroup=new z,this.wavesGroup=new z,this.cyberGroup=new z,this.boxGroup=new z,this.dragonGroup=new z,this.galaxyGroup=new z,this.mandalaGroup=new z,this.cymaticsGroup=new z,this.snowflakeGroup=new z,this.swarmGroup=new z,this._snowData=null;try{this.scene=new _e,this.cymaticsCore=new st(this.cymaticsGroup),[this.sphereGroup,this.particleGroup,this.lightspeedGroup,this.lavaGroup,this.fireplaceGroup,this.rainforestGroup,this.zenGardenGroup,this.oceanGroup,this.wavesGroup,this.cyberGroup,this.boxGroup,this.dragonGroup,this.galaxyGroup,this.mandalaGroup,this.cymaticsGroup,this.snowflakeGroup,this.swarmGroup].forEach(o=>this.scene.add(o)),this.camera=new We(75,e.width/e.height,.1,1e3),this.renderer=new He({canvas:e,alpha:!0,antialias:!0,preserveDrawingBuffer:!0}),this.renderer.autoClear=!1,this.renderer.setClearColor(0,0),this.renderer.setSize(e.clientWidth,e.clientHeight);const s=this.isLowPower?1:2;this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,s)),this.postProcessor=new rt(this.renderer,e.clientWidth,e.clientHeight),this.particleSwarm=new nt(this.swarmGroup,2e3),this.textures={},this.customColors={},this.customColor=null,this._isRendering=!1,this._freqDataArray=null,this.initEnvironment(),this.camera.position.z=5,this.handleLayoutChange=this.handleLayoutChange.bind(this),window.addEventListener("resize",()=>{this.resize(),this.handleLayoutChange()}),window.addEventListener("mindwave:layout-change",this.handleLayoutChange),this._boundSafeMode=()=>{this.currentLodLevel!=="low"&&(this.degradeLOD(),this.degradeLOD())},window.addEventListener("mindwave:safe-mode-start",this._boundSafeMode),this._boundVisibilityChange=()=>{document.hidden||(this.lastTime=performance.now()*.001,this.active!==!1&&this.initialized&&this.render(x.analyserLeft,x.analyserRight))},document.addEventListener("visibilitychange",this._boundVisibilityChange),this._boundContextLost=o=>{o.preventDefault(),this.active=!1,x.animationId&&(cancelAnimationFrame(x.animationId),x.animationId=null)},e.addEventListener("webglcontextlost",this._boundContextLost),this._boundContextRestored=()=>{try{this.initialized=!1,this._freqDataArray=null,this.initEnvironment(),this.updateVisibility(),this.initialized=!0,this.active=!0,this.lastTime=performance.now()*.001,x.animationId&&cancelAnimationFrame(x.animationId),this._isRendering=!1,this.render(x.analyserLeft,x.analyserRight)}catch{}},e.addEventListener("webglcontextrestored",this._boundContextRestored),this._boundCymaticPointer=o=>{const r=o.touches?o.touches[0].clientX:o.clientX,n=o.touches?o.touches[0].clientY:o.clientY;this.mouseX=r/window.innerWidth*2-1,this.mouseY=-(n/window.innerHeight)*2+1},window.addEventListener("mousemove",this._boundCymaticPointer),window.addEventListener("touchmove",this._boundCymaticPointer,{passive:!0}),this.resize(),setTimeout(this.handleLayoutChange,100),this._boundThemeChange=o=>{if(o.detail&&o.detail.type){const r=o.detail.type;this.themeType!==r&&(this.themeType=r,this.updateLogoTexture())}},window.addEventListener("themeChanged",this._boundThemeChange),this.speedMultiplier=1,this.brightnessMultiplier=1,this.lastTime=performance.now()*.001,this.simTime=0,this.targetFPS=60,this.lastFrameRenderTime=0,this.active=!0,this.updateVisibility(),this.initialized=!0,this.initOverlayLogo(),window.viz3D=this}catch{this.initialized=!1}}resize(){if(!this.renderer||!this.canvas||!this.camera)return;const e=window.innerWidth,t=window.innerHeight;this.renderer.setSize(e,t),this.camera.aspect=e/t,this.camera.updateProjectionMatrix(),this.postProcessor&&this.postProcessor.resize(e,t),this.handleLayoutChange()}handleLayoutChange(){!this.renderer||!this.camera||this.camera.clearViewOffset()}resizeOverlayCanvas(){this.overlayCanvas&&(this.overlayCanvas.width=window.innerWidth,this.overlayCanvas.height=window.innerHeight)}generateCyberStyle(){const e=this.overlayCanvas,t=this.overlayCtx;if(!e||!t)return;this.matrixCyberStreams=[];const i=Math.max(11,Math.min(21,Math.floor(e.width/50)));this.isLowPower=localStorage.getItem("mindwave_battery_saver")==="true"||document.body.classList.contains("system-stability-mode");let a=Math.ceil(e.width/i);this.isLowPower||(a*=1.5);let s=window.getCurrentTheme?window.getCurrentTheme():{color:"#2dd4bf"};this.resizeOverlayCanvas(),this.cyberColorCustomized=!1;const o=[{name:"System Yellow",color:"#FFFF00",family:"yellow"},{name:"Neon Pink",color:"#FF1493",family:"pink"},{name:"Chrome Static",color:"#C0C0C0",family:"mono"},{name:"Royal Data",color:"#4169E1",family:"blue"},{name:"White Noise",color:"#FFFFFF",family:"mono"},{name:"Cyan Future",color:"#00FFFF",family:"blue"},{name:"Lime Access",color:"#32CD32",family:"green"},{name:"Purple Haze",color:"#800080",family:"purple"},{name:"Emerald Link",color:"#50C878",family:"green"},{name:"Ice Blue",color:"#A5F2F3",family:"blue"},{name:"sentAIent Blue",color:"#60A9FF",family:"blue"},{name:"Rainbow Surge",color:"rainbow",family:"rainbow"}],r=this.lastCyberFamily||"";let n=o.filter(c=>!this.themeHistory.includes(c.name));const h=n.filter(c=>c.family!==r);h.length>0&&(n=h);let f=n;if(f.length===0){const c=this.themeHistory[this.themeHistory.length-1];f=o.filter(u=>u.name!==c)}const l=f[Math.floor(Math.random()*f.length)];this.themeHistory.push(l.name),this.themeHistory.length>5&&this.themeHistory.shift(),this.lastCyberFamily=l.family;try{localStorage.setItem("cyberThemeHistory",JSON.stringify(this.themeHistory)),localStorage.setItem("cyberLastFamily",this.lastCyberFamily)}catch{}this.cyberSpeedMultiplier===void 0&&(this.cyberSpeedMultiplier=.2),this.cyberLengthMultiplier===void 0&&(this.cyberLengthMultiplier=.7);const g=document.getElementById("cyberRainbowToggle");if(g&&this.cyberRainbowMode===void 0&&(this.cyberRainbowMode=!0,this.cyberConfig.rainbow=!0,g.checked=!0),!this.cyberRainbowMode&&s.color!=="rainbow"){this.cyberColor=s.color;const c=document.getElementById("cyberColorPicker");c&&(c.value=this.cyberColor)}for(let c=0;c<a;c++){const u=Math.random(),v=Math.floor(8+u*11),G=(2+u*8+Math.random()*2)*this.cyberSpeedMultiplier;this.matrixCyberStreams.push({x:c*i,y:Math.random()*this.overlayCanvas.height*1.5-this.overlayCanvas.height*.5,baseSpeed:G,opacity:.2+u*.8,size:v,chars:[],color:s.color!=="rainbow"?s.color:"#FFFFFF"})}this.updateCyberStrings(),this.matrixCyberStreams.sort((c,u)=>c.size-u.size)}renderCyberCyber(){const e=this.activeModes.has("cyber");if(!this.overlayCtx||!this.matrixCyberStreams||!e)return;const t=this.overlayCtx,i=this.overlayCanvas;t.clearRect(0,0,i.width,i.height);const a=this.activeModes.size>1;t.fillStyle=a?"rgba(0, 0, 0, 0.4)":"rgba(0, 0, 0, 0.85)",t.fillRect(0,0,i.width,i.height),t.save(),t.textAlign="center";const s=this.cyberConfig,o=s.speed||1,r=s.length||1;s.angle!==0&&(t.translate(i.width/2,i.height/2),t.rotate(s.angle*Math.PI/180),t.translate(-i.width/2,-i.height/2));const n=20,h=Date.now()*.1;t.textBaseline="middle";let f=-1;this.matrixCyberStreams.forEach((l,g)=>{l.y+=l.baseSpeed*o;let c=Math.max(3,Math.floor(n*r));(this.isLowPower||this.currentLodLevel==="low")&&(c=Math.floor(c*.4)),l.y-c*l.size>i.height*1.5&&(l.y=0);const u="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";l.size!==f&&(t.font=`${l.size}px monospace`,f=l.size);const v=this.isLowPower&&this.currentLodLevel==="low"?2:1;for(let G=0;G<c&&G<l.chars.length;G+=v){!l.isTextMode&&Math.random()<.02&&(l.chars[G]=u.charAt(Math.floor(Math.random()*u.length)));const q=l.chars[G],A=l.y-G*l.size;if(A<-l.size*2||A>i.height*1.5)continue;const C=1-G/c,I=Math.pow(C,.4)*(l.opacity*1.2);if(t.globalAlpha=Math.min(1,I),this.cyberRainbowMode){const w=(h+g*15+G*5)%360;t.fillStyle=`hsl(${w}, 100%, 60%)`}else t.fillStyle=l.color||this.cyberColor;t.fillText(q,l.x,A)}}),t.shadowBlur=0,t.globalAlpha=1,t.restore()}initSphere(){var o;const e=new ne(2,2),t=new U({color:6334975,wireframe:!0,transparent:!0,opacity:.5});this.sphere=new P(e,t);const i=new ne(1.8,1),a=new U({color:6334975,transparent:!0,opacity:.1,blending:T});this.core=new P(i,a),this.sphereGroup.add(this.sphere),this.sphereGroup.add(this.core),this.sphereGroup.visible=!1;const s=((o=this.customColors)==null?void 0:o.sphere)||this.customColor;s&&(this.sphere.material.color.copy(s),this.core.material.color.copy(s))}initLightspeed(){const t=new O,i=new Float32Array(2e3*3);for(let a=0;a<2e3*3;a++)i[a]=(Math.random()-.5)*80;t.setAttribute("position",new D(i,3)),this.lightspeedMaterial=new k({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new _(3003583)},uTexture:{value:this.createCircleTexture()}},vertexShader:`
            ...
            `,transparent:!0,blending:T,depthWrite:!1}),this.lightspeed=new H(t,this.lightspeedMaterial),this.lightspeedGroup.add(this.lightspeed)}initParticles(){const e=this.batterySaver?400:1e3,t=new O,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*60,i[o+2]=(Math.random()-.5)*80;const r=Math.random();r<.3?(a[o]=.4,a[o+1]=.7,a[o+2]=1):r<.6?(a[o]=.3,a[o+1]=.9,a[o+2]=.95):r<.85?(a[o]=.6,a[o+1]=.4,a[o+2]=1):(a[o]=.9,a[o+1]=.9,a[o+2]=1)}t.setAttribute("position",new D(i,3)),t.setAttribute("color",new D(a,3)),this.particleMaterial=new k({uniforms:{uTime:{value:0},uSpeed:{value:1},uSize:{value:.4},uTexture:{value:this.createCircleTexture()}},vertexShader:`
                varying vec3 vColor;
                uniform float uTime, uSpeed;
                attribute vec3 color;
                void main() {
            ...
            `,transparent:!0,blending:T,depthWrite:!1}),this.particles=new H(t,this.particleMaterial),this.particleGroup.add(this.particles),this.particleGroup.visible=!1}initBox(){var f;this.boxOuter=new z;const e=new fe(new ue(3,3,3)),t=new ae({color:16777215,transparent:!0,opacity:.9,blending:T}),i=new ae({color:3900150,transparent:!0,opacity:.5,blending:T});this.boxOuter.add(new ie(e,t));for(let l=1;l<=3;l++){const g=new ie(e,i);g.scale.setScalar(1+l*.012),this.boxOuter.add(g)}const a=new fe(new ue(2,2,2)),s=new ae({color:14742270,transparent:!0,opacity:.8,blending:T}),o=new ae({color:6333946,transparent:!0,opacity:.4,blending:T});this.boxInner=new z,this.boxInner.add(new ie(a,s));for(let l=1;l<=2;l++){const g=new ie(a,o);g.scale.setScalar(1+l*.015),this.boxInner.add(g)}const r=new fe(new ue(3.05,3.05,3.05)),n=new ae({color:9684477,transparent:!0,opacity:.8,blending:T});this.boxEdges=new ie(r,n),this.boxGroup.add(this.boxOuter),this.boxGroup.add(this.boxInner),this.boxGroup.add(this.boxEdges),this.boxGroup.visible=!1;const h=((f=this.customColors)==null?void 0:f.box)||this.customColor;h&&(this.boxOuter.children.forEach(l=>l.material.color.copy(h)),this.boxInner.children.forEach(l=>l.material.color.copy(h)),this.boxEdges&&this.boxEdges.material&&this.boxEdges.material.color.copy(h))}initDragon(){var g;this.dragonDummy=new Xe,this.dragonLength=80;const e=new ne(.8,1),t=new U({color:15680580,wireframe:!0,transparent:!0,opacity:.8,blending:T}),i=new ne(.5,1),a=new U({color:16096779,wireframe:!1,transparent:!0,opacity:.6,blending:T});this.dragonBodyInstanced=new be(e,t,this.dragonLength),this.dragonGlowInstanced=new be(i,a,this.dragonLength);const s=new me(1.5,3.5,5);s.rotateX(Math.PI/2);const o=new U({color:16638023,wireframe:!0,transparent:!0,opacity:.9,blending:T});this.dragonHead=new P(s,o),this.dragonPearlGroup=new z;const r=new le(1,16,16),n=new U({color:3718648,wireframe:!1,transparent:!0,opacity:.9,blending:T}),h=new le(1.3,16,16),f=new U({color:8246268,wireframe:!0,transparent:!0,opacity:.5,blending:T});this.dragonPearl=new P(r,n),this.dragonPearlHalo=new P(h,f),this.dragonPearlGroup.add(this.dragonPearl),this.dragonPearlGroup.add(this.dragonPearlHalo),this.dragonGroup.add(this.dragonBodyInstanced),this.dragonGroup.add(this.dragonGlowInstanced),this.dragonGroup.add(this.dragonHead),this.dragonGroup.add(this.dragonPearlGroup),this.dragonGroup.visible=!1;const l=((g=this.customColors)==null?void 0:g.dragon)||this.customColor;l&&this.updateDragonColor(l)}updateGalaxyColor(e){if(this.galaxySunMesh&&this.galaxySunMesh.traverse(t=>{t.isMesh&&t.material&&t.material.color.copy(e)}),this.galaxyStars&&this.galaxyStars.geometry){const t={};e.getHSL(t);const i=(t.h+.5)%1,a=this.galaxyStars.geometry.getAttribute("color");if(a){const s=a.count,o=this._tempColor;for(let r=0;r<s;r++){const n=r/s,h=n<.2?.8:n<.5?.6:.4+Math.random()*.15,f=.6+Math.random()*.3;o.setHSL(i,f,h),a.setXYZ(r,o.r,o.g,o.b)}a.needsUpdate=!0}}}updateDragonColor(e){if(!this.dragonBodyInstanced)return;this.dragonBodyInstanced.material.color.copy(e),this.dragonHead.material.color.copy(e);const t={};e.getHSL(t);const i=(t.h+.5)%1,a=new _().setHSL(i,t.s,t.l);this.dragonGlowInstanced.material.color.copy(a)}initGalaxy(){const e=this.batterySaver?500:1500,t=new O,i=[],a=[],s=[];for(let n=0;n<e;n++){const h=n/e*Math.PI*10,f=2.5+n/e*20+Math.random()*2,l=n%4*(Math.PI*2/4),g=Math.max(.5,n/e*4),c=Math.cos(h+l)*f+(Math.random()-.5)*g,u=(Math.random()-.5)*1.5,v=Math.sin(h+l)*f+(Math.random()-.5)*g;i.push(c,u,v);const G=n/e;G<.2?a.push(1,.95,.7):G<.5?a.push(.7+Math.random()*.3,.8,1):a.push(.4+Math.random()*.2,.3+Math.random()*.3,.8+Math.random()*.2),s.push(Math.random()<.05?.4+Math.random()*.3:.08+Math.random()*.15)}t.setAttribute("position",new Y(i,3)),t.setAttribute("color",new Y(a,3)),t.setAttribute("size",new Y(s,1));const o=this.createStarTexture(),r=new he({size:.25,vertexColors:!0,map:o,transparent:!0,opacity:.9,blending:T,depthWrite:!1,sizeAttenuation:!0});this.galaxyStars=new H(t,r),this.galaxyGroup.add(this.galaxyStars),this.galaxySunStyle=this.galaxySunStyle||"sun",this.createGalaxySun(this.galaxySunStyle),this.galaxyGroup.visible=!1}createGalaxySun(e){this.galaxySunMesh&&(this.galaxyGroup.remove(this.galaxySunMesh),this.galaxySunMesh=null),this.galaxySunMesh=new z;const t=new U({color:4892415,transparent:!0,opacity:.85,blending:T,depthWrite:!1,side:$}),i=new z;if(e==="sun2"){const a=t.clone();a.side=Ye;const s=new we(1.5,.25,16,64);i.add(new P(s,a));const o=8,r=new me(.4,2.7,4);r.translate(0,2.7/2,0);const n=new me(.2,1.7,4);n.translate(0,1.7/2,0);for(let h=0;h<o;h++){const f=h/o*Math.PI*2,l=new P(r,a);l.rotation.z=-f,l.position.set(Math.sin(f)*1.5,Math.cos(f)*1.5,0),i.add(l);const g=f+Math.PI/o,c=new P(n,a);c.rotation.z=-g,c.position.set(Math.sin(g)*1.5,Math.cos(g)*1.5,0),i.add(c)}}else if(e==="sun3"){const a={uTime:{value:0},uColor:{value:t.color},uBassIntt:{value:0}};this.galaxySunUniforms=a;const s=`
                varying vec2 vUv;
            ...
            `,r=new k({vertexShader:s,fragmentShader:o,uniforms:a,transparent:!0,blending:T,depthWrite:!1,side:$}),n=new le(2,64,64),h=new P(n,r);i.add(h);const f=new U({color:t.color,transparent:!0,opacity:.15,blending:T,depthWrite:!1,side:je}),l=new le(3,32,32);i.add(new P(l,f))}else{const a=new we(1.5,.12,8,64);i.add(new P(a,t));const s=8;for(let o=0;o<s;o++){const r=o/s*Math.PI*2,n=new Ce;n.moveTo(-.4,0),n.lineTo(.4,0),n.lineTo(0,2.7),n.lineTo(-.4,0);const h=new Me(n),f=new P(h,t);f.rotation.z=-r,f.position.set(Math.sin(r)*1.5,Math.cos(r)*1.5,0),i.add(f);const l=r+Math.PI/s,g=new Ce;g.moveTo(-.2,0),g.lineTo(.2,0),g.lineTo(0,1.7),g.lineTo(-.2,0);const c=new Me(g),u=new P(c,t);u.rotation.z=-l,u.position.set(Math.sin(l)*1.5,Math.cos(l)*1.5,0),i.add(u)}}this.galaxySunMesh.add(i),this.galaxySunMesh.position.set(0,0,.1),this.galaxyGroup.add(this.galaxySunMesh)}setGalaxySunStyle(e){e!=="sun"&&e!=="sun2"&&e!=="sun3"||(this.galaxySunStyle=e,this.galaxyGroup&&this.galaxyGroup.children.length>0&&this.createGalaxySun(e))}setGalaxySunRotation(e,t){e==="x"&&(this.sunRotationSpeedX=parseFloat(t)),e==="y"&&(this.sunRotationSpeedY=parseFloat(t)),e==="z"&&(this.sunRotationSpeedZ=parseFloat(t))}createStarTexture(){if(this.textures.star)return this.textures.star;const e=64,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e/2);return o.addColorStop(0,"rgba(255, 255, 255, 1.0)"),o.addColorStop(.1,"rgba(255, 255, 240, 0.8)"),o.addColorStop(.25,"rgba(255, 255, 200, 0.3)"),o.addColorStop(.5,"rgba(200, 200, 255, 0.1)"),o.addColorStop(1,"rgba(100, 100, 255, 0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.fillStyle="rgba(255, 255, 255, 0.6)",i.beginPath(),i.moveTo(0,s-1),i.lineTo(a,s-.5),i.lineTo(e,s-1),i.lineTo(e,s+1),i.lineTo(a,s+.5),i.lineTo(0,s+1),i.closePath(),i.fill(),i.beginPath(),i.moveTo(a-1,0),i.lineTo(a-.5,s),i.lineTo(a-1,e),i.lineTo(a+1,e),i.lineTo(a+.5,s),i.lineTo(a+1,0),i.closePath(),i.fill(),i.beginPath(),i.arc(a,s,2,0,Math.PI*2),i.fillStyle="rgba(255, 255, 255, 1.0)",i.fill(),this.textures.star=new ee(t),this.textures.star}initMandala(){var s;this.mandalaRings=[];const e=[2450411,15357964,3900150,16347926,12573694];for(let o=0;o<5;o++){const r=1.2+o*.8,n=6+o*6,h=new Qe(r-.05,r+.05,n),f=new U({color:e[o],side:$,transparent:!0,opacity:.4-o*.05,blending:T}),l=new P(h,f);l.userData={speed:(.01+o*.005)*(o%2===0?1:-1),segments:n},this.mandalaRings.push(l),this.mandalaGroup.add(l)}const t=new Ze(.3,32),i=new U({color:16347926,transparent:!0,opacity:.6,blending:T});this.mandalaCenter=new P(t,i),this.mandalaGroup.add(this.mandalaCenter),this.mandalaGroup.visible=!1;const a=(s=this.customColors)==null?void 0:s.mandala;a&&(this.mandalaRings.forEach(o=>o.material.color.copy(a)),this.mandalaCenter.material.color.copy(a))}createSnowflakeTexture(){if(this.textures.snowflake)return this.textures.snowflake;const e=128,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e),i.strokeStyle="rgba(220,240,255,1.0)",i.lineCap="round";for(let o=0;o<6;o++){const r=o/6*Math.PI*2;i.save(),i.translate(a,s),i.rotate(r),i.lineWidth=2.5,i.beginPath(),i.moveTo(0,0),i.lineTo(0,-52),i.stroke();const n=[{d:18,len:14,angle:Math.PI/4},{d:32,len:20,angle:Math.PI/4},{d:46,len:10,angle:Math.PI/5}];i.lineWidth=1.5,n.forEach(({d:h,len:f,angle:l})=>{[1,-1].forEach(g=>{i.beginPath(),i.moveTo(0,-h),i.lineTo(g*f*Math.cos(Math.PI/2-l),-h-f*Math.sin(Math.PI/2-l)),i.stroke()})}),i.restore()}i.beginPath();for(let o=0;o<6;o++){const r=o/6*Math.PI*2-Math.PI/6,n=a+Math.cos(r)*4,h=s+Math.sin(r)*4;o===0?i.moveTo(n,h):i.lineTo(n,h)}return i.closePath(),i.fillStyle="rgba(255, 255, 255, 0.8)",i.fill(),this.textures.snowflake=new ee(t),this.textures.snowflake}initSnowflake(){for(;this.snowflakeGroup.children.length>0;){const c=this.snowflakeGroup.children[0];this.snowflakeGroup.remove(c),c.geometry&&c.geometry.dispose(),c.material&&c.material.dispose()}this._snowData=null;const e=700,t=new Float32Array(e*3),i=new Float32Array(e),a=new Float32Array(e),s=new Float32Array(e),o=new Float32Array(e),r=new Float32Array(e),n=new Float32Array(e);for(let c=0;c<e;c++){const u=c*3;t[u]=(Math.random()-.5)*80,t[u+1]=(Math.random()-.5)*60,t[u+2]=-40+Math.random()*35;const v=(t[u+2]+40)/35;i[c]=1.5+v*8,a[c]=.2+v*.6,s[c]=Math.random()*Math.PI*2,o[c]=.015+Math.random()*.04+v*.03,r[c]=.01+Math.random()*.02,n[c]=.4+Math.random()*.8}const h=new O;h.setAttribute("position",new D(t,3)),h.setAttribute("aSize",new D(i,1)),h.setAttribute("aOpacity",new D(a,1));const f=this.createSnowflakeTexture(),l=new k({uniforms:{uTexture:{value:f},uColor:{value:this.customColor?this.customColor.clone():new _(10875883)},uIntensity:{value:0},uSizeMultiplier:{value:1},uGlowAmount:{value:.5}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),g=new H(h,l);this.snowflakeGroup.add(g),this._snowData={count:e,positions:t,phases:s,speeds:o,drifts:r,driftFreqs:n,points:g,material:l,spinMeshes:[],spinSpeeds:[]}}setSnowSize(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uSizeMultiplier.value=Math.max(.2,Math.min(4,e)))}setSnowGlow(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uGlowAmount.value=Math.max(0,Math.min(1,e)))}initLava(){var o;this.lavaBlobs=[];const e=16;this.lavaUniforms={uBlobs:{value:[]},uColor:{value:new _(((o=x.visualColors)==null?void 0:o.lava)||16737792)},uSecondaryColor:{value:new _(16755200)},uTime:{value:0},uIntensity:{value:0},uResolution:{value:new ce(window.innerWidth,window.innerHeight)}};for(let r=0;r<e;r++)this.lavaUniforms.uBlobs.value.push(new $e(0,-100,0,0));const t=new k({uniforms:this.lavaUniforms,vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
            ...
            `,transparent:!0,blending:T,depthWrite:!1,side:$}),i=new oe(100,100),a=new P(i,t);a.position.z=-5,[{count:5,minSize:1,maxSize:1.5},{count:7,minSize:1.5,maxSize:2.2},{count:4,minSize:2.2,maxSize:3.5}].forEach(r=>{for(let n=0;n<r.count;n++){const h=r.minSize+Math.random()*(r.maxSize-r.minSize),f=["heating","rising","cooling","falling"],l=f[Math.floor(Math.random()*f.length)],g=-18+Math.random()*4,c=18+Math.random()*4;let u=0,v=.5;l==="heating"?(u=g,v=Math.random()*.5):l==="rising"?(u=g+Math.random()*(c-g),v=.8+Math.random()*.2):l==="cooling"?(u=c,v=1-Math.random()*.3):l==="falling"&&(u=c-Math.random()*(c-g),v=.2+Math.random()*.3),this.lavaBlobs.push({position:new Ke((Math.random()-.5)*12,u,(Math.random()-.5)*6),material:{uniforms:{uTemp:{value:0},uIntensity:{value:0},uTime:{value:0}}},userData:{baseSize:h,state:l,temperature:v,floatMin:g,floatMax:c,heatRate:.15+Math.random()*.3,coolRate:.15+Math.random()*.3,riseSpeed:(.04+Math.random()*.05)/(h*.5),fallSpeed:(.05+Math.random()*.05)/(h*.5),driftPhase:Math.random()*Math.PI*2,driftSpeed:.01+Math.random()*.02}})}}),this.lavaGroup.add(a),this.lavaGroup.visible=!1}initFireplace(){const i=new oe(100,80);this.fireMaterial=this.createFireShader(),this.fireMesh=new P(i,this.fireMaterial),this.fireMesh.position.set(0,0,-15),this.fireMesh.renderOrder=10,this.fireplaceGroup.add(this.fireMesh),this.fireLight=new Je(16737792,5,200),this.fireLight.position.set(0,0,30),this.fireplaceGroup.add(this.fireLight);const a=650,s=new O,o=[];for(let r=0;r<a;r++)o.push((Math.random()-.5)*100,-20+Math.random()*40,-15+(Math.random()-.5)*20);s.setAttribute("position",new Y(o,3)),this.emberMat=new he({color:16763904,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.8,blending:T,depthWrite:!1}),this.embers=new H(s,this.emberMat),this.fireplaceGroup.add(this.embers),this.emberVelocities=new Float32Array(a);for(let r=0;r<a;r++)this.emberVelocities[r]=.02+Math.random()*.05;this.fireplaceGroup.position.set(0,0,0),this.fireplaceGroup.visible=!1}createFireShader(){return new k({uniforms:{uTime:{value:0},uColor:{value:new _(16729088)},uSpeed:{value:1},uIntensity:{value:1}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            ...
            `,transparent:!0,side:$,blending:T,depthWrite:!1})}initRainforest(){const e=this.batterySaver?400:1e3,t=new O,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*40,i[o+2]=(Math.random()-.5)*40,a[o]=Math.random(),a[o+1]=Math.random(),a[o+2]=.08+Math.random()*.12}t.setAttribute("position",new D(i,3)),t.setAttribute("aRandom",new D(a,3)),this.rainMaterial=new k({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new _(8965375)},uIntensity:{value:.6}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),this.raindrops=new H(t,this.rainMaterial),this.rainforestGroup.add(this.raindrops),this.rainforestGroup.visible=!1}initZenGarden(){const e=this.batterySaver?200:500,t=new O,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let o=0;o<e;o++){const r=o*3;i[r]=(Math.random()-.5)*40,i[r+1]=(Math.random()-.5)*20,i[r+2]=(Math.random()-.5)*40,a[r]=Math.random(),a[r+1]=Math.random(),a[r+2]=Math.random()*Math.PI*2}t.setAttribute("position",new D(i,3)),t.setAttribute("aRandom",new D(a,3)),this.petalMaterial=new k({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new _(16758725)},uIntensity:{value:.8}},vertexShader:`
                attribute vec3 aRandom;
                uniform float uTime, uSpeed;
                varying float vAlpha;
                void main() {
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new H(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new oe(40,40,32,32);this.zenWaterMaterial=new U({color:2245734,transparent:!0,opacity:.3,side:$}),this.zenWater=new P(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new oe(300,100,128,64);this.oceanWave=new P(e,new k({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new _(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
            ...
            `,wireframe:!0,transparent:!0,side:$})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new O,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new Y(a,3));const s=new he({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new H(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let h="🪷MINDWAVE";const f=e.logicMode,l=e.customText;f==="custom"||f==="txt"?h="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(f==="random"||f==="rnd"||f==="matrix"||f==="int"||f==="interstellar")&&(h="");const g=f==="matrix"||f==="int"||f==="interstellar"?[]:["LOGO",...h],c=g.length;for(let v=0;v<64;v++){const G=v%8,q=Math.floor(v/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(G*r,q*n,r,n),a.save(),a.translate(G*r+r/2,q*n+n/2);let A="",C=!1;if(v<c){const I=g[v];I==="LOGO"?C=!0:A=I}else{const w="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";A=w.charAt(Math.floor(Math.random()*w.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(A,0,0),a.restore(),A="")}if(A||C)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",C)if(this.logoImage){const B=document.createElement("canvas");B.width=100,B.height=100;const R=B.getContext("2d");R.imageSmoothingEnabled=!0,R.imageSmoothingQuality="high",R.drawImage(this.logoImage,0,0,100,100);const N=R.getImageData(0,0,100,100),F=N.data;for(let L=0;L<F.length;L+=4){const K=180+(F[L]+F[L+1]+F[L+2])/3/255*75;F[L]=K,F[L+1]=K,F[L+2]=K}R.putImageData(N,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(B,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new et().load("./mindwave-logo-icon.png",w=>{if(this.logoImage=w,this.logoLoading=!1,this.cyberMaterial){const B=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=B}},void 0,w=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(A,0,0);a.restore()}const u=new ee(i);return u.magFilter=Se,u.minFilter=Se,u}createCyberShader(e,t=this.matrixConfig){return new k({uniforms:{uTexture:{value:e},uColor:{value:new _(t.color||65345)},uHeadColor:{value:new _(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
            ...
            `,transparent:!0,wireframe:!1,side:$,extensions:{derivatives:!0}}),this.wavesMesh=new P(e,this.wavesMaterial),this.wavesMesh.rotation.x=-Math.PI/2.2,this.wavesMesh.position.y=-5,this.wavesMesh.position.z=-10,this.wavesGroup.add(this.wavesMesh)}setSpeed(e){this.speedMultiplier=e}setIntent(e){this.currentIntent=e,e==="delta"||e==="theta"?this.particles&&this.particles.material&&(this.particles.material.size=.3,this.particles.material.opacity=.5):e==="gamma"||e==="hyper-gamma"?(this.particles&&this.particles.material&&(this.particles.material.size=.08,this.particles.material.opacity=.9),this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uTailLength&&(this.cyberMaterial.uniforms.uHeadColor.value.setHex(16777215),this.cyberMaterial.uniforms.uTailLength.value=.4)):(this.particles&&this.particles.material&&(this.particles.material.size=.15,this.particles.material.opacity=.8),this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uTailLength&&(this.cyberMaterial.uniforms.uHeadColor.value.setHex(15794160),this.cyberMaterial.uniforms.uTailLength.value=1)),this.renderSingleFrame()}setColor(e,t=null){var i,a,s,o;this.customColors||(this.customColors={}),!t||t==="all"?(this.customColors={},this.customColor=new _(e)):this.customColors[t]=new _(e);try{const r=n=>{n&&(n.color&&typeof n.color.set=="function"?n.color.set(e):n.uniforms&&n.uniforms.uColor&&n.uniforms.uColor.value.set(e))};this.particles&&this.particles.material&&r(this.particles.material),this.lightspeed&&this.lightspeed.material&&r(this.lightspeed.material),this.sphere&&this.sphere.material&&(r(this.sphere.material),this.core&&this.core.material&&r(this.core.material)),(t==="box"||!t||t==="all")&&(this.boxOuter&&this.boxOuter.children.forEach(n=>r(n.material)),this.boxInner&&this.boxInner.children.forEach(n=>r(n.material)),this.boxEdges&&this.boxEdges.material&&r(this.boxEdges.material)),t==="mandala"&&(this.mandalaRings&&this.mandalaRings.forEach(n=>r(n.material)),this.mandalaCenter&&this.mandalaCenter.material&&r(this.mandalaCenter.material)),this.wavesMesh&&this.wavesMesh.material&&r(this.wavesMesh.material),this.lavaBlobs&&this.lavaBlobs.forEach(n=>r(n.material)),this.lavaGlow&&this.lavaGlow.material&&r(this.lavaGlow.material),this.flames&&this.flames.material&&r(this.flames.material),this.raindrops&&this.raindrops.material&&r(this.raindrops.material),this.petals&&this.petals.material&&r(this.petals.material),this.zenWater&&this.zenWater.material&&r(this.zenWater.material),this.oceanWave&&this.oceanWave.material&&r(this.oceanWave.material),this.oceanFoam&&this.oceanFoam.material&&r(this.oceanFoam.material),this.cyberRain&&this.cyberRain.material&&this.cyberRain.material.uniforms&&this.cyberRain.material.uniforms.uColor&&this.cyberRain.material.uniforms.uColor.value.set(e),this.logoMesh&&this.originalLogoImg&&this.updateLogoTexture(),this.dragonGroup&&this.updateDragonColor&&this.updateDragonColor(new _(e)),(this.galaxyStars||this.galaxySunMesh)&&this.updateGalaxyColor(new _(e)),this.cymaticMaterial&&((i=this.cymaticMaterial.uniforms)!=null&&i.uColor)&&this.cymaticMaterial.uniforms.uColor.value.set(e),(t==="snowflake"||!t||t==="all")&&typeof this.setSnowColor=="function"&&this.setSnowColor(e),(o=(s=(a=this._snowData)==null?void 0:a.material)==null?void 0:s.uniforms)!=null&&o.uColor&&this._snowData.material.uniforms.uColor.value.set(e)}catch{}this.renderSingleFrame()}renderSingleFrame(){!this.renderer||!this.scene||!this.camera||(this.renderer.clear(),this.renderer.render(this.scene,this.camera))}createCircleTexture(){if(this.textures.circle)return this.textures.circle;const e=32,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d");return i.beginPath(),i.arc(e/2,e/2,e/2,0,Math.PI*2),i.fillStyle="#ffffff",i.fill(),this.textures.circle=new ee(t),this.textures.circle}render(e,t){var i,a,s,o,r,n,h,f;if(!this.initialized||!this.renderer||document.hidden||!this.active){this._isRendering=!1;return}if(!(this._isRendering&&typeof e=="number")){this._isRendering=!0;try{(typeof e=="number"||!e&&x.analyserLeft)&&(e=x.analyserLeft,t=x.analyserRight);let l=0,g=0,c=0;if(e){const d=e.frequencyBinCount;(!this._freqDataArray||this._freqDataArray.length!==d)&&(this._freqDataArray=new Uint8Array(d)),e.getByteFrequencyData(this._freqDataArray);let y=0;for(let S=0;S<15;S++)y+=this._freqDataArray[S];l=Math.pow(y/15/255,.8);let M=0;for(let S=10;S<100;S++)M+=this._freqDataArray[S];g=M/90/255;let b=0;for(let S=100;S<300;S++)b+=this._freqDataArray[S];c=b/200/255}const u=Math.max(.001,this.speedMultiplier||1),v=performance.now()*.001;if(this.activeModes.has("cymatics")&&this.cymaticsCore){const d=window.cymaticsAudioSync!==!1?{bass:l||0,mids:g||0,highs:c||0}:{bass:0,mids:0,highs:0},y=window.cymaticsMouseSync!==!1?{x:this.mouseX||0,y:this.mouseY||0}:null;this.cymaticsCore.update(v*u,d,y)}this.lastTime||(this.lastTime=v);const G=Math.min(.1,v-this.lastTime);this.lastTime=v;const q=x.visualSpeedAuto?x.beatFrequency||10:u*10,A=Math.sin(v*Math.PI*2*q)*.5+.5,C=this.vibrationEnabled?1:0,I=(A||0)*C,w=(l||0)*C,B=(g||0)*C,R=(c||0)*C,N=C*(.02+w*.15+I*.08),F=Math.sin(v*47.3)*Math.cos(v*31.7)*N,L=Math.cos(v*53.1)*Math.sin(v*29.3)*N,J=Math.sin(v*37.9)*Math.cos(v*43.1)*N,K=[this.sphereGroup,this.particleGroup,this.lightspeedGroup,this.lavaGroup,this.fireplaceGroup,this.rainforestGroup,this.zenGardenGroup,this.oceanGroup,this.wavesGroup,this.cyberGroup,this.boxGroup,this.dragonGroup,this.galaxyGroup,this.mandalaGroup,this.cymaticsGroup,this.snowflakeGroup];for(const d of K)d&&d.position.set(F,L,J);if(this.activeModes.has("galaxy")&&this.galaxyGroup){const d=this.sunRotationSpeedY||.002,y=this.sunRotationSpeedZ||5e-4;if(this.galaxyStars&&(this.galaxyStars.rotation.y+=d*u*.5,this.galaxyStars.rotation.z+=y*u*.5),this.galaxySunMesh){this.galaxySunMesh.rotation.z-=y*u*1.5,this.galaxySunMesh.rotation.y+=d*u*2;const M=1+w*.15*(this.galaxySunStyle==="sun3"?2.5:1);this.galaxySunMesh.scale.setScalar(M)}this.galaxySunUniforms&&(this.galaxySunUniforms.uTime.value=v*u,this.galaxySunUniforms.uBassIntt.value=w)}if(this.activeModes.has("sphere")&&this.sphere&&(this.sphere.scale.setScalar(1+w*.15),this.sphere.rotation.y+=.005*u),this.activeModes.has("particles")&&this.particleMaterial){const d=((a=(i=window.MindWaveState)==null?void 0:i.envIntensities)==null?void 0:a.flow)??1,y=(.015*u+w*.08+I*.05)*d;this.particleMaterial.uniforms.uTime.value=v,this.particleMaterial.uniforms.uSpeed.value=y*10,this.particleGroup.rotation.z+=(.001*u+w*.005)*d}if(this.activeModes.has("lightspeed")&&this.lightspeedMaterial&&(this.lightspeedMaterial.uniforms.uTime.value=v,this.lightspeedMaterial.uniforms.uSpeed.value=u),this.activeModes.has("matrix")&&this.cyberMaterial&&(this.cyberMaterial.uniforms.uTime.value=v*u),this.activeModes.has("waves")&&this.wavesMaterial){const d=((o=(s=window.MindWaveState)==null?void 0:s.envIntensities)==null?void 0:o.ocean)??1;this.wavesMaterial.uniforms.uTime.value=v*u*.5,this.wavesMaterial.uniforms.uNormBass.value=w*d,this.wavesMaterial.uniforms.uIntensity&&(this.wavesMaterial.uniforms.uIntensity.value=d)}if(this.activeModes.has("ocean")&&this.oceanWave&&this.oceanWave.material.uniforms){const d=((n=(r=window.MindWaveState)==null?void 0:r.envIntensities)==null?void 0:n.ocean)??1;this.oceanWave.material.uniforms.uTime.value=v*u,this.oceanWave.material.uniforms.uNormBass.value=w*d,this.oceanWave.material.uniforms.uBeatPulse.value=I*d,this.oceanFoam&&(this.oceanFoam.material.opacity=(.4+w*.3+I*.2)*(this.brightnessMultiplier||1)*d)}if(this.activeModes.has("box")&&this.boxOuter&&(this.boxOuter.rotation.x+=.008*u+w*.02,this.boxOuter.rotation.y+=.012*u,this.boxInner&&(this.boxInner.rotation.x-=.015*u,this.boxInner.rotation.y-=.01*u,this.boxInner.scale.setScalar(.95+w*.2)),this.boxOuter.scale.setScalar(1+w*.2),this.boxEdges&&this.boxEdges.rotation.copy(this.boxOuter.rotation)),this.activeModes.has("dragon")&&this.dragonBodyInstanced&&this.dragonDummy){this.dragonGroup.rotation.y+=.005*u;const d=v*u*2,y=1+w*.2;for(let M=0;M<this.dragonLength;M++){const b=d-M*.12,S=Math.sin(b)*8,Z=Math.cos(b*1.5)*4+Math.sin(b*.5)*3,qe=Math.cos(b*.8)*8;this.dragonDummy.position.set(S,Z,qe);const se=b+.1,De=Math.sin(se)*8,Fe=Math.cos(se*1.5)*4+Math.sin(se*.5)*3,Le=Math.cos(se*.8)*8;this.dragonDummy.lookAt(De,Fe,Le);const Re=1-M/this.dragonLength*.8,Ve=1+Math.sin(b*4)*.15*(.5+w);this.dragonDummy.scale.setScalar(Re*Ve*y),this.dragonDummy.updateMatrix(),this.dragonBodyInstanced.setMatrixAt(M,this.dragonDummy.matrix),this.dragonGlowInstanced&&this.dragonGlowInstanced.setMatrixAt(M,this.dragonDummy.matrix),M===0&&this.dragonHead&&(this.dragonHead.position.copy(this.dragonDummy.position),this.dragonHead.quaternion.copy(this.dragonDummy.quaternion),this.dragonHead.scale.copy(this.dragonDummy.scale).multiplyScalar(1.4))}if(this.dragonBodyInstanced.instanceMatrix.needsUpdate=!0,this.dragonGlowInstanced&&(this.dragonGlowInstanced.instanceMatrix.needsUpdate=!0),this.dragonPearlGroup){const M=d+.5;this.dragonPearlGroup.position.set(Math.sin(M)*9,Math.cos(M*1.5)*5+Math.sin(M*.5)*4,Math.cos(M*.8)*9),this.dragonPearlGroup.rotation.x+=.08*u,this.dragonPearlGroup.rotation.y+=.12*u}}if(this.activeModes.has("mandala")&&this.mandalaRings&&(this.mandalaRings.forEach((d,y)=>{d.userData&&d.userData.speed&&(d.rotation.z+=d.userData.speed*u+w*.005);const M=1+I*.1*(y+1)*.3;d.scale.setScalar(M)}),this.mandalaCenter&&(this.mandalaCenter.rotation.z-=.01*u,this.mandalaCenter.scale.setScalar(1+w*.3))),this.dimmerPlane){const d=x&&x.screenDimmer!==void 0?x.screenDimmer:0;this.dimmerPlane.material.opacity=d}if(this.logoMesh){const d=x.lotusState||"auto";let y=A;if(d==="heartbeat")if(x.lotusHeartbeatSync)y=Math.min(1,A*.7+w*.8);else{const b=(x.lotusHeartbeatBpm||11)/60;y=Math.sin(v*Math.PI*2*b)*.5+.5}let M=.8;d==="faded"?M=.15:d==="full"?M=1:d==="heartbeat"?M=.2+.8*y:d==="manual"?M=x.lotusOpacity!==void 0?x.lotusOpacity:.8:M=.4+w*.6,this._lotusCurrentOpacity===void 0&&(this._lotusCurrentOpacity=.8),this._lotusCurrentOpacity+=(M-this._lotusCurrentOpacity)*.1,this.logoMesh.material.opacity=this._lotusCurrentOpacity,d==="heartbeat"?this.logoMesh.scale.setScalar(1+y*.15):this.logoMesh.scale.setScalar(1+w*.05+I*.02)}if(this.activeModes.has("lava")&&this.lavaBlobs&&this.lavaUniforms){const d=((f=(h=window.MindWaveState)==null?void 0:h.envIntensities)==null?void 0:f.lava)??1;this.lavaUniforms.uTime.value=v*u,this.lavaUniforms.uIntensity.value=w*d,this.lavaBlobs.forEach((y,M)=>{const b=y.userData,S=u*(1+w*.8);if(b.state==="heating"?(b.temperature+=b.heatRate*G*S,b.temperature>=1&&(b.temperature=1,b.state="rising")):b.state==="rising"?(y.position.y+=b.riseSpeed*S,y.position.y>=b.floatMax&&(b.state="cooling")):b.state==="cooling"?(b.temperature-=b.coolRate*G*S,b.temperature<=0&&(b.temperature=0,b.state="falling")):b.state==="falling"&&(y.position.y-=b.fallSpeed*S,y.position.y<=b.floatMin&&(b.state="heating")),y.position.x+=Math.sin(v*b.driftSpeed+b.driftPhase)*.02*S,this.lavaUniforms.uBlobs.value[M]){const Z=b.baseSize*(.8+.5*b.temperature);this.lavaUniforms.uBlobs.value[M].set(y.position.x,y.position.y,y.position.z,Z)}})}if(this.snowflakeGroup&&this.snowflakeGroup.visible&&this._snowData){const d=this._snowData,y=d.points.geometry.attributes.position.array,M=u*2;for(let b=0;b<d.count;b++){const S=b*3;y[S+1]-=d.speeds[b]*M;let Z=Math.sin(v*d.driftFreqs[b]+d.phases[b])*d.drifts[b]*M;y[S]+=Z,y[S+1]<-22&&(y[S+1]=22),y[S]>35&&(y[S]=-35),y[S]<-35&&(y[S]=35)}if(d.points.geometry.attributes.position.needsUpdate=!0,d.spinMeshes){const b=1+(w||0)*.15;d.spinMeshes.forEach((S,Z)=>{S.rotation.z+=d.spinSpeeds[Z]*M,S.position.y-=d.spinSpeeds[Z]*.1*M,S.scale.setScalar(b),S.position.y<-25&&(S.position.y=25)})}d.material&&d.material.uniforms&&d.material.uniforms.uIntensity&&(d.material.uniforms.uIntensity.value=.2+w*.8)}if(this.activeModes.has("rainforest")&&this.rainMaterial){const d=u*.8*(1+I*.3);this.rainMaterial.uniforms.uTime.value=v,this.rainMaterial.uniforms.uSpeed.value=d,this.rainMaterial.uniforms.uIntensity.value=(.5+w*.2+I*.2)*(this.brightnessMultiplier||1)}if(this.activeModes.has("zengarden")&&this.petalMaterial){const d=u*.3*(1+I*.5);this.petalMaterial.uniforms.uTime.value=v,this.petalMaterial.uniforms.uSpeed.value=d,this.zenWater&&(this.zenWater.material.opacity=(.3+I*.2)*(this.brightnessMultiplier||1))}if(this.activeModes.has("fireplace")&&this.fireMesh&&this.fireMesh.material&&this.fireMesh.material.uniforms){const d=.8+.2*Math.sin(v*15)*Math.sin(v*7),y=this.fireMesh.material.uniforms;y.uTime&&(y.uTime.value=v*1.5*u),y.uIntensity&&(y.uIntensity.value=d+w*.5),this.fireLight&&(this.fireLight.intensity=(2+w*5)*d)}const Q=performance.now(),Pe=Q-(this._lastFrameStartTime||Q);if(this._lastFrameStartTime=Q,this._fpsRingBuffer&&(this._fpsRingBuffer[this._fpsRingIndex]=Pe,this._fpsRingIndex=(this._fpsRingIndex+1)%60,this._fpsRingCount=Math.min(60,this._fpsRingCount+1),this._fpsRingCount===60)){let d=0;for(let M=0;M<60;M++)d+=this._fpsRingBuffer[M];d/=60,1e3/d<35&&Q-(this._lastLodDegradation||0)>3e3&&(this.degradeLOD(),this._lastLodDegradation=Q,this._fpsRingCount=0)}const Ge=1e3/(this.targetFPS||60);if(Q-this.lastFrameRenderTime>=Ge){this.renderer.autoClear=!this.activeModes.has("lightspeed"),this.renderer.autoClear&&this.renderer.clear();const d=(w+(g||0)*C+R)/3,y=Q-this.lastFrameRenderTime;this.particleSwarm&&this.activeModes.has("cymatics")&&this.particleSwarm.update(Math.min(y*.001,.1),d),this.postProcessor&&this.currentLodLevel!=="low"?this.postProcessor.render(this.scene,this.camera,Math.min(y*.001,.1),d):this.renderer.render(this.scene,this.camera),this.activeModes.has("cyber")&&this.renderCyberCyber(),this.lastFrameRenderTime=Q}}catch(l){l.name==="TypeError"&&l.message.includes("uniforms")}finally{this._isRendering=!1}this.active&&!document.hidden?x.animationId=requestAnimationFrame(()=>this.render(x.analyserLeft,x.analyserRight)):x.animationId=null}}setCyberColor(e){this.cyberConfig.color=e,this.cyberConfig.rainbow=!1,this.cyberColorCustomized=!0,this.matrixCyberStreams&&this.matrixCyberStreams.forEach(t=>{t.color=e})}setMatrixColor(e){this.matrixConfig.color=e,this.matrixConfig.rainbow=!1,this.cyberMaterial&&this.cyberMaterial.uniforms.uColor&&(this.cyberMaterial.uniforms.uColor.value.set(e),this._tempColor.set(e),this._tempColor.r+=(1-this._tempColor.r)*.8,this._tempColor.g+=(1-this._tempColor.g)*.8,this._tempColor.b+=(1-this._tempColor.b)*.8,this.cyberMaterial.uniforms.uHeadColor.value.copy(this._tempColor),this.cyberMaterial.uniforms.uRainbow&&(this.cyberMaterial.uniforms.uRainbow.value=0))}getAverageFrequency(e,t){let i=x.analyserLeft||x.analyserRight;if(!i)return 0;const a=i.frequencyBinCount;(!this._freqDataArray||this._freqDataArray.length!==a)&&(this._freqDataArray=new Uint8Array(a));const s=this._freqDataArray;i.getByteFrequencyData(s),e===void 0&&(e=0),t===void 0&&(t=s.length);let o=0,r=0;for(let n=e;n<t&&n<s.length;n++)o+=s[n],r++;return r>0?o/r:0}setMouseInfluence(e){this.mouseInfluence=e}setGlobalSpeed(e){this.speedMultiplier=e,this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uSpeed&&(this.cyberMaterial.uniforms.uSpeed.value=1+(e-1)*.5)}setGlobalBrightness(e){this.brightnessMultiplier=e,this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uBrightness&&(this.cyberMaterial.uniforms.uBrightness.value=e)}setCyberBrightness(e){this.cyberConfig.brightness=e}setMatrixBrightness(e){this.matrixConfig.brightness=e,this.cyberMaterial&&this.cyberMaterial.uniforms.uBrightness&&(this.cyberMaterial.uniforms.uBrightness.value=e)}setCyberSpeed(e){this.cyberConfig.speed=e}setCyberLength(e){this.cyberConfig.length=e}setMatrixSpeed(e){this.matrixConfig.speed=e,this.cyberMaterial&&this.cyberMaterial.uniforms.uSpeed&&(this.cyberMaterial.uniforms.uSpeed.value=e)}setMatrixLength(e){this.matrixConfig.length=e,this.cyberMaterial&&this.cyberMaterial.uniforms.uTailLength&&(this.cyberMaterial.uniforms.uTailLength.value=e)}setCyberRainbow(e){if(this.cyberConfig.rainbow=e,this.cyberRainbowMode=e,!e){const t=this.cyberConfig.color||this.cyberColor||"#00FF41";this.matrixCyberStreams&&this.matrixCyberStreams.forEach(i=>{i.color=t})}}setMatrixRainbow(e){this.matrixConfig.rainbow=e,this.cyberMaterial&&this.cyberMaterial.uniforms.uRainbow&&(this.cyberMaterial.uniforms.uRainbow.value=e?1:0)}setBatterySaver(e){this.batterySaver=e,this.targetFPS=e?30:60;const t=e?1:2;this.renderer&&(this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,t)),this.resize())}degradeLOD(){this.renderer&&(this.currentLodLevel==="high"?(this.renderer.setPixelRatio(1),this.currentLodLevel="medium",this.resize()):this.currentLodLevel==="medium"&&(this.renderer.setPixelRatio(.75),this.currentLodLevel="low",this.resize()))}updateCyberStrings(){if(!this.matrixCyberStreams)return;let e=!1,t="MINDWAVE🪷";const i=this.cyberConfig;i.logicMode==="custom"||i.logicMode==="txt"?(e=!0,t="🪷"+(i.customText||"WELCOME TO MINDWAVE")):(i.logicMode==="mindwave"||i.logicMode==="mw")&&(e=!0,t="MINDWAVE🪷");const a="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ",s="0123456789·:.-+x[]<>/\\∆ΣΩ∞",o=100;this.matrixCyberStreams.forEach(r=>{if(r.chars=[],r.isTextMode=e,e&&t.length>0){const n=[...t].reverse();for(let h=0;h<o;h++)r.chars.push(n[h%n.length])}else if(i.logicMode==="matrix"||i.logicMode==="interstellar"||i.logicMode==="int")for(let n=0;n<o;n++)r.chars.push(s.charAt(Math.floor(Math.random()*s.length)));else for(let n=0;n<o;n++)r.chars.push(a.charAt(Math.floor(Math.random()*a.length)))})}setCyberLogicMode(e,t){this.cyberConfig.logicMode=e,t!==void 0&&(this.cyberConfig.customText=t),this.updateCyberStrings()}setMatrixLogicMode(e,t){if(this.matrixConfig.logicMode=e,t!==void 0&&(this.matrixConfig.customText=t),this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uTexture){const i=this.cyberMaterial.uniforms.uTexture.value,a=this.createCyberTexture(this.matrixConfig);this.cyberMaterial.uniforms.uTexture.value=a,this.cyberMaterial.needsUpdate=!0,i&&i.dispose()}this.initCyber()}setCyberColor(e){this.cyberConfig.color=e,this.matrixCyberStreams&&this.matrixCyberStreams.forEach(t=>{t.color=e})}setMatrixColor(e){this.matrixConfig.color=e,this.cyberMaterial&&this.cyberMaterial.uniforms&&this.cyberMaterial.uniforms.uColor&&this.cyberMaterial.uniforms.uColor.value.set(e)}setCyberAngle(e){this.cyberConfig.angle=e}setMatrixAngle(e){this.matrixConfig.angle=e,this.currentCyberAngle=e,this.cyberRotationGroup&&(this.cyberRotationGroup.rotation.z=Te.degToRad(-e))}setVibrationEnabled(e){this.vibrationEnabled=e}updateLogoTexture(){if(!this.originalLogoImg)return;const e=512;this._logoRenderCanvas||(this._logoRenderCanvas=document.createElement("canvas"),this._logoRenderCanvas.width=e,this._logoRenderCanvas.height=e);const t=this._logoRenderCanvas,i=t.getContext("2d",{willReadFrequently:!0});i.clearRect(0,0,e,e),i.drawImage(this.originalLogoImg,0,0,e,e);const a=i.getImageData(0,0,t.width,t.height),s=a.data,o=document.body.dataset.theme||"default",r=ye[o]||ye.default,n=r.secondary||r.accent||"#ffffff",h=this.customColors&&this.customColors.mandala?this.customColors.mandala:this.customColor,f=h?h.getHex():parseInt(r.accent.replace("#",""),16),l=parseInt(n.replace("#",""),16),g=f>>16&255,c=f>>8&255,u=f&255,v=l>>16&255,G=l>>8&255,q=l&255;this.themeType=document.body.dataset.themeType||"dark",this.themeType;for(let C=0;C<s.length;C+=4){const I=s[C],w=s[C+1],B=s[C+2],R=s[C+3];if(R<220){s[C+3]=0;continue}I>200&&w>200&&B>200?(s[C]=g,s[C+1]=c,s[C+2]=u,s[C+3]=R):(s[C]=v,s[C+1]=G,s[C+2]=q,s[C+3]=R)}i.putImageData(a,0,0);const A=new ee(t);if(A.minFilter=j,A.magFilter=j,A.generateMipmaps=!0,A.wrapS=Ae,A.wrapT=Ae,this.renderer&&(A.anisotropy=this.renderer.capabilities.getMaxAnisotropy()),this.logoMesh){const C=this.logoMesh.material.map;this.logoMesh.material.map=A,this.logoMesh.material.needsUpdate=!0,C&&C.dispose()}else{const C=new O,I=5.625,w=4.78,B=new Float32Array([-I/2,-w/2,0,I*1.5,-w/2,0,-I/2,w*1.5,0]),R=new Float32Array([0,0,2,0,0,2]);C.setAttribute("position",new D(B,3)),C.setAttribute("uv",new D(R,2));const N=new U({map:A,transparent:!0,opacity:.8,color:16777215,depthTest:!1,depthWrite:!1});if(this.logoMesh=new P(C,N),this.logoMesh.position.set(0,0,-10),this.logoMesh.renderOrder=999,this.scene.add(this.logoMesh),this.camera&&!this.dimmerPlane){const F=new O,L=2e3,J=2e3,K=new Float32Array([-L/2,-J/2,0,L*1.5,-J/2,0,-L/2,J*1.5,0]);F.setAttribute("position",new D(K,3)),this.dimmerPlane=new P(F,new U({color:0,transparent:!0,opacity:0,depthTest:!1})),this.dimmerPlane.renderOrder=998,this.dimmerPlane.position.z=-5,this.camera.add(this.dimmerPlane),this.scene.add(this.camera)}}}initOverlayLogo(){if(this.logoMesh)return;const e=new Image;e.crossOrigin="anonymous",e.src="./mindwave-logo-icon.png",e.onload=()=>{this.originalLogoImg=e,this.updateLogoTexture(),this.targetLogoOpacity!==void 0?this.setLogoOpacity(this.targetLogoOpacity):this.setLogoOpacity(.8)},e.onerror=t=>{}}setLogoOpacity(e){this.targetLogoOpacity=e,this.logoMesh&&this.logoMesh.material?this.logoMesh.material.opacity=e*this.brightnessMultiplier:!this.logoMesh&&e>0&&this.initOverlayLogo()}applyCymaticClassAndVariation(e,t){this.cymaticsCore&&(this.cymaticsCore.setPattern(e,t),this.activeModes.has("cymatics")||this.toggleMode("cymatics"))}setCymaticColor(e,t,i){if(this.cymaticsCore){if(i!==void 0)this.cymaticsCore.setColor(e,t,i);else if(e!==void 0){const a=e,s=this.cymaticsCore.activeClassId||22;this.cymaticsCore.setColor(s,1,a)}}}setCymaticColor2(e){if(!this.cymaticsCore)return;const t=this.cymaticsCore.activeClassId||22;this.cymaticsCore.setColor(t,2,new _(e))}setCymaticColor3(e){if(!this.cymaticsCore)return;const t=this.cymaticsCore.activeClassId||22;this.cymaticsCore.setColor(t,3,new _(e))}setCymaticParam(e,t,i){this.cymaticsCore&&this.cymaticsCore.setParam(e,t,i)}dispose(){this.active=!1,x.animationId&&(cancelAnimationFrame(x.animationId),x.animationId=null),window.removeEventListener("resize",this.resize),window.removeEventListener("resize",this.resizeOverlayCanvas),window.removeEventListener("mindwave:layout-change",this.handleLayoutChange),this._boundSafeMode&&window.removeEventListener("mindwave:safe-mode-start",this._boundSafeMode),this._boundVisibilityChange&&document.removeEventListener("visibilitychange",this._boundVisibilityChange),this._boundThemeChange&&window.removeEventListener("themeChanged",this._boundThemeChange),this.canvas&&(this._boundContextLost&&this.canvas.removeEventListener("webglcontextlost",this._boundContextLost),this._boundContextRestored&&this.canvas.removeEventListener("webglcontextrestored",this._boundContextRestored)),this._boundCymaticPointer&&(window.removeEventListener("mousemove",this._boundCymaticPointer),window.removeEventListener("touchmove",this._boundCymaticPointer),this._boundCymaticPointer=null);const e=i=>{if(i)for(;i.children.length>0;){const a=i.children[0];if(i.remove(a),a.geometry&&a.geometry.dispose(),a.material){if(a.material.map&&a.material.map.dispose(),a.material.uniforms)for(const s in a.material.uniforms){const o=a.material.uniforms[s];o&&o.value&&o.value.dispose&&o.value.dispose()}a.material.dispose()}a.children&&a.children.length&&a.traverse(s=>{s.geometry&&s.geometry.dispose(),s.material&&(s.material.map&&s.material.map.dispose(),s.material.dispose())})}},t=[this.sphereGroup,this.particleGroup,this.lightspeedGroup,this.lavaGroup,this.fireplaceGroup,this.rainforestGroup,this.zenGardenGroup,this.oceanGroup,this.wavesGroup,this.cyberGroup,this.boxGroup,this.dragonGroup,this.galaxyGroup,this.mandalaGroup,this.cymaticsGroup,this.snowflakeGroup];this._fpsRingBuffer=null,this._tempColor=null,this._logoRenderCanvas=null,t.forEach(e),this.cyberMaterial&&(this.cyberMaterial.dispose(),this.cyberMaterial=null),this.fireMaterial&&(this.fireMaterial.dispose(),this.fireMaterial=null),this.particleMaterial&&(this.particleMaterial.dispose(),this.particleMaterial=null),this.lightspeedMaterial&&(this.lightspeedMaterial.dispose(),this.lightspeedMaterial=null),this.rainMaterial&&(this.rainMaterial.dispose(),this.rainMaterial=null),this.petalMaterial&&(this.petalMaterial.dispose(),this.petalMaterial=null),this.logoMesh&&(this.logoMesh.material&&(this.logoMesh.material.map&&this.logoMesh.material.map.dispose(),this.logoMesh.material.dispose()),this.logoMesh.geometry&&this.logoMesh.geometry.dispose(),this.scene&&this.scene.remove(this.logoMesh),this.logoMesh=null);for(const i in this.textures)this.textures[i]&&this.textures[i].dispose&&this.textures[i].dispose();if(this.textures={},this._freqDataArray=null,this.renderer){try{this.renderer.forceContextLoss()}catch{}this.renderer.dispose(),this.renderer=null}this.scene=null,this.camera=null}setWarpSpeed(e){this.lightspeedMaterial&&(this.lightspeedMaterial.uniforms.uSpeed.value=e)}setWarpFov(e){this.camera&&(this.camera.fov=e,this.camera.updateProjectionMatrix())}setWarpChromatic(e){this.lightspeedMaterial}}xe(ze,"CYMATIC_PATTERNS",[{classId:22,variationId:0,name:"Fundamental Zenith",style:"fractal"},{classId:22,variationId:1,name:"Quantum Flower",style:"sacred"},{classId:22,variationId:2,name:"Sacred Sun Resonance",style:"complex"},{classId:22,variationId:3,name:"Ethereal Nexus",style:"geometry"},{classId:22,variationId:4,name:"Golden Ratio Spiral",style:"fractal"},{classId:22,variationId:5,name:"Obsidian Bloom",style:"sacred"},{classId:22,variationId:6,name:"Prismatic Core",style:"complex"},{classId:22,variationId:7,name:"Void Resonance",style:"geometry"},{classId:22,variationId:8,name:"Nebula Plasma",style:"fractal"},{classId:22,variationId:9,name:"Sacred Gold Obsidian",style:"sacred"},{classId:22,variationId:10,name:"Biolum Abyssal",style:"complex"},{classId:22,variationId:11,name:"Emerald Cyber Matrix",style:"geometry"},{classId:22,variationId:12,name:"Liquid Mercury Crimson",style:"fractal"},{classId:22,variationId:13,name:"Quantum Crystal Lattice",style:"sacred"},{classId:22,variationId:14,name:"Solar Flare Harmonics",style:"complex"},{classId:22,variationId:15,name:"Amethyst Hyperdimensional",style:"geometry"},{classId:22,variationId:16,name:"Neon Labyrinth",style:"fractal"},{classId:22,variationId:17,name:"Celestial Mandala",style:"sacred"},{classId:22,variationId:18,name:"Astral Lotus",style:"complex"},{classId:22,variationId:19,name:"Lunar Tides",style:"geometry"},{classId:22,variationId:20,name:"Cybernetic Lotus",style:"fractal"},{classId:22,variationId:21,name:"Bioluminescent Shroom",style:"sacred"},{classId:22,variationId:22,name:"Vortex of Time",style:"complex"},{classId:22,variationId:23,name:"Diamond Lattice",style:"geometry"},{classId:22,variationId:24,name:"Seraphim Wings",style:"fractal"},{classId:22,variationId:25,name:"Fractal Heart",style:"sacred"},{classId:22,variationId:26,name:"Particle Swarm",style:"complex"},{classId:22,variationId:27,name:"Fluid SDF",style:"geometry"},{classId:22,variationId:28,name:"Quantum Topology",style:"fractal"},{classId:22,variationId:29,name:"Ai Cymatic 15",style:"sacred"}]);function vt(){return p?Promise.resolve(p):new Promise(m=>{lt(),setTimeout(()=>m(p),10)})}function lt(){if(!p&&W.canvas&&W.canvas.activeVisualizer&&W.canvas.activeVisualizer.isVisualizer3D&&(p=W.canvas.activeVisualizer),W.canvas&&W.canvas.activeVisualizer){if(p&&W.canvas.activeVisualizer===p)return p;W.canvas.activeVisualizer.dispose(),W.canvas.activeVisualizer=null,p=null}x.animationId&&(cancelAnimationFrame(x.animationId),x.animationId=null);const m=W.canvas||document.getElementById("visualizer");if(!p&&m){const e=m&&m.activeVisualizer&&m.activeVisualizer.isVisualizer3D?{activeModes:m.activeVisualizer.activeModes,mode:m.activeVisualizer.mode,mindWaveMode:m.activeVisualizer.mindWaveMode,cyberLogicMode:m.activeVisualizer.cyberLogicMode,cyberCustomText:m.activeVisualizer.cyberCustomText,currentCyberAngle:m.activeVisualizer.currentCyberAngle,cyberSpeedMultiplier:m.activeVisualizer.cyberSpeedMultiplier,rainbowEnabled:m.activeVisualizer._rainbowEnabled}:{};p=new ze(m,e),m.activeVisualizer=p,setTimeout(()=>{p&&(p.updateVisibility(),ct())},0)}return p}function gt(){return p}let ge=!1;function xt(){ge=!0,p&&x.animationId&&(cancelAnimationFrame(x.animationId),x.animationId=null,p.renderer&&p.scene&&p.camera&&p.renderer.render(p.scene,p.camera))}function ct(){p&&!x.animationId&&(p.active=!0,p.render(x.analyserLeft,x.analyserRight),ge=!1)}function yt(){return ge}function bt(m){p&&p.toggleMode(m)}function wt(m){p&&(p.setGlobalSpeed(m),p.setCyberSpeed&&p.setCyberSpeed(m))}function Ct(m,e=null){p&&(p.setVisualColor(m,e),p.setCyberColor&&(!e||e=="cyber")&&p.setCyberColor(m))}function Mt(m){p&&p.setGlobalBrightness&&p.setGlobalBrightness(m)}function St(m){p&&p.setLogoOpacity(m)}function Tt(m){p&&p.setMouseInfluence(m)}window.toggleGalaxySun=function(){return p?p.toggleGalaxySunStyle():null};window.setCymaticPattern=function(m,e){p&&p.cymaticsCore?(p.cymaticsCore.setPattern(m,e),p.activeModes.has("cymatics")||(window.switchRightTab&&window.switchRightTab("cymatics",document.querySelector('.tab-pill[title="Cymatics"]')),p.activeModes.add("cymatics"),p.cymaticsGroup.visible=!0)):(window.setVisualMode&&window.setVisualMode("cymatics"),setTimeout(()=>{p&&p.cymaticsCore&&p.cymaticsCore.setPattern(m,e)},500))};window.setCymaticColor=function(m,e,t){p&&p.cymaticsCore&&p.cymaticsCore.setColor(m,e,t)};window.setCymaticParam=function(m,e,t){p&&p.cymaticsCore&&p.cymaticsCore.setParam(m,e,t)};window.setConstellationLayer2Type=function(m){p&&p.cymaticsCore&&p.cymaticsCore.setConstellationLayer2Type(m)};let pe=null,X=null,E=null,V=null,ve=!0;window.toggleQuantumHum=function(m){if(ve=m,V&&X){const e=X.currentTime;if(V.gain.cancelScheduledValues(e),V.gain.setValueAtTime(V.gain.value,e),!m)V.gain.linearRampToValueAtTime(0,e+.5);else{const i=(document.getElementById("observerToggle")?document.getElementById("observerToggle").checked:!1)?.3:.15;V.gain.linearRampToValueAtTime(i,e+.5)}}};window.toggleObserverEffect=function(m){if(!p||!p.cymaticsCore)return;p.cymaticsCore.activeClassId!==25&&window.setCymaticPattern(25,0);const e=m?1:0,t=p.cymaticsCore.materials[25];if(!t)return;try{if(!X){const s=window.AudioContext||window.webkitAudioContext;X=new s}X.state==="suspended"&&X.resume(),E||(E=X.createOscillator(),V=X.createGain(),E.type="triangle",E.frequency.value=108,V.gain.value=0,E.connect(V),V.connect(X.destination),E.start());const a=X.currentTime;V.gain.cancelScheduledValues(a),E.frequency.cancelScheduledValues(a),V.gain.setValueAtTime(V.gain.value,a),E.frequency.setValueAtTime(E.frequency.value,a),m?(ve&&V.gain.linearRampToValueAtTime(.3,a+1),E.frequency.exponentialRampToValueAtTime(216,a+1)):(ve&&V.gain.linearRampToValueAtTime(.15,a+1),E.frequency.exponentialRampToValueAtTime(108,a+1))}catch{}pe&&cancelAnimationFrame(pe);const i=()=>{let a=t.uniforms.uObserver.value;const s=e-a;if(Math.abs(s)<.01){t.uniforms.uObserver.value=e;return}t.uniforms.uObserver.value=a+s*.05,pe=requestAnimationFrame(i)};i()};export{ze as Visualizer3D,gt as getVisualizer,lt as initVisualizer,yt as isVisualsPaused,xt as pauseVisuals,vt as preloadVisualizer,ct as resumeVisuals,Tt as setMouseInfluence,Mt as setVisualBrightness,Ct as setVisualColor,St as setVisualLogoOpacity,wt as setVisualSpeed,bt as toggleVisual};
//# sourceMappingURL=visualizer_v4-CklXs-gG.js.map
