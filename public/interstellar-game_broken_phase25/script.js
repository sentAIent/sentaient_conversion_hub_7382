console.log("🚀 INTERSTELLAR ENGINE: v3_stable LOADED");
console.log("🚀 INTERSTELLAR ENGINE Phase 18 - Eternal Frontier");
...
class InterstellarEngine {
    ...
    constructor() {
        console.log("Interstellar Engine v3.2 - Phase 18: The Eternal Frontier");
        ...
        document.title = "INTERSTELLAR v3.2 - Phase 25: Majestic Restoration";
        ...
        this.spaceBases = [];
        this.planets = [];
        const savedStyles = localStorage.getItem('activeBgStyles');
        this.activeStyles = new Set(); // Will be populated during splash sequence

        this.shootingStarsActive = [];
        ...
        this.nebulae = [];
        this.galaxies = [];
        this.blackHoles = []; // Legacy field
        this.spacecraft = [];
        this.matrixStreams = [];
        this.cyberNodes = []; // Phase 30
        this.alienArtifacts = []; // Phase 30
        this.decorations = [];
        this.asteroids = [];
        this.comets = [];
        this.lootItems = [];
        ...
        this.speedLines = []; // RE-ADDED FOR PHASE 3
        ...
        // --- PHASE 19: MISSING STATE INITIALIZATION ---
        ...
        try {
            ...
            class SafeMock {
                ...
                updateWarpUI() { /* no-op */ }
                showEmpireOverview() { console.warn("[Mock] showEmpireOverview ignored"); }
            ...
            Object.entries(managers).forEach(([globalName, localName]) => {
                if (typeof window[globalName] === 'function') {
                    ...
                    try {
                        ...
                        console.log(`✅ [BOOT] ${globalName} READY.`);
                    } catch (e) {
                        console.error(`❌ [BOOT] ${globalName} FAILED:`, e);
            ...
            if (window.BaseBuilder) {
                this.baseBuilder = new window.BaseBuilder(this);
            } else {
                console.warn('[Engine] BaseBuilder missing.');
                this.baseBuilder = null;
                this.spaceBases = this.spaceBases || [];
                this.spaceBase = null;
            }
    ...
    async splashTransition() {
        ...
        try {
            ...
            const savedStyles = JSON.parse(localStorage.getItem('activeBgStyles') || '["deep-space", "nebula", "alien", "cyber"]');
            
            // Only generate what is actually needed
            if (savedStyles && savedStyles.length > 0) {
                ...
                // Default to standard majestic set if no saved preference
                ['deep-space', 'nebula', 'alien', 'cyber'].forEach(s => {
                    this.activeStyles.add(s);
                    this.proceduralManager?.generateSingleStyle(s);
                });
            }
            ...
            // Wait for user interaction or auto-proceed after 3s
            return new Promise(resolve => {
                const autoTimer = setTimeout(() => proceed(), skipRequested ? 100 : 3000);
                const proceed = () => {
                    ...
                    // SEAMLESS TRANSITION: Auto-engage flight mode for "PRESS TO FLY"
                    if (this.engineCore) {
                        this.engineCore.toggleFlightMode();
                    } else {
    ...
    init() {
        ...
        */
        ...
        // Initialize Hangar with default ship if empty
        if (!this.hangarShips || this.hangarShips.length === 0) {
            console.log("🛠️ Hangar Registry is empty. Initializing with Interceptor Class ship.");
            this.hangarShips = [{
                id: 'interceptor',
                name: 'Vanguard Alpha',
                model: 'V-1 Interceptor',
                type: 'interceptor',
                color: '#00f3ff',
                speed: 1500,
                armor: 250,
                power: 1000,
                desc: 'A versatile, high-speed strike craft designed for deep space recon and sector stabilization.',
                health: 100,
                shield: 50,
                unlocked: true
            }];
    ...
    toggleBgStyle(category) {
        console.log(`[BG Toggle] ${category}`);
        
        // Find if any style in this category is active
        let activeStyleInCat = null;
        this.activeStyles.forEach(s => {
            if (s === category || (s.startsWith(category) && s.includes('-'))) {
                activeStyleInCat = s;
            }
        });
        ...
        if (activeStyleInCat) {
            ...
            this.activeStyles.delete(activeStyleInCat);
            this.clearStyleData(category);
        } else {
            // Enabling - only generate this specific style
            this.activeStyles.add(category);
            if (this.proceduralManager) {
                this.proceduralManager.generateSingleStyle(category);
            }
        }
        ...
        if (this.hudManager) this.hudManager.updateBgUI();
        this.updateBgUI(); // Phase 25: Ensure top-bar buttons highlight correctly
    ...
    showShipModal() { 
        ...
        this.hangarManager.showShipModal(); 
    }
    ...
    clearStyleData(style) {
        // Broad clear based on category prefix
        if (this.backgroundStars) {
            this.backgroundStars = this.backgroundStars.filter(s => {
                if (s.style === style) return false;
                if (style === 'nebula' && s.style.startsWith('nebula')) return false;
                return true;
            });
        }
        ...
        if (style === 'deep-space') {
            ...
            this.blackHoles = [];
            this.planets = [];
        } else if (style === 'nebula' || style.startsWith('nebula')) {
            this.nebulae = [];
        } else if (style === 'alien') {
            this.spacecraft = [];
            this.alienArtifacts = [];
    ...
    updateBgUI() {
        ...
        document.querySelectorAll('.bg-toggle').forEach(btn => {
            ...
            // Flexible match: button 'nebula' should match 'nebula', 'nebula-blue', etc.
            let isActive = false;
            this.activeStyles.forEach(activeStyle => {
                if (activeStyle === btnStyle || (activeStyle.startsWith(btnStyle) && activeStyle.includes('-'))) {
                    isActive = true;
                }
            });
    ...
    toggleRotationPanel() {
        if (!document.getElementById('rotationPanel')?.classList.contains('hidden')) {
            document.getElementById('rotationPanel')?.classList.add('hidden');
            return;
        }

        if (this.hudManager) this.hudManager.closeAllModals();
        document.getElementById('rotationPanel')?.classList.remove('hidden');
    }
    ...
    updateBgUI() { this.hudManager.updateBgUI(); }
    updateStats() {
        ...
        statsEl.textContent = `Mode: ${mode} | Stars: ${starCount} | Constellations: ${constellationCount} | Zoom: ${zoom}x`;
    }
    ...
    // --- Bridge SDK Implementation (PURGED for Phase 25 Stabilization) ---
    requestLoadData() {
        // [STUB] Cloud bridge decoupled to prevent boot-up initialization hang.
        console.log('[Bridge] Cloud load bypassed.');
    }
    ...
    renderPlayerShip(ctx, time, overrideShip = null, isGhost = false) {
        ...
        const size = 45 / safeZoom; // Base ship size
    ...
    renderMinerals(ctx, time) {
        this.minerals.forEach(mineral => {
            ...
            gradient.safeAddColorStop(0, mineral.color);
            gradient.safeAddColorStop(0.5, mineral.color + '88');
            gradient.safeAddColorStop(1, mineral.color + '00');
    ...
    initHangar() {
        ...
        track.innerHTML = '';
        const gemBalanceEl = document.getElementById('hangarGemBalance');
        if (gemBalanceEl) gemBalanceEl.textContent = this.playerGems || 0;
        ...
        this.hangarShips.forEach((ship, index) => {
            ...
            const lockHtml = isLocked ? `<div class="lock-overlay">🔒 ENCRYPTED BEYOND CLEARANCE</div>` : '';
    ...
    selectShip() {
        if (!this.hangarShips || this.hangarShips.length === 0) return;
        const shipData = this.hangarShips[this.currentHangarBay || 0];
        ...
        const ships = {
            ...
            viper: { name: 'Viper', maxSpeed: 120, acceleration: 1.1, premium: true, cost: 1000 },
            bulwark: { name: 'Bulwark', maxSpeed: 60, acceleration: 0.4, premium: true, cost: 1500 },
            prospector: { name: 'Prospector', maxSpeed: 80, acceleration: 0.7, premium: true, cost: 2000 },
            spectre: { name: 'Spectre', maxSpeed: 105, acceleration: 0.9, premium: true, cost: 3500 },
            titan: { name: 'Titan', maxSpeed: 50, acceleration: 0.3, premium: true, cost: 5000 }
        };

        const ship = ships[shipId];
        const isLocked = ship && ship.premium && !this.unlockedShips.includes(shipId);

        if (isLocked) {
            if (confirm(`Unlock ${ship.name} for 💎${ship.cost}?`)) {
                this.unlockShip(shipId, ship.cost);
            }
            return;
        }

        if (ship && this.playerShip) {
            this.playerShip.type = shipId;
            this.playerShip.maxSpeed = ship.maxSpeed;
            this.playerShip.acceleration = ship.acceleration;
            localStorage.setItem('playerShipType', shipId);
            this.hudManager.showToast(`SYSTEMS ONLINE: ${ship.name.toUpperCase()} CLASS`, 2000, 'success');
    ...
    updateHangarUI() {
        ...
        const currentBay = this.currentHangarBay || 0;
        
        if (track) {
            track.style.transform = `translateX(${-(currentBay) * 100}%)`;
        }
        if (indicator) {
            const total = (this.hangarShips && this.hangarShips.length) || 0;
            indicator.textContent = `BAY ${(currentBay) + 1} / ${total}`;
        }

        // Phase 19: Update Sidebar Stats
        this.updateHangarStats(currentBay);
    }
    ...
    renderHangarPreviews() {
        ...
        const total = (this.hangarShips && this.hangarShips.length) || 0;
        if (total === 0) return;
        const current = this.currentHangarBay || 0;
        const toRender = [
            current,
    ...
    renderShipPreview(type) {
        ...
        gradient.safeAddColorStop(0, 'rgba(0, 243, 255, 0.2)');
        gradient.safeAddColorStop(1, 'rgba(0, 0, 0, 0)');
        ctx.fillStyle = gradient;
    ...
    drawVaultGem(canvas, type, time) {
        ...
        gradient.safeAddColorStop(0, def.color);
        gradient.safeAddColorStop(1, 'transparent');
        ctx.fillStyle = gradient;
        ctx.beginPath();
    ...
    async start() {
        try {
            ...
            // 2. Cinematic Splash Transition
            await this.splashTransition();
            console.log(`📍 [Boot] Splash sequence complete at ${Math.round(performance.now() - startTimestamp)}ms`);
