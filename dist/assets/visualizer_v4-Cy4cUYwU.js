        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                if (pos.z > curlStartHeight) {
                    ...
                    // BEND THE ENTIRE CREST AS ONE SOLID PIECE!
                    // We use a perfectly symmetric curl envelope so the front and back slopes throw forward identically, preventing tearing!
                    float curlEnvelope = exp(-pow(waveX * 0.05, 2.0)); 
                    
                    // Max bend is ~1.15 PI (curls past vertical, pointing down into the trough)
                    float bendAngle = pow(normH, 1.5) * 3.14159 * 1.15 * normalizedCurl * curlEnvelope; 
                    
                    // The entire lip physically throws out over the trough before dropping
                    float forwardThrow = pow(normH, 2.0) * 25.0 * normalizedCurl * curlEnvelope;
