...
class SurfaceManager {
    constructor(game) {
        ...
        this.BUILDING_TYPES = {
            'outpost': { name: 'Command Outpost', cost: 5000, foodRate: 5, icon: '🏠', desc: 'Base of operations. Allows more complex modules.' },
            'extractor': { name: 'Industrial Extractor', cost: 12000, genRate: 25, icon: '⛏️', desc: 'Mines minerals and generates passive credits.' },
            'shield_gen': { name: 'Atmospheric Shield', cost: 25000, powerReq: 10, icon: '🛡️', desc: 'Protects the surface from cosmic hazards.' },
            'solar_array': { name: 'Solar Relay', cost: 8000, powerGen: 30, icon: '⚡', desc: 'Generates power for colony systems.' },
            'research_lab': { name: 'Science Bay', cost: 15000, rpRate: 0.1, powerReq: 15, icon: '🧪', desc: 'Generates Research Points (RP) per minute.' }
        };
    ...
    completeLanding(planet) {
        ...
        // Trigger UI Refresh
        if (this.game.hudManager.updateSurfaceUI) {
            this.game.hudManager.updateSurfaceUI();
        }
    }
