...
`,ot=f=>f===1?`
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                // Adjusted offset from 100.0 to 80.0 to bring the wave forward +20 on the X axis.
                float waveX = pos.x + 80.0;
                vWaveX = waveX; 
                ...
                if (uLoopActive > 0.5) {
                    ...
                    waveX = mod(waveX + loopLen*0.5, loopLen) - loopLen*0.5;
                }
                ...
                // Symmetrical peak: exact same slope for the front and the back!
                float envelope = exp(-pow(waveX * 0.04, 2.0));
                ...
                if (envelope > 0.01) {
                    float chaos = snoise(pos.xy * 0.1 + vec2(time * 0.5));
                ...
                if (pos.z > curlStartHeight) {
                    ...
                    // Symmetrical curl envelope perfectly matching the height envelope
                    float curlEnvelope = exp(-pow(waveX * 0.04, 2.0)); 
                    
                    // Wrap the curl to exactly 135 degrees (0.75 PI) so the lip hits the ocean floor perfectly
                    float bendAngle = pow(normH, 1.2) * 3.14159 * 0.75 * normalizedCurl * curlEnvelope; 
                    
                    // The massive forward throw (monotonic function)
                    // Decoupled from sine, it strictly increases with height, guaranteeing no overlap!
                    float monotonicXPush = pow(normH, 1.5) * 60.0 * normalizedCurl * curlEnvelope;
                    
                    // Hyper-aggressive radius shrinking ONLY at the very tip (normH > 0.8)
                    // This creates the "carpet roll" effect to give the 2D plane fake solid thickness
                    float tipRoll = smoothstep(0.8, 1.0, normH);
                    float radius = h * (1.0 - tipRoll * 0.8);
                    
                    float originalX = pos.x;
                    
                    // x shift = STRICTLY monotonic forward push! No sine wave!
                    pos.x = originalX + monotonicXPush;
                    
                    // z shift = circular arc to drop it to the ocean floor
        ...
        `,s=new O({vertexShader:i,fragmentShader:a,uniforms:t,transparent:!0,side:Z,wireframe:!1});this.tsunamiMaterial=s;const o=new G(e,s);o.rotation.x=-Math.PI/2,o.position.y=-50,o.position.z=-150,o.position.x=0,o.frustumCulled=!1,this.tsunamiGroup.add(o);const n="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="512" height="512">
            <g>
            ...
            `,transparent:!0,side:Z,blending:T,depthWrite:!1})}initRainforest(){const e=this.batterySaver?400:1e3,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*40,i[o+2]=(Math.random()-.5)*40,a[o]=Math.random(),a[o+1]=Math.random(),a[o+2]=.08+Math.random()*.12}t.setAttribute("position",new k(i,3)),t.setAttribute("aRandom",new k(a,3)),this.rainMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new A(8965375)},uIntensity:{value:.6}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),this.raindrops=new Y(t,this.rainMaterial),this.rainforestGroup.add(this.raindrops),this.rainforestGroup.visible=!1}initZenGarden(){const e=this.batterySaver?200:500,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let o=0;o<e;o++){const r=o*3;i[r]=(Math.random()-.5)*40,i[r+1]=(Math.random()-.5)*20,i[r+2]=(Math.random()-.5)*40,a[r]=Math.random(),a[r+1]=Math.random(),a[r+2]=Math.random()*Math.PI*2}t.setAttribute("position",new k(i,3)),t.setAttribute("aRandom",new k(a,3)),this.petalMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new A(16758725)},uIntensity:{value:.8}},vertexShader:`
                attribute vec3 aRandom;
                uniform float uTime, uSpeed;
                varying float vAlpha;
                void main() {
