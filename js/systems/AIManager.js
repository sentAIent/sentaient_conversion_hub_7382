...
class AIManager {
    constructor(game) {
        ...
        this.rivals = [];
        this.factions = {
            VOID_BORN: { name: "The Void-Born", rating: 0, stance: "NEUTRAL", color: "#FF4500" },
            NEURAL_NEXUS: { name: "Neural Nexus", rating: 0, stance: "NEUTRAL", color: "#00CED1" }
        };
        this.outposts = [];
        this.ships = [];
    ...
    seedInitialFactions() {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            this.spawnOutpost(p, "VOID_BORN");
        }

        for (let i = 0; i < 2; i++) {
            const p = planets[Math.floor(planets.length / 2) + i];
            this.spawnOutpost(p, "NEURAL_NEXUS");
        }
    }
    ...
    update(dt) {
        ...
        if (player) {
            for (let i = this.ships.length - 1; i >= 0; i--) {
                ...
                if (dist < 2500) {
                    // --- NEW: Inter-Faction AI Targeting ---
                    if (!ship.currentTarget || ship.currentTarget === player || ship.targetUpdateTimer <= 0) {
                        ship.currentTarget = this.findNearestHostileForShip(ship);
                        ship.targetUpdateTimer = 60 + Math.random() * 60; // Tick every second
                    }
