...
// toggleMixerSection moved to line 2571 to resolve SyntaxError
...
// --- SOPHISTICATED UI: TAB & UCC MANAGEMENT ---
window.switchTab = function(side, tabId) {
    console.log(`[UI] Switching ${side} tab to: ${tabId}`);
    // Hide all panels for this side
    const panels = document.querySelectorAll(`[id^="${side}-tab-"]`);
    panels.forEach(p => p.classList.remove('active'));
    
    // Show selected
    const target = document.getElementById(`${side}-tab-${tabId}`);
    if (target) target.classList.add('active');
    
    // Update pills
    const pills = document.querySelectorAll(`#${side}Panel .tab-pill`);
    pills.forEach(p => {
        if (p.textContent.toLowerCase().includes(tabId.toLowerCase())) {
    ...
    localStorage.setItem(`mindwave_tab_${side}`, tabId);
};

window.switchSubTab = function(subTabId) {
    console.log(`[UI] Switching subtab to: ${subTabId}`);
    ...
    const pills = document.querySelectorAll('#right-tab-submenus .tab-pill');
    pills.forEach(p => {
        if (p.textContent.toLowerCase() === subTabId.toLowerCase()) {
            p.classList.add('active');
        } else {
            p.classList.remove('active');
        }
...
window.toggleUCCPin = function() {
    const ucc = document.getElementById('commandCenter');
    if (!ucc) return;
    
    if (ucc.classList.contains('pinned-bottom')) {
        ucc.classList.remove('pinned-bottom');
        ucc.classList.add('pinned-top');
        localStorage.setItem('mindwave_ucc_pin', 'top');
    } else {
        ucc.classList.remove('pinned-top');
        ucc.classList.add('pinned-bottom');
        localStorage.setItem('mindwave_ucc_pin', 'bottom');
    }
};

window.updateUCCWidth = function() {
    const ucc = document.getElementById('commandCenter');
    const leftPanel = document.getElementById('leftPanel');
    const rightPanel = document.getElementById('rightPanel');
    if (!ucc) return;
    
    // Check if panels are visible (not translated off-screen)
    ...
    const leftW = leftActive ? leftPanel.offsetWidth : 0;
    const rightW = rightActive ? rightPanel.offsetWidth : 0;
    
    ucc.style.left = `${leftW}px`;
    ucc.style.right = `${rightW}px`;
};

// Override legacy toggles to redirect to Submenus tab
window.toggleMatrixSettings = () => {
    switchTab('right', 'submenus');
    switchSubTab('matrix');
};
window.toggleCyberSettings = () => {
    switchTab('right', 'submenus');
    switchSubTab('cyber');
};
window.toggleGalaxySettings = () => {
    switchTab('right', 'submenus');
    switchSubTab('galaxy');
};
export function setupUI() {
    // 1. Populate els (DOM Element References) - MUST BE FIRST
    ...
    els.speedSliderContainer = document.getElementById('speedSliderContainer');
    ...
    // 2. Restore tab states
    const lastLeftTab = localStorage.getItem('mindwave_tab_left') || 'brainwaves';
    const lastRightTab = localStorage.getItem('mindwave_tab_right') || 'optics';
    const uccPin = localStorage.getItem('mindwave_ucc_pin') || 'bottom';
    
    switchTab('left', lastLeftTab);
    switchTab('right', lastRightTab);
    
    const ucc = document.getElementById('commandCenter');
    if (ucc) {
        ucc.classList.remove('pinned-top', 'pinned-bottom');
        ucc.classList.add(`pinned-${uccPin}`);
    }
    
    // 3. Desktop Auto-Reveal
    if (window.innerWidth > 1024) {
        ...
        // Force width update after revealing
        setTimeout(updateUCCWidth, 100);
    }
    
    // Add resize listener for UCC
    window.addEventListener('resize', updateUCCWidth);
    // Observe sidebars for width changes/toggles
    const uiObserver = new MutationObserver(updateUCCWidth);
    const leftEl = document.getElementById('leftPanel');
    const rightEl = document.getElementById('rightPanel');
    if (leftEl) uiObserver.observe(leftEl, { attributes: true, attributeFilter: ['class', 'style'] });
    if (rightEl) uiObserver.observe(rightEl, { attributes: true, attributeFilter: ['class', 'style'] });
    
    // Initial width sync
    setTimeout(updateUCCWidth, 500);

    // Warp Sliders
    const warpSpeed = document.getElementById('warpSpeedSlider');
    if (warpSpeed) {
        warpSpeed.oninput = (e) => {
            const val = parseFloat(e.target.value);
            document.getElementById('warpSpeedVal').textContent = val.toFixed(1);
            const viz = getVisualizer();
            if (viz && viz.setWarpSpeed) viz.setWarpSpeed(val);
        };
    }
    const warpFov = document.getElementById('warpFovSlider');
    if (warpFov) {
        warpFov.oninput = (e) => {
            const val = parseInt(e.target.value);
            document.getElementById('warpFovVal').textContent = val;
            const viz = getVisualizer();
            if (viz && viz.setWarpFov) viz.setWarpFov(val);
        };
    }
    const warpChromatic = document.getElementById('warpChromaticSlider');
    if (warpChromatic) {
        warpChromatic.oninput = (e) => {
            const val = parseFloat(e.target.value);
            document.getElementById('warpChromaticVal').textContent = val.toFixed(2);
            const viz = getVisualizer();
            if (viz && viz.setWarpChromatic) viz.setWarpChromatic(val);
        };
    }
    const warpTurbulence = document.getElementById('warpTurbulenceSlider');
    if (warpTurbulence) {
        warpTurbulence.oninput = (e) => {
            const val = parseFloat(e.target.value);
            document.getElementById('warpTurbulenceVal').textContent = val.toFixed(2);
            const viz = getVisualizer();
            if (viz && viz.setWarpTurbulence) viz.setWarpTurbulence(val);
        };
    }
    const warpStars = document.getElementById('warpStarsSlider');
    if (warpStars) {
        warpStars.oninput = (e) => {
            const val = parseFloat(e.target.value);
            document.getElementById('warpStarsVal').textContent = val.toFixed(2);
            const viz = getVisualizer();
            if (viz && viz.setWarpStars) viz.setWarpStars(val);
        };
    }
    ...
    if (els.leftToggle) els.leftToggle.addEventListener('click', () => {
        ...
        els.leftPanel.classList.toggle('active');
    ...
    if (els.rightToggle) els.rightToggle.addEventListener('click', () => {
        ...
        els.rightPanel.classList.toggle('active');
        updateVisualizerScale();
        window.dispatchEvent(new CustomEvent('mindwave:layout-change'));
    ...
    if (els.cubeBtn) els.cubeBtn.addEventListener('click', () => setVisualMode('cube'));
    ...
    if (els.cyberBtn) els.cyberBtn.addEventListener('click', () => setVisualMode('cyber'));
    ...
    // IMMEDIATE DISPLAY: Reveal controls now
    const targets = document.querySelectorAll('.ui-fade-target');
    targets.forEach(t => {
        t.style.opacity = '1';
        t.classList.remove('no-transition', 'idle-hidden');
    });
    ...
    window.setBinauralMode = setBinauralMode; // NEW
    window.resumeVisuals = resumeVisuals; // NEW
    window.openProfile = openProfile;
    window.startSweepPreset = startSweepPresetUI; 
    window.stopSweep = stopSweepUI; 
    window.handleHyperGammaClick = handleHyperGammaClick; 
    ...
    const applyVisualDefaults = () => {
        ...
        try {
            ...
            if (ucc) {
                ...
                setTimeout(() => ucc.classList.remove('no-transition'), 100);
            }
            if (header) {
                updateTopBarWidth();
...
export function setVisualMode(mode, forceState = null) {
    ...
    if (viz) {
        ...
        if (Array.isArray(mode)) {
            ...
            // EXPERT REFACTOR: Define additive vs primary visuals
            const additiveModes = ['cyber', 'matrix'];
            const modesToApply = Array.isArray(mode) ? mode : [mode];
            
            // If we are activating a PRIMARY visual (not cyber/matrix), 
            // clear other primary visuals but leave additive layers alone.
            const isApplyingPrimary = modesToApply.some(m => !additiveModes.includes(m));
            
            if (isApplyingPrimary) {
                // Clear all modes that are NOT in the additive list
                Array.from(viz.activeModes).forEach(m => {
                    if (!additiveModes.includes(m)) viz.activeModes.delete(m);
                });
            }
        }
...
/**
 * Set Binaural Mode (binaural, isochronic, mono)
 * @param {string} mode 
 */
export function setBinauralMode(mode) {
    console.log('[Audio] Setting mode to:', mode);
    state.binauralMode = mode;
    
    // Update UI Buttons
    const modes = ['Bin', 'Iso', 'Mono'];
    modes.forEach(m => {
        const btn = document.getElementById(`mode${m}`);
        if (btn) {
            if (m.toLowerCase() === mode.replace('binaural', 'bin').replace('isochronic', 'iso').toLowerCase()) {
    ...
    // Update Engine
    if (window.BrainwaveEngine && window.BrainwaveEngine.setMode) {
        window.BrainwaveEngine.setMode(mode);
    }
}
...
export function loadSettings(payload) {
    ...
    if (settings.base && els.baseSlider) {
    ...
    if (settings.beat && els.beatSlider) {
        if (!storyState.isPlaying && !storyState.isTransitioning) els.beatSlider.value = settings.beat;
    }
    if (settings.beatsVol && els.volSlider) els.volSlider.value = settings.beatsVol;
    if (settings.masterVol && els.masterVolSlider) els.masterVolSlider.value = settings.masterVol;
    if (settings.atmosMaster && els.atmosMasterSlider) els.atmosMasterSlider.value = settings.atmosMaster;
    ...
    sliders.forEach(s => {
        ...
        if (slider) {
            slider.addEventListener('input', (e) => {
                const val = parseFloat(e.target.value);
                if (valDisp) valDisp.textContent = s.id.includes('Angle') ? `${val}°` : `${val.toFixed(1)}x`;
                const viz = getVisualizer();
