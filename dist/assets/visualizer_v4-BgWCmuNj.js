...
`,ot=f=>f===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class st{constructor(e){this.group=e,this.meshes=[],this.activeClassId=22,this.activeVariationId=0,this.materials={},this.rainbowMode=!1;const t=new Ee(new Uint8Array([0,0,0,0]),1,1,ke);t.needsUpdate=!0,this.dummyTexture=t;const i=new $(2,2);for(let a=1;a<=25;a++){let o=ot(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                if (uLoopActive > 0.5) {
                    ...
                    // The wave peak starts left (+60) and sweeps right (-60)
                    waveX = pos.x + 60.0 - (phase * 120.0);
                }
                
                // Massive base height
                float baseHeight = 60.0 * activeAmp * (1.0 + react * 0.3);
                
                // Asymmetric bell curve for the left/right profile
                float envelope = 0.0;
                if (waveX < 0.0) {
                ...
                // Localize the wave along the depth axis (pos.y) so it forms a single distinct Kanagawa peak
                // instead of an infinitely long tunnel/crater.
                float depthEnvelope = exp(-pow(pos.y * 0.015, 2.0));
                ...
                if (uReactivity > 0.1) {
                    // Scroll the noise along the wave to simulate water flowing up the face
                    float chaos = snoise(vec2(pos.x * 0.2 - time * 2.0, pos.y * 0.2 + time));
                    pos.z += (chaos * 20.0 * react * envelope); 
                }
                
                // --- TRUE SURFING BARREL CURL ---
                // Rotate the top of the wave around a pivot point to form a perfect tube
                float curlStartHeight = 10.0;
                if (pos.z > curlStartHeight) {
                    float heightAbovePivot = pos.z - curlStartHeight;
                    // Max activeCurl is 4.0, bendAngle determines the radians of rotation
                    float bendAngle = heightAbovePivot * 0.035 * activeCurl * envelope; 
                    
                    float originalX = pos.x;
                    float originalZ = pos.z;
                    
                    // The crest pushes forward (X) and curls down (Z) to form the pipeline
        ...
        `,s=new O({vertexShader:i,fragmentShader:a,uniforms:t,transparent:!0,side:K,wireframe:!1});this.tsunamiMaterial=s;const o=new G(e,s);o.rotation.x=-Math.PI/2,o.position.y=-70,o.position.z=-150,o.position.x=0,o.frustumCulled=!1,this.tsunamiGroup.add(o);const n="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="512" height="512">
            <g>
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new Y(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new $(40,40,32,32);this.zenWaterMaterial=new E({color:2245734,transparent:!0,opacity:.3,side:K}),this.zenWater=new G(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new $(300,100,128,64);this.oceanWave=new G(e,new O({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new A(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
