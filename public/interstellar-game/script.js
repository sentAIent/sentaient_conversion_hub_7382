...
class AetherEngine {
    constructor() {
        ...
        this.activeStyles = new Set(['deep-space', 'nebula']); // Restored Phase 18 Defaults
    ...
    init() {
        ...
        if (matrixColorPicker) {
            const updateMatrixColor = (val) => {
                this.matrixColor = val;
                this.matrixColorCustomized = true;
                
                // Aggressively disable ALL rainbow modes when a specific color is chosen
                if (this.matrixRainbowMode) {
                    this.matrixRainbowMode = false;
                    const btn = document.getElementById('matrixRainbowBtn');
                    if (btn) btn.classList.remove('active');
                }
                if (this.colorMode === 'rainbow') {
                    this.colorMode = 'fixed';
                    if (typeof this.updateColorModeUI === 'function') this.updateColorModeUI();
                }
                const templateCb = document.getElementById('templateRainbowMode');
                if (templateCb && templateCb.checked) {
                    templateCb.checked = false;
                }

                if (matrixColorHex && matrixColorHex.value !== val) matrixColorHex.value = val;
                if (this.matrixStreams) this.matrixStreams.forEach(s => s.color = this.matrixColor);
            };

            // Standard real-time input event (supported by Safari/Chrome/Firefox)
            matrixColorPicker.addEventListener('input', (e) => updateMatrixColor(e.target.value));
            matrixColorPicker.addEventListener('change', (e) => updateMatrixColor(e.target.value));
        }
        if (matrixColorHex) {
    ...
    setFixedColor(hexColor) {
        ...
        // Sync global color choice to the Matrix system
        this.matrixColor = hexColor;
        this.matrixColorCustomized = true;
        this.matrixRainbowMode = false;
        
        // Update Matrix UI to match
        const matrixBtn = document.getElementById('matrixRainbowBtn');
        if (matrixBtn) matrixBtn.classList.remove('active');
        const matrixHex = document.getElementById('matrixColorHex');
        if (matrixHex) matrixHex.value = hexColor;
        const matrixPicker = document.getElementById('matrixColorPicker');
        if (matrixPicker) matrixPicker.value = hexColor;

        if (this.matrixStreams) {
            this.matrixStreams.forEach(s => s.color = this.matrixColor);
        }
    ...
    updateMinerals() {
        ...
        if (this.playerShip.type === 'hauler') {
            const tractorRadius = this.playerShip.tractorRadius || 25;
            const maxPullVelocity = 40; // Pull strength

            for (const mineral of this.minerals) {
                // Skip dark debris - coal and dark matter cannot be tractor-pulled
                ...
                if (dist < tractorRadius && dist > 5) {
                    ...
                    // Normalize direction and apply velocity (3D)
                    const dirX = dx / dist;
                    const dirY = dy / dist;
                    const dirZ = dz / dist;
    ...
    draw(rawTime) {
        ...
        if (this.activeStyles.has('cyber') && this.matrixStreams) {
            ...
            const isTemplateRainbow = document.getElementById('templateRainbowMode')?.checked;
            ...
            this.matrixStreams.forEach((stream, streamIndex) => {
                ...
                for (let i = 0; i < visibleLength && i < stream.chars.length; i++) {
                    ...
                    const isGlobalRainbow = this.colorMode === 'rainbow' || isTemplateRainbow;
                    const forceRainbow = this.matrixRainbowMode || (isGlobalRainbow && !this.matrixColorCustomized);
