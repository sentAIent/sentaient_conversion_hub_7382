class MissionManager {
    ...
    getTrainingLessons() {
        return [
            ...
            },
            {
                ...
                reward: { gold: 2700, silver: 1800, bronze: 1200 }
            },
            {
                id: 'boost',
                name: 'Boost Control',
                ...
                reward: { gold: 3300, silver: 2250, bronze: 1500 }
            },
            {
                id: 'precision',
                ...
                reward: { gold: 4500, silver: 3000, bronze: 1800 }
            },
            {
                id: 'collection',
                name: 'Gem Collection',
                ...
                reward: { gold: 6000, silver: 4000, bronze: 2500 }
            },
            {
                id: 'radar',
                ...
                reward: { gold: 7500, silver: 5250, bronze: 3000 }
            },
            {
                id: 'final',
                name: 'Final Exam',
                ...
                reward: { gold: 12000, silver: 7500, bronze: 4000 }
            },
            {
                id: 'weaponry',
                name: 'Weapon Systems',
                ...
                showArrow: true,
                medals: { gold: 15, silver: 25, bronze: 40 },
                reward: { gold: 12000, silver: 8000, bronze: 4000, premiumGems: { type: 'sapphire', count: 10 } }
            },
            {
                id: 'shielding',
                ...
                reward: { gold: 18000, silver: 12000, bronze: 7000, premiumGems: { type: 'emerald', count: 10 } }
            },
            {
                id: 'hazards',
                ...
                reward: { gold: 25000, silver: 15000, bronze: 10000, premiumGems: { type: 'diamond', count: 5 } }
            },
            {
                id: 'warp',
                name: 'Warp Navigation',
                icon: '🌌',
                subtitle: 'Inter-sector travel mechanics',
                briefing: 'Final Lesson: Warp Gates allow travel between sectors.\nFly through the warp gate core to initiate a jump.\nMaintain course during the tunnel trip for a safe arrival.',
                keys: [{ key: 'W', action: 'Enter Gate' }],
                gates: (() => {
                    return [
                        { x: 1500, y: 0, size: 400, reached: false, isWarpGate: true }
                    ];
                })(),
                showArrow: true,
                medals: { gold: 10, silver: 20, bronze: 30 },
                reward: { gold: 50000, silver: 30000, bronze: 20000, premiumGems: { type: 'void_shard', count: 1 } }
            }
        ];
    }
    ...
    updateTraining() {
        ...
        if (lesson.id === 'weaponry' && lesson.targets) {
            this.game.projectiles.forEach(bullet => {
        ...
        if (lesson.id === 'collection') {
            ...
            if (gate) {
                ...
                if (lesson.id === 'weaponry' && !gate.targetDestroyed) {
                    ...
                    const dist = Math.hypot(dx, dy);
                    if (dist < gate.size * 1.4) { // Slightly more generous collision for training
                        gate.reached = true;
                        
                        // SPECIAL: Warp Navigation Trigger
                        if (gate.isWarpGate) {
                            this.game.hudManager.showToast('🌌 Warp Drive Activated!');
    ...
    checkMissionComplete() {
        ...
        switch (mission.type) {
            case 'survive':
                ...
                if (elapsed >= (mission.goal || 60)) isComplete = true;
                break;
            case 'collect':
                if (mission.progress >= (mission.goal || 10)) isComplete = true;
                break;
            case 'kill':
                if (mission.progress >= (mission.goal || 5)) isComplete = true;
                break;
            case 'boss':
                if (mission.bossDefeated) isComplete = true;
                break;
        }
    ...
    completeMission() {
        ...
        console.log(`🏆 [Missions] Mission Complete: ${mission.name}`);
        
        // Award Credits
        const reward = mission.reward || 1000;
        this.game.addCredits(reward);
        
        // Award Gems if specified
        if (mission.premiumReward) {
            const type = mission.premiumReward.type;
            const count = mission.premiumReward.count || 1;
            if (!this.game.playerInventory[type]) this.game.playerInventory[type] = 0;
            this.game.playerInventory[type] += count;
            this.game.saveInventory();
        }

        // Show Overlay
        this.showMissionCompleteOverlay(mission);

        // Stats
        this.game.missionsCompleted = (this.game.missionsCompleted || 0) + 1;
    ...
    updateMap() {
        ...
        canvases.forEach(canvas => {
            ...
            const offsetX = -(this.game.playerShip?.x || 0);
            const offsetY = -(this.game.playerShip?.y || 0);
            ...
            ctx.globalAlpha = 0.5;
            const minerals = this.game.minerals || [];
            ...
            const deposits = this.game.resourceDeposits || [];
    ...
    updateRadar() {
        ...
        if (this.game.activeMission) {
            ...
            // Phase 10: Specific Waypoint Rendering
            if (this.game.missionWaypoint) {
                const wp = this.game.missionWaypoint;
                const dx = wp.x - this.game.playerShip.x;
                const dy = wp.y - this.game.playerShip.y;
                const dist = Math.sqrt(dx * dx + dy * dy);
                
                const effectiveRange = 10000;
                const rx = cx + (dx / effectiveRange) * 35;
                const ry = cy + (dy / effectiveRange) * 35;
