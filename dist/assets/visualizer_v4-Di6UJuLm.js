...
`,ot=u=>u===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class st{constructor(e){this.group=e,this.meshes=[],this.activeClassId=22,this.activeVariationId=0,this.materials={};const t=new ke(new Uint8Array([0,0,0,0]),1,1,Ie);t.needsUpdate=!0,this.dummyTexture=t;const i=new ee(2,2);for(let a=1;a<=25;a++){let o=ot(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            `,depthWrite:!1,depthTest:!1,transparent:!0}),this.mesh.material=this.material,this.scene=new _e,this.scene.add(this.mesh)}resize(e,t){this.renderTarget.setSize(e,t),this.material.uniforms.uResolution.value.set(e,t)}render(e,t,i,a){this.renderer.setRenderTarget(this.renderTarget),this.renderer.clear(),this.renderer.render(e,t),this.material.uniforms.uTime.value+=i;const s=Math.max(0,a-.7)*3;this.material.uniforms.uAberration.value=5*s,this.material.uniforms.uBloomIntensity.value=.5+a*1.5,this.renderer.setRenderTarget(null),this.renderer.clear(),this.renderer.render(this.scene,this.camera)}dispose(){this.renderTarget.dispose(),this.geometry.dispose(),this.material.dispose()}}class nt{constructor(e,t=5e3){this.scene=e,this.count=t,this.geometry=new B,this.positions=new Float32Array(this.count*3),this.velocities=new Float32Array(this.count*3),this.colors=new Float32Array(this.count*3),this.sizes=new Float32Array(this.count);const i=new _(3718648),a=new _(11032055);for(let h=0;h<this.count;h++){const m=h*3,l=20*Math.cbrt(Math.random()),g=Math.random()*2*Math.PI,c=Math.acos(2*Math.random()-1);this.positions[m]=l*Math.sin(c)*Math.cos(g),this.positions[m+1]=l*Math.sin(c)*Math.sin(g),this.positions[m+2]=l*Math.cos(c),this.velocities[m]=-this.positions[m+2]*.05+(Math.random()-.5),this.velocities[m+1]=(Math.random()-.5)*.5,this.velocities[m+2]=this.positions[m]*.05+(Math.random()-.5);const f=i.clone().lerp(a,Math.random());this.colors[m]=f.r,this.colors[m+1]=f.g,this.colors[m+2]=f.b,this.sizes[h]=Math.random()*2+.5}this.geometry.setAttribute("position",new D(this.positions,3)),this.geometry.setAttribute("color",new D(this.colors,3)),this.geometry.setAttribute("size",new D(this.sizes,1));const s=document.createElement("canvas");s.width=64,s.height=64;const o=s.getContext("2d"),r=o.createRadialGradient(32,32,0,32,32,32);r.addColorStop(0,"rgba(255,255,255,1)"),r.addColorStop(.2,"rgba(255,255,255,0.8)"),r.addColorStop(.6,"rgba(255,255,255,0.2)"),r.addColorStop(1,"rgba(0,0,0,0)"),o.fillStyle=r,o.fillRect(0,0,64,64);const n=new te(s);this.material=new O({uniforms:{uTime:{value:0},uAudioLevel:{value:0},tSprite:{value:n}},vertexShader:`
                uniform float uTime;
            ...
            `,r=new O({vertexShader:s,fragmentShader:o,uniforms:a,transparent:!0,blending:T,depthWrite:!1,side:$}),n=new le(2,64,64),h=new P(n,r);i.add(h);const m=new U({color:t.color,transparent:!0,opacity:.15,blending:T,depthWrite:!1,side:je}),l=new le(3,32,32);i.add(new P(l,m))}else{const a=new we(1.5,.12,8,64);i.add(new P(a,t));const s=8;for(let o=0;o<s;o++){const r=o/s*Math.PI*2,n=new Ce;n.moveTo(-.4,0),n.lineTo(.4,0),n.lineTo(0,2.7),n.lineTo(-.4,0);const h=new Me(n),m=new P(h,t);m.rotation.z=-r,m.position.set(Math.sin(r)*1.5,Math.cos(r)*1.5,0),i.add(m);const l=r+Math.PI/s,g=new Ce;g.moveTo(-.2,0),g.lineTo(.2,0),g.lineTo(0,1.7),g.lineTo(-.2,0);const c=new Me(g),f=new P(c,t);f.rotation.z=-l,f.position.set(Math.sin(l)*1.5,Math.cos(l)*1.5,0),i.add(f)}}this.galaxySunMesh.add(i),this.galaxySunMesh.position.set(0,0,.1),this.galaxyGroup.add(this.galaxySunMesh)}setGalaxySunStyle(e){e!=="sun"&&e!=="sun2"&&e!=="sun3"||(this.galaxySunStyle=e,this.galaxyGroup&&this.galaxyGroup.children.length>0&&this.createGalaxySun(e))}setGalaxySunRotation(e,t){e==="x"&&(this.sunRotationSpeedX=parseFloat(t)),e==="y"&&(this.sunRotationSpeedY=parseFloat(t)),e==="z"&&(this.sunRotationSpeedZ=parseFloat(t))}createStarTexture(){if(this.textures.star)return this.textures.star;const e=64,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e/2);return o.addColorStop(0,"rgba(255, 255, 255, 1.0)"),o.addColorStop(.1,"rgba(255, 255, 240, 0.8)"),o.addColorStop(.25,"rgba(255, 255, 200, 0.3)"),o.addColorStop(.5,"rgba(200, 200, 255, 0.1)"),o.addColorStop(1,"rgba(100, 100, 255, 0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.fillStyle="rgba(255, 255, 255, 0.6)",i.beginPath(),i.moveTo(0,s-1),i.lineTo(a,s-.5),i.lineTo(e,s-1),i.lineTo(e,s+1),i.lineTo(a,s+.5),i.lineTo(0,s+1),i.closePath(),i.fill(),i.beginPath(),i.moveTo(a-1,0),i.lineTo(a-.5,s),i.lineTo(a-1,e),i.lineTo(a+1,e),i.lineTo(a+.5,s),i.lineTo(a+1,0),i.closePath(),i.fill(),i.beginPath(),i.arc(a,s,2,0,Math.PI*2),i.fillStyle="rgba(255, 255, 255, 1.0)",i.fill(),this.textures.star=new te(t),this.textures.star}initMandala(){var s;this.mandalaRings=[];const e=[2450411,15357964,3900150,16347926,12573694];for(let o=0;o<5;o++){const r=1.2+o*.8,n=6+o*6,h=new Qe(r-.05,r+.05,n),m=new U({color:e[o],side:$,transparent:!0,opacity:.4-o*.05,blending:T}),l=new P(h,m);l.userData={speed:(.01+o*.005)*(o%2===0?1:-1),segments:n},this.mandalaRings.push(l),this.mandalaGroup.add(l)}const t=new Ze(.3,32),i=new U({color:16347926,transparent:!0,opacity:.6,blending:T});this.mandalaCenter=new P(t,i),this.mandalaGroup.add(this.mandalaCenter),this.mandalaGroup.visible=!1;const a=(s=this.customColors)==null?void 0:s.mandala;a&&(this.mandalaRings.forEach(o=>o.material.color.copy(a)),this.mandalaCenter.material.color.copy(a))}createSnowflakeTexture(){if(this.textures.snowflake)return this.textures.snowflake;const e=128,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e),i.strokeStyle="rgba(220,240,255,1.0)",i.lineCap="round";for(let o=0;o<6;o++){const r=o/6*Math.PI*2;i.save(),i.translate(a,s),i.rotate(r),i.lineWidth=2.5,i.beginPath(),i.moveTo(0,0),i.lineTo(0,-52),i.stroke();const n=[{d:18,len:14,angle:Math.PI/4},{d:32,len:20,angle:Math.PI/4},{d:46,len:10,angle:Math.PI/5}];i.lineWidth=1.5,n.forEach(({d:h,len:m,angle:l})=>{[1,-1].forEach(g=>{i.beginPath(),i.moveTo(0,-h),i.lineTo(g*m*Math.cos(Math.PI/2-l),-h-m*Math.sin(Math.PI/2-l)),i.stroke()})}),i.restore()}i.beginPath();for(let o=0;o<6;o++){const r=o/6*Math.PI*2-Math.PI/6,n=a+Math.cos(r)*4,h=s+Math.sin(r)*4;o===0?i.moveTo(n,h):i.lineTo(n,h)}return i.closePath(),i.fillStyle="rgba(255, 255, 255, 0.8)",i.fill(),this.textures.snowflake=new te(t),this.textures.snowflake}initSnowflake(){for(;this.snowflakeGroup.children.length>0;){const c=this.snowflakeGroup.children[0];this.snowflakeGroup.remove(c),c.geometry&&c.geometry.dispose(),c.material&&c.material.dispose()}this._snowData=null;const e=700,t=new Float32Array(e*3),i=new Float32Array(e),a=new Float32Array(e),s=new Float32Array(e),o=new Float32Array(e),r=new Float32Array(e),n=new Float32Array(e);for(let c=0;c<e;c++){const f=c*3;t[f]=(Math.random()-.5)*80,t[f+1]=(Math.random()-.5)*60,t[f+2]=-40+Math.random()*35;const v=(t[f+2]+40)/35;i[c]=1.5+v*8,a[c]=.2+v*.6,s[c]=Math.random()*Math.PI*2,o[c]=.015+Math.random()*.04+v*.03,r[c]=.01+Math.random()*.02,n[c]=.4+Math.random()*.8}const h=new B;h.setAttribute("position",new D(t,3)),h.setAttribute("aSize",new D(i,1)),h.setAttribute("aOpacity",new D(a,1));const m=this.createSnowflakeTexture(),l=new O({uniforms:{uTexture:{value:m},uColor:{value:this.customColor?this.customColor.clone():new _(10875883)},uIntensity:{value:0},uSizeMultiplier:{value:1},uGlowAmount:{value:.5}},vertexShader:`
            ...
            `,transparent:!0,blending:T,depthWrite:!1,side:$}),i=new ee(100,100),a=new P(i,t);a.position.z=-5,[{count:5,minSize:1,maxSize:1.5},{count:7,minSize:1.5,maxSize:2.2},{count:4,minSize:2.2,maxSize:3.5}].forEach(r=>{for(let n=0;n<r.count;n++){const h=r.minSize+Math.random()*(r.maxSize-r.minSize),m=["heating","rising","cooling","falling"],l=m[Math.floor(Math.random()*m.length)],g=-18+Math.random()*4,c=18+Math.random()*4;let f=0,v=.5;l==="heating"?(f=g,v=Math.random()*.5):l==="rising"?(f=g+Math.random()*(c-g),v=.8+Math.random()*.2):l==="cooling"?(f=c,v=1-Math.random()*.3):l==="falling"&&(f=c-Math.random()*(c-g),v=.2+Math.random()*.3),this.lavaBlobs.push({position:new Ke((Math.random()-.5)*12,f,(Math.random()-.5)*6),material:{uniforms:{uTemp:{value:0},uIntensity:{value:0},uTime:{value:0}}},userData:{baseSize:h,state:l,temperature:v,floatMin:g,floatMax:c,heatRate:.15+Math.random()*.3,coolRate:.15+Math.random()*.3,riseSpeed:(.04+Math.random()*.05)/(h*.5),fallSpeed:(.05+Math.random()*.05)/(h*.5),driftPhase:Math.random()*Math.PI*2,driftSpeed:.01+Math.random()*.02}})}}),this.lavaGroup.add(a),this.lavaGroup.visible=!1}initFireplace(){const i=new ee(100,80);this.fireMaterial=this.createFireShader(),this.fireMesh=new P(i,this.fireMaterial),this.fireMesh.position.set(0,0,-15),this.fireMesh.renderOrder=10,this.fireplaceGroup.add(this.fireMesh),this.fireLight=new Je(16737792,5,200),this.fireLight.position.set(0,0,30),this.fireplaceGroup.add(this.fireLight);const a=650,s=new B,o=[];for(let r=0;r<a;r++)o.push((Math.random()-.5)*100,-20+Math.random()*40,-15+(Math.random()-.5)*20);s.setAttribute("position",new j(o,3)),this.emberMat=new he({color:16763904,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.8,blending:T,depthWrite:!1}),this.embers=new H(s,this.emberMat),this.fireplaceGroup.add(this.embers),this.emberVelocities=new Float32Array(a);for(let r=0;r<a;r++)this.emberVelocities[r]=.02+Math.random()*.05;this.fireplaceGroup.position.set(0,0,0),this.fireplaceGroup.visible=!1}createFireShader(){return new O({uniforms:{uTime:{value:0},uColor:{value:new _(16729088)},uSpeed:{value:1},uIntensity:{value:1}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            ...
            `,transparent:!0,side:$,blending:T,depthWrite:!1})}initRainforest(){const e=this.batterySaver?400:1e3,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*40,i[o+2]=(Math.random()-.5)*40,a[o]=Math.random(),a[o+1]=Math.random(),a[o+2]=.08+Math.random()*.12}t.setAttribute("position",new D(i,3)),t.setAttribute("aRandom",new D(a,3)),this.rainMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new _(8965375)},uIntensity:{value:.6}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),this.raindrops=new H(t,this.rainMaterial),this.rainforestGroup.add(this.raindrops),this.rainforestGroup.visible=!1}initZenGarden(){const e=this.batterySaver?200:500,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let o=0;o<e;o++){const r=o*3;i[r]=(Math.random()-.5)*40,i[r+1]=(Math.random()-.5)*20,i[r+2]=(Math.random()-.5)*40,a[r]=Math.random(),a[r+1]=Math.random(),a[r+2]=Math.random()*Math.PI*2}t.setAttribute("position",new D(i,3)),t.setAttribute("aRandom",new D(a,3)),this.petalMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new _(16758725)},uIntensity:{value:.8}},vertexShader:`
                attribute vec3 aRandom;
                uniform float uTime, uSpeed;
                varying float vAlpha;
                void main() {
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new H(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new ee(40,40,32,32);this.zenWaterMaterial=new U({color:2245734,transparent:!0,opacity:.3,side:$}),this.zenWater=new P(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new ee(300,100,128,64);this.oceanWave=new P(e,new O({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new _(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
            ...
            `,wireframe:!0,transparent:!0,side:$})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new B,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new j(a,3));const s=new he({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new H(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let h="🪷MINDWAVE";const m=e.logicMode,l=e.customText;m==="custom"||m==="txt"?h="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(m==="random"||m==="rnd"||m==="matrix"||m==="int"||m==="interstellar")&&(h="");const g=m==="matrix"||m==="int"||m==="interstellar"?[]:["LOGO",...h],c=g.length;for(let v=0;v<64;v++){const G=v%8,q=Math.floor(v/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(G*r,q*n,r,n),a.save(),a.translate(G*r+r/2,q*n+n/2);let A="",C=!1;if(v<c){const I=g[v];I==="LOGO"?C=!0:A=I}else{const w="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";A=w.charAt(Math.floor(Math.random()*w.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(A,0,0),a.restore(),A="")}if(A||C)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",C)if(this.logoImage){const k=document.createElement("canvas");k.width=100,k.height=100;const R=k.getContext("2d");R.imageSmoothingEnabled=!0,R.imageSmoothingQuality="high",R.drawImage(this.logoImage,0,0,100,100);const N=R.getImageData(0,0,100,100),F=N.data;for(let L=0;L<F.length;L+=4){const K=180+(F[L]+F[L+1]+F[L+2])/3/255*75;F[L]=K,F[L+1]=K,F[L+2]=K}R.putImageData(N,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(k,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new et().load("./mindwave-logo-icon.png",w=>{if(this.logoImage=w,this.logoLoading=!1,this.cyberMaterial){const k=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=k}},void 0,w=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(A,0,0);a.restore()}const f=new te(i);return f.magFilter=Se,f.minFilter=Se,f}createCyberShader(e,t=this.matrixConfig){return new O({uniforms:{uTexture:{value:e},uColor:{value:new _(t.color||65345)},uHeadColor:{value:new _(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
