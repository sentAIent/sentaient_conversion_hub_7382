import { state, els, THEMES, SOUNDSCAPES, PRESET_COMBOS, STATE_VISUALS } from '../state.js';
...
export async function applyPreset(type, btnElement, autoStart = true) {
    ...
    // Apply default visual mode for this brainwave state if defined
    if (STATE_VISUALS && STATE_VISUALS[type]) {
        setVisualMode(STATE_VISUALS[type]);
    }
...
export async function applyComboPreset(comboId, btnElement) {
    ...
    // 5. Update visualizer color and mode
    if (combo.color) {
        const viz = getVisualizer();
