class GameLoopManager {
    ...
    animate(time) {
        ...
        if (game.flightMode) {
            try {
                ...
                // --- CAMERA SMOOTHING & ZOOM LERPING (Phase 2 Upgrade) ---
                if (game.camera.targetZoom === undefined) game.camera.targetZoom = game.camera.zoom;
                
                // Zoom Lerp: Faster in standard, slower in cinematic
                const zoomFactor = game.cinematicMode ? 0.03 : 0.15;
                game.camera.zoom += (game.camera.targetZoom - game.camera.zoom) * zoomFactor;

                // Position Lerp: Center on ship
    ...
    updateBackgroundParallax() {
        ...
        if (game.bgWarpMode || game.warpDisengaging) {
            ...
            const rampSpeed = 0.02;
            game.warpSpeed = game.warpSpeed || 0;
            game.warpSpeed += (targetWarp - game.warpSpeed) * rampSpeed;
            game.warpSpeed = Math.min(game.warpSpeed, 25.2);
            if (game.warpSpeed < 0.01) game.warpSpeed = 0;
        }

        const warpIntensity = game.warpIntensity || 0;
