...
`,S=i=>i===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class P{constructor(e){this.group=e,this.meshes=[],this.activeClassId=17,this.activeVariationId=9,this.materials={};const a=new b(new Uint8Array([0,0,0,0]),1,1,C);a.needsUpdate=!0,this.dummyTexture=a;const n=new w(2,2);for(let t=1;t<=24;t++){let o=S(t);if(t<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        uniform float uAudioHigh;`),o=o.replace("float intMod = max(0.1, uIntensity);","float intMod = max(0.1, uIntensity * (1.0 + uAudioLow * 3.0 + uAudioMid * 1.5));")),!o.includes("uniform float uMouseSync;")){let u="";/uniform\s+vec2\s+uMouse\s*;/.test(o)||(u+=`uniform vec2 uMouse;
        `),u+=`uniform float uMouseSync;
        void main() {`,o=o.replace("void main() {",u)}t<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
