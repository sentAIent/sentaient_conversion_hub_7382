...
export class Visualizer3D {
    ...
    initSnowflake() {
        // Clear group
        if (!this.snowflakeGroup) this.snowflakeGroup = new THREE.Group();
    ...
    initCymatics() {
        console.log('[Visualizer] initCymatics CALLED - Initializing Fractal Resonance Engine...');
        ...
        // Clean any leftover glow or layers
        ...
        // Using 128x128 for smoothness, but with simple math to prevent crash
        ...
        this.cymaticMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uMedium: { value: state.cymaticMedium || 0.0 }, // 0:Water, 1:Sand, 2:Ether, 3:Ice
                uColor: { value: new THREE.Color(0x60a9ff) },
                uResolution: { value: new THREE.Vector2(window.innerWidth, window.innerHeight) }
            },
            vertexShader: `
                varying vec2 vUv;
                varying float vDisplace;
                uniform float uTime, uIntensity, uN, uM;
                ...
                void main() {
                    ...
                    // Chladni Plate Nodal Math
                    float f = cos(uN * PI * p.x) * cos(uM * PI * p.y) - cos(uM * PI * p.x) * cos(uN * PI * p.y);
                    float displace = abs(f) * uIntensity * 2.0;
                    vDisplace = displace;

                    vec3 pos = position;
                    pos.z += displace * (1.0 + 0.2 * sin(uTime * 2.0));
            ...
            fragmentShader: `
                varying vec2 vUv;
                varying float vDisplace;
                uniform float uTime, uIntensity, uBeatFreq, uMedium, uN, uM;
                uniform vec3 uColor;
                ...
                void main() {
                    ...
                    // Domain Warping / Fluid Flow
                    float t = uTime * 0.5;
                    float n1 = hash(uv + t * 0.1);
                    vec2 p = uv + (0.1 + uIntensity * 0.1) * vec2(cos(uTime + uv.y), sin(uTime + uv.x));

                    // Nodal Calculation
                    float val = cos(uN * PI * p.x) * cos(uM * PI * p.y) - cos(uM * PI * p.x) * cos(uN * PI * p.y);
                    val = abs(val);

                    vec3 col = uColor;
                    
                    // --- Medium Specific Aesthetics ---
                    if (uMedium < 0.5) { 
                        // WATER: Smooth liquid ripples
                        float ripple = sin(val * 10.0 - uTime * 4.0) * 0.5 + 0.5;
                        col = mix(uColor * 0.2, uColor * 1.5, ripple);
                    } else if (uMedium < 1.5) {
                        // SAND: Granular particles on nodal lines
                        float particles = step(0.8, hash(uv * 1000.0) + (1.0 - val));
                        col = mix(vec3(0.01), uColor * 2.0, particles);
                    } else if (uMedium < 2.5) {
                        // ETHER: Glowing plasma clouds
                        float plasma = pow(val, 2.0) * (1.0 + uIntensity);
                        col = uColor * plasma + (0.1 / length(uv));
                    } else {
                        // ICE: Sharp crystalline structures
                        float crystal = step(0.4, val + hash(uv * 50.0) * 0.1);
                        col = mix(uColor * 0.1, vec3(0.9, 0.95, 1.0), crystal);
                        col += vDisplace * 0.3;
                    }

                    gl_FragColor = vec4(col * (1.0 + uIntensity * 0.5), 0.9);
                }
            `,
            transparent: true,
            blending: THREE.AdditiveBlending,
        ...
        this.cymaticMesh.position.z = -5;
        this.scene.add(this.cymaticMesh);
        
        // Add a soft backlight for depth
        ...
        console.log('[Visualizer] Cymatics Engine v2: Precision Plate Dynamics deployed.');
    }

    nextCymatic() {
        const patterns = [
            { name: "Fundamental Zenith", n: 1, m: 1 },
            { name: "Dual Horizon", n: 1, m: 2 },
            { name: "Triple Axis", n: 1, m: 3 },
            { name: "Quad Core", n: 1, m: 4 },
            { name: "Square Harmony", n: 2, m: 2 },
            { name: "Lotus Flow", n: 2, m: 3 },
            { name: "Cresent Node", n: 2, m: 4 },
            { name: "Cross Pulse", n: 3, m: 3 },
            { name: "Solar Mandala", n: 7, m: 3 },
            { name: "Hexa Flux", n: 6, m: 6 }
        ];
    ...
    applyCymatic(data) {
        ...
        console.log(`[Cymatics] Pattern: ${data.name} (n:${data.n}, m:${data.m})`);
    }

    setCymaticTimer(seconds) {
        this.cymaticsTimer = seconds;
        console.log(`[Cymatics] Auto-rotation set to ${seconds}s`);
    }
    ...
    updateUIPanels() {
        ...
        if (this.activeModes.has('cymatics') || this.activeModes.has('snowflake')) document.getElementById('cymaticsPanel')?.classList.remove('hidden');
    }
    ...
    updateLabel(mode) {
        ...
        if (label) {
            ...
            else if (mode === 'cymatics') label.textContent = "GEOMETRIC RESONANCE";
    ...
    render(analyserL, analyserR) {
        try {
            ...
            if (this.activeModes.has('snowflake') && this._snowData) {
                ...
                const posAttr = sd.points.geometry.attributes.position;
                const baselineSpeed = 1.0 * multiplier;
                
                for (let i = 0; i < sd.count; i++) {
                    const i3 = i * 3;
                    // Fall down
                    sd.positions[i3 + 1] -= sd.speeds[i] * baselineSpeed;
                    
                    // Sideways drift (sinusoidal)
                    const drift = Math.sin(now * sd.driftFreqs[i]) * sd.drifts[i] * baselineSpeed;
                    sd.positions[i3] += drift;
                    
                    // Wrap around
                    if (sd.positions[i3 + 1] < -22) {
                        sd.positions[i3 + 1] = 22;
                        sd.positions[i3] = (Math.random() - 0.5) * 70;
                    }
                    if (sd.positions[i3] > 35) sd.positions[i3] = -35;
                    if (sd.positions[i3] < -35) sd.positions[i3] = 35;
                    
                    posAttr.setXY(i, sd.positions[i3], sd.positions[i3 + 1]);
                }
                posAttr.needsUpdate = true;
                
                // Audio reactivity
                if (sd.material) {
                    sd.material.uniforms.uIntensity.value = Math.max(vNormBass, vNormMids);
                }
            }

            // CYMATICS (Fractal Resonance Engine)
            if (this.activeModes.has('cymatics') && this.cymaticMaterial) {
                // Uniform Updates
                this.cymaticMaterial.uniforms.uTime.value = now;
                this.cymaticMaterial.uniforms.uIntensity.value = vNormBass * 2.5; 
                this.cymaticMaterial.uniforms.uBeatFreq.value = state.beatFrequency || 10.0;
                
                // Rotation (Direct Mesh)
                if (this.cymaticMesh) {
                    this.cymaticMesh.visible = true;
                    this.cymaticMesh.rotation.z += 0.002 * multiplier;
                }
    ...
    dispose() {
        ...
        const groups = [
            ...
            this.rainforestGroup, this.zenGardenGroup, this.oceanGroup,
            this.cymaticsGroup, this.snowflakeGroup, this.wavesGroup
        ];
