import{N as v}from"./share-DXATB9W1.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";const c={sun:{name:"Sun",icon:"☀️",create:t=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="32" height="32">
            <g>
        ...
        </svg>`},sun2:{name:"Sun 2",icon:"🌞",create:()=>null},default:{name:"Default",icon:"↖",create:()=>null}};let n="lotus",s=null,d=1;function k(){v.currentTheme=localStorage.getItem("mindwave_theme")||"default";const t=localStorage.getItem("mindwave_cursor_shape");t&&c[t]?n=t:(n="default",localStorage.setItem("mindwave_cursor_shape","default"));const e=localStorage.getItem("mindwave_cursor_color");e&&(s=e),l(),window.addEventListener("themeChanged",r=>{v.currentTheme=r.detail.name,l()})}function u(){return s||getComputedStyle(document.documentElement).getPropertyValue("--accent").trim()||"#60a9ff"}function h(){return getComputedStyle(document.documentElement).getPropertyValue("--accent-secondary").trim()||"#fb7185"}function l(){let t=document.getElementById("dynamic-cursor-styles");if(t||(t=document.createElement("style"),t.id="dynamic-cursor-styles",document.head.appendChild(t)),n==="default"){t.textContent="";return}const e=u(),r=h();if(n==="sun2"){t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./tribal-sun-cursor.png') 16 16, auto !important; }`;return}if(n==="mindwave"){t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./mindwave-cursor.png') 16 16, auto !important; }`;return}const a=c[n].create(e,r);if(!a){t.textContent="";return}let i=a;d!==1&&(i=i.replace("<svg ",`<svg opacity="${d}" `));const o=`url("data:image/svg+xml;utf8,${encodeURIComponent(i)}") 16 16, auto`;t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: ${o} !important; }`}function m(t){c[t]&&(n=t,localStorage.setItem("mindwave_cursor_shape",t),l(),document.querySelectorAll(".cursor-option").forEach(e=>{const r=e.dataset.shape===t;e.classList.toggle("active",r),r?(e.classList.add("bg-[var(--accent)]/20","border-[var(--accent)]"),e.classList.remove("bg-white/5","border-white/10")):(e.classList.remove("bg-[var(--accent)]/20","border-[var(--accent)]"),e.classList.add("bg-white/5","border-white/10"))}))}function y(t){d=t,l()}function p(t){s=t,t?localStorage.setItem("mindwave_cursor_color",t):localStorage.removeItem("mindwave_cursor_color"),l();const e=document.getElementById("cursorColorPicker");e&&t&&(e.value=t);const r=document.getElementById("cursorColorPreview");r&&(r.style.backgroundColor=u())}function f(){p(null)}function C(){const t=document.getElementById("themeGrid");if(!t)return;const e=document.getElementById("cursorSection");e&&e.remove();const r=document.createElement("div");r.id="cursorSection",r.className="mt-6 pt-6 border-t border-white/10",r.innerHTML=`
        ...
        <div class="grid grid-cols-3 sm:grid-cols-5 gap-2" id="cursorShapeGrid">
            ${Object.entries(c).map(([o,g])=>{const w={sun:"☀️",moon:"🌙",heart:"❤️",mindwave:'<img src="./mindwave-cursor.png" width="24" height="24" style="display:inline-block;">',sun2:"🌞",default:"🖱️"}[o]||"🖱️";return`<button class="cursor-option p-3 rounded-xl text-center transition-all border ${o===n?"active bg-[var(--accent)]/20 border-[var(--accent)]":"bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20"}" data-shape="${o}" title="${g.name}"><span class="text-2xl block">${w}</span><div class="text-[10px] mt-1 font-semibold" style="color: var(--text-main);">${g.name}</div></button>`}).join("")}
        </div>`,t.parentElement.appendChild(r),r.querySelectorAll(".cursor-option").forEach(o=>o.addEventListener("click",()=>m(o.dataset.shape)));const a=r.querySelector("#cursorColorPicker");a&&a.addEventListener("input",o=>p(o.target.value));const i=r.querySelector("#resetCursorColor");i&&i.addEventListener("click",()=>{f();const o=getComputedStyle(document.documentElement).getPropertyValue("--accent").trim();r.querySelector("#cursorColorPreview")&&(r.querySelector("#cursorColorPreview").style.backgroundColor=o),a&&(a.value=o)})}window.setCursorShape=m;window.setCursorColor=p;window.setCursorOpacity=y;window.resetCursorColor=f;window.createCursorUIInThemeModal=C;window._setCursorShape=m;export{C as createCursorUIInThemeModal,k as initCursor,f as resetCursorColor,p as setCursorColor,y as setCursorOpacity,m as setCursorShape};
//# sourceMappingURL=cursor-YVGMrtdS.js.map
