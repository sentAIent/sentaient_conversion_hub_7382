...
import { setupUI, applyAIIntent, showDisclaimerModal } from './ui/controls_v4.js';
...
const initApp = () => {
    ...
    if (loadingScreen) {
        ...
        const removeLoader = () => {
            ...
            if (entryGate) entryGate.style.display = 'none'; // Completely bypass entry gate
...
window.showFatalError = function(title, msg) {
    console.error(`[FATAL ERROR] ${title}: ${msg}`);
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.9);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);z-index:999999;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:2rem;text-align:center;color:#fff;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace;';
    overlay.innerHTML = `
        <div style="border:1px solid rgba(239,68,68,0.5);background:rgba(127,29,29,0.2);padding:2rem;border-radius:1rem;max-width:32rem;box-shadow:0 0 40px rgba(239,68,68,0.2);">
            ...
            <h1 style="font-size:1.5rem;font-weight:700;color:#f87171;margin-bottom:1rem;letter-spacing:0.05em;text-transform:uppercase;">${title}</h1>
            <p style="font-size:0.875rem;color:#fca5a5;line-height:1.6;margin-bottom:1.5rem;">${msg}</p>
            <button onclick="window.location.reload()" style="background:rgba(239,68,68,0.2);border:1px solid #ef4444;color:#f87171;padding:0.5rem 1.5rem;border-radius:0.5rem;font-weight:600;cursor:pointer;transition:all 0.2s;">REBOOT SEQUENCE</button>
        </div>
    `;
    document.body.appendChild(overlay);
};

        // Automatically initialize audio context on the user's FIRST click anywhere on the page
        document.body.addEventListener('click', async () => {
            console.log('[Main] First user interaction detected - Initializing AudioContext seamlessly');
            try {
                const { initAudio } = await import('./audio/engine.js');
                initAudio();
            } catch (e) {
                console.error('[Main] Audio engine seamless init failed:', e);
                window.showFatalError("AUDIO ENGINE FAILED", "The Web Audio API was blocked by the browser or failed to initialize.<br><br>Please check your site permissions and ensure autoplay is allowed, then click Reboot Sequence.");
            }
