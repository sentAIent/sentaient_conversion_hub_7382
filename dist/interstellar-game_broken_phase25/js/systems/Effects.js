class EffectManager {
    ...
    /**
     * Safely adds a color stop to a gradient, validating inputs.
     */
    safeAddColorStop(grad, offset, color) {
        ...
        if (safeColor.includes('undefined') || safeColor.includes('NaN')) {
            console.warn(`[Effects] Invalid color blocked: ${safeColor}`);
            safeColor = 'transparent';
        }
        try {
            grad.addColorStop(safeOffset, safeColor);
        } catch (e) {
            console.error(`[Effects] addColorStop failed:`, e);
        }
    }

    createExplosion(x, y, type = 'destruction', colorOverride = null) {
        const audio = window.gameAudio;
        const pm = this.game.particleManager;
        
        if (type === 'hit') {
            if (audio) audio.playExplosionSmall();
            if (pm) pm.burst(x, y, 8, 'SPARK', [2, 6]);
        } else if (type === 'collect') {
            if (pm) pm.burst(x, y, 15, 'PLASMA', [1, 4]);
        } else {
            if (audio) audio.playExplosionBig();
            if (pm) {
                pm.burst(x, y, 30, 'FIRE', [2, 12]);
                pm.burst(x, y, 20, 'SMOKE', [1, 5]);
                pm.burst(x, y, 5, 'BLOOM', [0.5, 2]);
            }
        }
    }

    /**
     * Creates a soft, expanding life-burst effect for Mindwave Lotus collection.
     */
    createLifeBurst(x, y) {
        const pm = this.game.particleManager;
        if (!pm) return;
        ...
        for (let i = 0; i < 20; i++) {
            pm.spawn({
                ...
                color: '#ffb6c1',
                colors: ['#ffb6c1', '#ffffff', 'transparent'],
                gravity: -0.05,
                bloom: true
            });
        }
    ...
    createSectorJumpEffect(onComplete) {
        const now = performance.now();
        const duration = 7500; // Increased to 7.5s for a more "Interstellar" cinematic duration
        ...
        // Pre-generate tunnel particles - High Density for Premium feel
        if (this.game.proceduralManager && this.game.proceduralManager.generateTunnelParticles) {
            this.game.hazardEffect.particles = this.game.proceduralManager.generateTunnelParticles(250);
        }

        console.log('🚀 [Effects] Engaging Warp Drive. Objective: Next Sector.');
        if (window.gameAudio && window.gameAudio.playWarpJump) {
            window.gameAudio.playWarpJump();
        }
    }

    updateSectorJumpEffect(progress) {
        ...
        // Phase Timing: Flash (0-10%) | Entrance (10-25%) | Tunnel (25-90%) | Flash Out (90-100%)
        if (progress < 0.10) {
            effect.phase = 'flash_in';
            effect.flashIntensity = progress / 0.10;
            effect.tunnelSpeed = 0;
            effect.shake = progress * 20;
        } else if (progress < 0.25) {
            effect.phase = 'entrance';
            effect.flashIntensity = 1.0 - ((progress - 0.10) / 0.15);
            effect.tunnelSpeed = (progress - 0.10) / 0.15 * 65;
            effect.warpStretch = 1.0 + effect.tunnelSpeed * 0.15;
            effect.shake = (1.0 - (progress - 0.10) / 0.15) * 20 + 5;
        } else if (progress < 0.90) {
            effect.phase = 'tunnel';
            effect.flashIntensity = 0;
            const tunnelP = (progress - 0.25) / 0.65;
            effect.tunnelSpeed = 65 + Math.sin(now * 0.008) * 15;
            effect.warpStretch = 8.0 + Math.sin(tunnelP * Math.PI) * 4.0; // Relativistic stretch pulse
            effect.hueShift = tunnelP * 720; // 2 full hue rotations
            effect.shake = 2 + Math.sin(now * 0.01) * 2;
        } else {
            effect.phase = 'flash_out';
            const outP = (progress - 0.90) / 0.10;
            effect.flashIntensity = outP;
            effect.tunnelSpeed = 65 * (1.0 - outP);
            effect.warpStretch = 8.0 * (1.0 - outP);
            effect.shake = outP * 30;
            
            if (outP > 0.85 && !effect.triggeredCallback) {
                effect.triggeredCallback = true;
    ...
    updateSupernovaEffect(progress) {
        ...
        switch (effect.phase) {
            ...
            case 'dispersal': // Cinematic Aftermath
                // Slower flash decay for a lasting afterglow
                effect.flashIntensity = Math.max(0.01, effect.flashIntensity * 0.95);
                effect.cameraShake = 60 + Math.sin(now * 0.006) * 40;
                effect.chromaticIntensity = 8.0 * (1 - (progress - 0.6) / 0.32);

                // Volumetric secondary eruptions
                if (Math.random() > 0.90 && effect.lightSpears.length < 150) {
                    const ang = Math.random() * Math.PI * 2;
                    effect.lightSpears.push({
                        angle: ang,
                        len: 500 + Math.random() * 1200,
                        life: 1.0,
                        v: 60 + Math.random() * 80,
                        isBranch: true,
                        parentAngle: ang + (Math.random() - 0.5) * 0.8
                    });
                }
                break;
            ...
            case 'reset': // Final fade out
                ...
                effect.flashIntensity = Math.max(0, effect.flashIntensity - 0.12);
                effect.cameraShake = (1 - resetP) * 20;
                effect.lensPulse = 1.0; 
                effect.gravitationalWarp = 0;
                effect.chromaticIntensity = 0;
                break;
        }
        ...
        // Filter dead objects - Optimized for memory stability
        if (this.game.frameCounter % 5 === 0) { // Regular intervals
            effect.particles = effect.particles.filter(p => p.life > 0.05);
            if (effect.lightSpears.length > 150) effect.lightSpears.splice(0, effect.lightSpears.length - 150);
            effect.lightSpears = effect.lightSpears.filter(s => s.life > 0.1);
    ...
    renderPlanetImpactEffect(ctx, time) {
        ...
        // ============================================
        if (progress < 0.1) {
            const rawAlpha = 1.0 - (progress / 0.1);
            const whiteoutAlpha = isFinite(rawAlpha) ? Math.max(0, Math.min(1, rawAlpha)) : 0;
        ...
        // ============================================
        if (progress < 0.25) {
            const rawAlpha = 1.0 - (progress / 0.25);
            const flashAlpha = isFinite(rawAlpha) ? Math.max(0, Math.min(1, rawAlpha)) : 0;
            ...
            this.safeAddColorStop(flashGrad, 0, `rgba(255, 255, 255, ${flashAlpha})`);
            this.safeAddColorStop(flashGrad, 0.2, `rgba(255, 250, 150, ${flashAlpha * 0.9})`);
            this.safeAddColorStop(flashGrad, 0.5, `rgba(255, 100, 0, ${flashAlpha * 0.6})`);
            this.safeAddColorStop(flashGrad, 1, 'transparent');
        ...
        if (progress > 0.05 && progress < 0.8) {
            ...
            this.safeAddColorStop(cloudGrad, 0, `rgba(255, 240, 220, 0)`);
            this.safeAddColorStop(cloudGrad, 0.8, `rgba(200, 180, 160, ${shockAlpha * 0.5})`);
            this.safeAddColorStop(cloudGrad, 1, `rgba(150, 130, 110, 0)`);
    ...
    renderMissileBaseDestructionEffect(ctx, time) {
        ...
        if (progress < 0.15) {
            const rawAlpha = 1.0 - (progress / 0.15);
            const whiteAlpha = isFinite(rawAlpha) ? Math.max(0, Math.min(1, rawAlpha)) : 0;
        ...
        if (progress < 0.6) {
            ...
            this.safeAddColorStop(grad, 0, `rgba(255, 255, 255, ${alpha})`);
            this.safeAddColorStop(grad, 0.2, `rgba(200, 255, 100, ${alpha * 0.9})`); // Radioactive Yellow
            this.safeAddColorStop(grad, 0.5, `rgba(50, 255, 50, ${alpha * 0.7})`); // Toxic Green
    ...
    renderEMPEffect(ctx, time) {
        ...
        this.safeAddColorStop(grad, 0, `rgba(0, 243, 255, 0)`);
        this.safeAddColorStop(grad, 0.8, `rgba(0, 243, 255, ${alpha * 0.5})`);
        this.safeAddColorStop(grad, 1, `rgba(255, 255, 255, ${alpha})`);
        ctx.fillStyle = grad;
        ctx.fill();
    ...
    renderPlayerDeathEffect(ctx, time) {
        ...
        if (effect.phase === 'venting' || effect.phase === 'critical') {
            effect.ventingPoints.forEach(p => {
                ...
                this.safeAddColorStop(grad, 0, `rgba(255, 200, 50, ${alpha})`);
                this.safeAddColorStop(grad, 0.5, `rgba(255, 100, 0, ${alpha * 0.5})`);
                this.safeAddColorStop(grad, 1, 'transparent');
        ...
        if (effect.phase === 'explosion') {
            ...
            this.safeAddColorStop(grad, 0, '#fff');
            this.safeAddColorStop(grad, 0.2, '#ff0');
            this.safeAddColorStop(grad, 0.5, '#f60');
            this.safeAddColorStop(grad, 1, 'transparent');
    ...
    renderMissileHitEffect(ctx, time) {
        ...
        if (progress < 0.6) {
            ...
            this.safeAddColorStop(grad, 0, `rgba(255, 255, 255, ${1 - progress})`);
            this.safeAddColorStop(grad, 0.3, `rgba(255, 100, 0, ${0.5 * (1 - progress)})`);
            this.safeAddColorStop(grad, 1, 'transparent');
            ctx.fillStyle = grad;
    ...
    updateDamageEffects(dt = 1.0) {
        if (this.game.damageParticles.length === 0) return;

        for (let i = this.game.damageParticles.length - 1; i >= 0; i--) {
            const p = this.game.damageParticles[i];
            
            // Physics
            p.x += p.vx * dt;
            p.y += p.vy * dt;
            if (p.gravity) p.y += p.gravity * dt;
            
            // Life Decay
            p.life -= dt;
            
            // Expansion (for aura particles)
            if (p.type === 'aura') {
                p.size += 0.5 * dt;
                p.alpha *= 0.98;
            }
            ...
            // Remove dead
            if (p.life <= 0) {
                this.game.damageParticles.splice(i, 1);
            }
        }
    }
    ...
    renderDamageEffects(ctx) {
        ...
        this.game.damageParticles.forEach(p => {
            ...
            const finalAlpha = (p.alpha || 0.6) * alpha;
            ctx.globalAlpha = Math.max(0, Math.min(1, finalAlpha));

            if (p.type === 'aura') {
                // Premium Radial Glow
                const grad = ctx.createRadialGradient(0, 0, 0, 0, 0, p.size);
                this.safeAddColorStop(grad, 0, p.color);
                this.safeAddColorStop(grad, 0.4, p.color);
                this.safeAddColorStop(grad, 1, 'transparent');
                ctx.fillStyle = grad;
                
                // Add soft additive bloom
            ...
            ctx.beginPath();
            ctx.arc(0, 0, p.size / (this.game.camera.zoom || 1), 0, Math.PI * 2);
            ctx.fill();
