class GameLoopManager {
    ...
    start() {
        ...
        this.isRunning = true;
        console.log("🎮 [GameLoop] requestAnimationFrame CALLED.");
    ...
    animate(time) {
        ...
        const rawDt = this._lastTime ? (now - this._lastTime) / 16.666 : 1.0;
        this._lastTime = now;

        // Standardized dt (1.0 = 60fps). Clamp to prevent extreme jumps.
        const dt = Math.min(2.0, Math.max(0.1, rawDt)) * (game.timeScale || 1.0);

        if (game.frameCounter < 10) {
            console.log(`[GameLoop] Frame ${game.frameCounter} | Mode: ${game.mode} | Flight: ${game.flightMode}`);
        }

        if (game.frameCounter % 60 === 0) {
            console.log(`[GameLoop] Heartbeat. Mode: ${game.mode}, Flight: ${game.flightMode}, Ship: ${!!game.playerShip}`);
        }

        if (game.flightMode) {
            try {
                // Safety Check: Avoid crashes if critical objects are missing
                if (!game.playerShip || !game.camera) {
                    if (game.frameCounter % 60 === 0) console.warn(`⚠️ [GameLoop] Physics ABORTED: ship=${!!game.playerShip}, camera=${!!game.camera}`);
                    return;
                }
                ...
                if (game.playerManager && game.playerManager.updatePlayerShip) {
                    ...
                    if (game.frameCounter % 60 === 0) console.warn(`⚠️ [GameLoop] PlayerManager MISSING: mgr=${!!game.playerManager}`);
                }
        ...
        if (game.camera) {
            ...
            if (game.flightMode && game.playerShip) {
                // Ensure finite ship coordinates
                ...
                // Ensure finite camera coordinates before lerping
                if (!isFinite(game.camera.x)) game.camera.x = targetCamX;
                if (!isFinite(game.camera.y)) game.camera.y = targetCamY;
        ...
        // Phase 28: Procedural Dynamics (Shooting Stars Spawning)
        if (game.proceduralManager) game.proceduralManager.update(dt);
    ...
    updateBackgroundObjects() {
        ...
        const half = range / 2;
        const px = game.playerShip ? game.playerShip.x : 0;
        const py = game.playerShip ? game.playerShip.y : 0;
        ...
        if (spacecraft && spacecraft.length > 0) {
            for (let i = 0; i < spacecraft.length; i++) {
                ...
                s.x += (s.vx || 0) * dt;
                s.y += (s.vy || 0) * dt;
                if (s.x > px + half) s.x -= range; 
                else if (s.x < px - half) s.x += range;
                if (s.y > py + half) s.y -= range; 
                else if (s.y < py - half) s.y += range;
            }
        }
        ...
        if (shootingStars && shootingStars.length > 0) {
            for (let i = shootingStars.length - 1; i >= 0; i--) {
                ...
                if (s.x > px + half) s.x -= range; else if (s.x < px - half) s.x += range;
                if (s.y > py + half) s.y -= range; else if (s.y < py - half) s.y += range;
            }
        }

        // --- PHASE 30: Procedural Dynamics (Shooting Stars Spawning) ---
        if (game.proceduralManager) game.proceduralManager.update(dt);
    }
    ...
    updateWarpWorldMotion() {
        ...
        // Iterate through loaded sectors for warp motion
        for (const sector of game.loadedSectors.values()) {
            if (sector.planets) sector.planets.forEach(p => this.flyObject(p, centerWorldX, centerWorldY, 0.8, 3000));
            if (sector.galaxies) sector.galaxies.forEach(g => this.flyObject(g, centerWorldX, centerWorldY, 0.5, 5000));
            if (sector.nebulae) sector.nebulae.forEach(n => this.flyObject(n, centerWorldX, centerWorldY, 0.6, 4000));
            if (sector.stars) sector.stars.forEach(s => this.flyObject(s, centerWorldX, centerWorldY, 0.9, 2500));
            
            if (sector.hazards && sector.hazards.blackHoles) {
                sector.hazards.blackHoles.forEach(bh => this.flyObject(bh, centerWorldX, centerWorldY, 0.4, 4000));
            }
        }
