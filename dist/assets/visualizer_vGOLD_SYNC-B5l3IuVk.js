...
`,je=u=>u===1?`
        ...
        void main() {
            ...
            float mouseForce = exp(-distToMouse * 4.0) * 0.3;
            
            // Audio react
            float lowPulse = uAudioLow * 0.2;
            float midPulse = uAudioMid * 0.15;
            float highPulse = uAudioHigh * 0.1;
            float audioGlobal = (uAudioLow + uAudioMid + uAudioHigh) * 0.1;
            
            vec2 uv = vUv;
            
            // Complex UV distortion based on variation and intensity
            float t = uTime * 0.5;
            float strength = uIntensity * 0.05 + audioGlobal;
            
#if VARIATION == 0
            // Asymmetric Fluid
            uv += vec2(sin(r*10.0 - t + mouseForce), cos(p.x*5.0 + t)) * strength;
            uv = fract(uv);
#elif VARIATION == 1
            // Neon Triangle
            p = rot(p, sin(t)*0.1 + mouseForce);
            uv = p * (0.5 + lowPulse) + 0.5;
            uv += vec2(sin(uv.y*20.0)*strength, 0.0);
#elif VARIATION == 2
            // Organic Spiral
            float theta = atan(p.y, p.x) + r * (2.0 + uIntensity*5.0) - t;
            p = vec2(cos(theta), sin(theta)) * r;
            uv = p * (0.5 + midPulse) + 0.5;
            uv += vec2(cos(t), sin(t)) * mouseForce;
#elif VARIATION == 3
            // Cyber Honeycomb
            uv.x += sin(uv.y * 50.0 + t*5.0) * strength * 0.5;
            uv.y += cos(uv.x * 50.0 - t*3.0) * strength * 0.5;
            uv += (uMouse - 0.5) * mouseForce * 2.0;
#endif

            // Ensure UV stays within bounds to prevent artifacting
            uv = clamp(uv, 0.0, 1.0);
            ...
            vec2 offset = vec2(1.0 / 1024.0, 1.0 / 1024.0);
            float n = texture2D(uTexture, uv + vec2(0.0, offset.y)).r;
            float s = texture2D(uTexture, uv - vec2(0.0, offset.y)).r;
            float e = texture2D(uTexture, uv + vec2(offset.x, 0.0)).r;
            float w = texture2D(uTexture, uv - vec2(offset.x, 0.0)).r;
            float edge = abs(n - s) + abs(e - w);
            
            // Color Mapping
            vec3 mappedColor = mix(uColor1, uColor2, luma + highPulse);
            vec3 finalColor = mix(texColor.rgb, mappedColor, 0.7 + uIntensity*0.3);
            
            // Boost edges with intensity
            finalColor += edge * uIntensity * uColor2 * 2.0;
            
            // Subtle pulse
            finalColor *= 1.0 + lowPulse + mouseForce;
        ...
        void main() {
            ...
            vec2 starUv = uv * 15.0 + warp * 10.0;
            
            // Background Layer (Layer 1)
            vec4 bgColor = texture2D(uTexture2, clamp(bgUv, 0.0, 1.0));
            
            // Animal Aura Layer (Layer 3 - Unwarped)
            vec4 auraTex = texture2D(uTexture, uv);
            float auraLuma = dot(auraTex.rgb, vec3(0.299, 0.587, 0.114));
            
            // Tint the aura
            vec3 tintedAura = auraTex.rgb * uColor1;
            
            // Procedural Voronoi Stars and Lines (Layer 2)
            vec3 v = voronoi(starUv);
            float border = 1.0 - smoothstep( 0.0, 0.03, v.x ); // Connecting lines
            float center = 1.0 - smoothstep( 0.0, 0.1, length(v.yz) ); // Stars
            
            if (uLayer2Type > 0.5 && uLayer2Type < 1.5) {
                border = 0.0; // Star Cluster (No lines)
            } else if (uLayer2Type > 1.5 && uLayer2Type < 2.5) {
                center = 0.0; // Deep Space Connections (Lines only)
            } else if (uLayer2Type > 2.5) {
                center = 1.0 - smoothstep( 0.0, 0.15, v.x ); // Quantum Nodes (Inverse cellular)
                border = 0.0;
            }
            
            // Mask stars/lines by the aura
            float starMask = smoothstep(0.05, 0.3, auraLuma);
            vec3 starColor = (border * uColor2 * 0.5 + center * vec3(1.0, 1.0, 1.0)) * starMask;
            
            // Combine with Additive Blending
            vec3 finalColor = bgColor.rgb + tintedAura + starColor + starColor * audioGlobal * 2.0;
            
            gl_FragColor = vec4(finalColor, max(bgColor.a, auraTex.a) * uAlpha);
        }
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class Ye{constructor(e){this.group=e,this.meshes=[],this.activeClassId=17,this.activeVariationId=9,this.materials={};const t=new Pe(new Uint8Array([0,0,0,0]),1,1,Ge);t.needsUpdate=!0,this.dummyTexture=t;const i=new j(2,2);for(let a=1;a<=24;a++){let o=je(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        ...
        void main() {`,o=o.replace("void main() {",d)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            `,transparent:!0,blending:T,depthWrite:!1}),this.particles=new k(t,this.particleMaterial),this.particleGroup.add(this.particles),this.particleGroup.visible=!1}initBox(){var m;this.boxOuter=new _;const e=new se(new re(3,3,3)),t=new $({color:16777215,transparent:!0,opacity:.9,blending:T}),i=new $({color:3900150,transparent:!0,opacity:.5,blending:T});this.boxOuter.add(new K(e,t));for(let l=1;l<=3;l++){const x=new K(e,i);x.scale.setScalar(1+l*.012),this.boxOuter.add(x)}const a=new se(new re(2,2,2)),s=new $({color:14742270,transparent:!0,opacity:.8,blending:T}),o=new $({color:6333946,transparent:!0,opacity:.4,blending:T});this.boxInner=new _,this.boxInner.add(new K(a,s));for(let l=1;l<=2;l++){const x=new K(a,o);x.scale.setScalar(1+l*.015),this.boxInner.add(x)}const r=new se(new re(3.05,3.05,3.05)),n=new $({color:9684477,transparent:!0,opacity:.8,blending:T});this.boxEdges=new K(r,n),this.boxGroup.add(this.boxOuter),this.boxGroup.add(this.boxInner),this.boxGroup.add(this.boxEdges),this.boxGroup.visible=!1;const d=((m=this.customColors)==null?void 0:m.box)||this.customColor;d&&(this.boxOuter.children.forEach(l=>l.material.color.copy(d)),this.boxInner.children.forEach(l=>l.material.color.copy(d)),this.boxEdges&&this.boxEdges.material&&this.boxEdges.material.color.copy(d))}initDragon(){var x;this.dragonDummy=new Le,this.dragonLength=80;const e=new ie(.8,1),t=new L({color:15680580,wireframe:!0,transparent:!0,opacity:.8,blending:T}),i=new ie(.5,1),a=new L({color:16096779,wireframe:!1,transparent:!0,opacity:.6,blending:T});this.dragonBodyInstanced=new de(e,t,this.dragonLength),this.dragonGlowInstanced=new de(i,a,this.dragonLength);const s=new ne(1.5,3.5,5);s.rotateX(Math.PI/2);const o=new L({color:16638023,wireframe:!0,transparent:!0,opacity:.9,blending:T});this.dragonHead=new P(s,o),this.dragonPearlGroup=new _;const r=new oe(1,16,16),n=new L({color:3718648,wireframe:!1,transparent:!0,opacity:.9,blending:T}),d=new oe(1.3,16,16),m=new L({color:8246268,wireframe:!0,transparent:!0,opacity:.5,blending:T});this.dragonPearl=new P(r,n),this.dragonPearlHalo=new P(d,m),this.dragonPearlGroup.add(this.dragonPearl),this.dragonPearlGroup.add(this.dragonPearlHalo),this.dragonGroup.add(this.dragonBodyInstanced),this.dragonGroup.add(this.dragonGlowInstanced),this.dragonGroup.add(this.dragonHead),this.dragonGroup.add(this.dragonPearlGroup),this.dragonGroup.visible=!1;const l=((x=this.customColors)==null?void 0:x.dragon)||this.customColor;l&&this.updateDragonColor(l)}updateGalaxyColor(e){if(this.galaxySunMesh&&this.galaxySunMesh.traverse(t=>{t.isMesh&&t.material&&t.material.color.copy(e)}),this.galaxyStars&&this.galaxyStars.geometry){const t={};e.getHSL(t);const i=(t.h+.5)%1,a=this.galaxyStars.geometry.getAttribute("color");if(a){const s=a.count,o=this._tempColor;for(let r=0;r<s;r++){const n=r/s,d=n<.2?.8:n<.5?.6:.4+Math.random()*.15,m=.6+Math.random()*.3;o.setHSL(i,m,d),a.setXYZ(r,o.r,o.g,o.b)}a.needsUpdate=!0}}}updateDragonColor(e){if(!this.dragonBodyInstanced)return;this.dragonBodyInstanced.material.color.copy(e),this.dragonHead.material.color.copy(e);const t={};e.getHSL(t);const i=(t.h+.5)%1,a=new G().setHSL(i,t.s,t.l);this.dragonGlowInstanced.material.color.copy(a)}initGalaxy(){const e=this.batterySaver?500:1500,t=new B,i=[],a=[],s=[];for(let n=0;n<e;n++){const d=n/e*Math.PI*10,m=2.5+n/e*20+Math.random()*2,l=n%4*(Math.PI*2/4),x=Math.max(.5,n/e*4),f=Math.cos(d+l)*m+(Math.random()-.5)*x,c=(Math.random()-.5)*1.5,v=Math.sin(d+l)*m+(Math.random()-.5)*x;i.push(f,c,v);const I=n/e;I<.2?a.push(1,.95,.7):I<.5?a.push(.7+Math.random()*.3,.8,1):a.push(.4+Math.random()*.2,.3+Math.random()*.3,.8+Math.random()*.2),s.push(Math.random()<.05?.4+Math.random()*.3:.08+Math.random()*.15)}t.setAttribute("position",new N(i,3)),t.setAttribute("color",new N(a,3)),t.setAttribute("size",new N(s,1));const o=this.createStarTexture(),r=new le({size:.25,vertexColors:!0,map:o,transparent:!0,opacity:.9,blending:T,depthWrite:!1,sizeAttenuation:!0});this.galaxyStars=new k(t,r),this.galaxyGroup.add(this.galaxyStars),this.galaxySunStyle=this.galaxySunStyle||"sun",this.createGalaxySun(this.galaxySunStyle),this.galaxyGroup.visible=!1}createGalaxySun(e){this.galaxySunMesh&&(this.galaxyGroup.remove(this.galaxySunMesh),this.galaxySunMesh=null),this.galaxySunMesh=new _;const t=new L({color:4892415,transparent:!0,opacity:.85,blending:T,depthWrite:!1,side:X}),i=new _;if(e==="sun2"){const a=t.clone();a.side=Re;const s=new pe(1.5,.25,16,64);i.add(new P(s,a));const o=8,r=new ne(.4,2.7,4);r.translate(0,2.7/2,0);const n=new ne(.2,1.7,4);n.translate(0,1.7/2,0);for(let d=0;d<o;d++){const m=d/o*Math.PI*2,l=new P(r,a);l.rotation.z=-m,l.position.set(Math.sin(m)*1.5,Math.cos(m)*1.5,0),i.add(l);const x=m+Math.PI/o,f=new P(n,a);f.rotation.z=-x,f.position.set(Math.sin(x)*1.5,Math.cos(x)*1.5,0),i.add(f)}}else if(e==="sun3"){const a={uTime:{value:0},uColor:{value:t.color},uBassIntt:{value:0}};this.galaxySunUniforms=a;const s=`
                varying vec2 vUv;
            ...
            `,r=new O({vertexShader:s,fragmentShader:o,uniforms:a,transparent:!0,blending:T,depthWrite:!1,side:X}),n=new oe(2,64,64),d=new P(n,r);i.add(d);const m=new L({color:t.color,transparent:!0,opacity:.15,blending:T,depthWrite:!1,side:Ve}),l=new oe(3,32,32);i.add(new P(l,m))}else{const a=new pe(1.5,.12,8,64);i.add(new P(a,t));const s=8;for(let o=0;o<s;o++){const r=o/s*Math.PI*2,n=new ve;n.moveTo(-.4,0),n.lineTo(.4,0),n.lineTo(0,2.7),n.lineTo(-.4,0);const d=new xe(n),m=new P(d,t);m.rotation.z=-r,m.position.set(Math.sin(r)*1.5,Math.cos(r)*1.5,0),i.add(m);const l=r+Math.PI/s,x=new ve;x.moveTo(-.2,0),x.lineTo(.2,0),x.lineTo(0,1.7),x.lineTo(-.2,0);const f=new xe(x),c=new P(f,t);c.rotation.z=-l,c.position.set(Math.sin(l)*1.5,Math.cos(l)*1.5,0),i.add(c)}}this.galaxySunMesh.add(i),this.galaxySunMesh.position.set(0,0,.1),this.galaxyGroup.add(this.galaxySunMesh)}setGalaxySunStyle(e){e!=="sun"&&e!=="sun2"&&e!=="sun3"||(this.galaxySunStyle=e,this.galaxyGroup&&this.galaxyGroup.children.length>0&&this.createGalaxySun(e))}setGalaxySunRotation(e,t){e==="x"&&(this.sunRotationSpeedX=parseFloat(t)),e==="y"&&(this.sunRotationSpeedY=parseFloat(t)),e==="z"&&(this.sunRotationSpeedZ=parseFloat(t))}createStarTexture(){if(this.textures.star)return this.textures.star;const e=64,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e/2);return o.addColorStop(0,"rgba(255, 255, 255, 1.0)"),o.addColorStop(.1,"rgba(255, 255, 240, 0.8)"),o.addColorStop(.25,"rgba(255, 255, 200, 0.3)"),o.addColorStop(.5,"rgba(200, 200, 255, 0.1)"),o.addColorStop(1,"rgba(100, 100, 255, 0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.fillStyle="rgba(255, 255, 255, 0.6)",i.beginPath(),i.moveTo(0,s-1),i.lineTo(a,s-.5),i.lineTo(e,s-1),i.lineTo(e,s+1),i.lineTo(a,s+.5),i.lineTo(0,s+1),i.closePath(),i.fill(),i.beginPath(),i.moveTo(a-1,0),i.lineTo(a-.5,s),i.lineTo(a-1,e),i.lineTo(a+1,e),i.lineTo(a+.5,s),i.lineTo(a+1,0),i.closePath(),i.fill(),i.beginPath(),i.arc(a,s,2,0,Math.PI*2),i.fillStyle="rgba(255, 255, 255, 1.0)",i.fill(),this.textures.star=new J(t),this.textures.star}initMandala(){var s;this.mandalaRings=[];const e=[2450411,15357964,3900150,16347926,12573694];for(let o=0;o<5;o++){const r=1.2+o*.8,n=6+o*6,d=new Oe(r-.05,r+.05,n),m=new L({color:e[o],side:X,transparent:!0,opacity:.4-o*.05,blending:T}),l=new P(d,m);l.userData={speed:(.01+o*.005)*(o%2===0?1:-1),segments:n},this.mandalaRings.push(l),this.mandalaGroup.add(l)}const t=new Ue(.3,32),i=new L({color:16347926,transparent:!0,opacity:.6,blending:T});this.mandalaCenter=new P(t,i),this.mandalaGroup.add(this.mandalaCenter),this.mandalaGroup.visible=!1;const a=(s=this.customColors)==null?void 0:s.mandala;a&&(this.mandalaRings.forEach(o=>o.material.color.copy(a)),this.mandalaCenter.material.color.copy(a))}createSnowflakeTexture(){if(this.textures.snowflake)return this.textures.snowflake;const e=128,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e*.5);o.addColorStop(0,"rgba(200,230,255,0.5)"),o.addColorStop(.4,"rgba(180,220,255,0.15)"),o.addColorStop(1,"rgba(150,200,255,0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.strokeStyle="rgba(220,240,255,1.0)",i.lineCap="round";for(let r=0;r<6;r++){const n=r/6*Math.PI*2;i.save(),i.translate(a,s),i.rotate(n),i.lineWidth=2.5,i.beginPath(),i.moveTo(0,0),i.lineTo(0,-52),i.stroke();const d=[{d:18,len:14,angle:Math.PI/4},{d:32,len:20,angle:Math.PI/4},{d:46,len:10,angle:Math.PI/5}];i.lineWidth=1.5,d.forEach(({d:m,len:l,angle:x})=>{[1,-1].forEach(f=>{i.beginPath(),i.moveTo(0,-m),i.lineTo(f*l*Math.cos(Math.PI/2-x),-m-l*Math.sin(Math.PI/2-x)),i.stroke()})}),i.restore()}i.beginPath();for(let r=0;r<6;r++){const n=r/6*Math.PI*2-Math.PI/6,d=a+Math.cos(n)*4,m=s+Math.sin(n)*4;r===0?i.moveTo(d,m):i.lineTo(d,m)}return i.closePath(),i.fillStyle="rgba(255, 255, 255, 0.8)",i.fill(),this.textures.snowflake=new J(t),this.textures.snowflake}initSnowflake(){for(;this.snowflakeGroup.children.length>0;){const f=this.snowflakeGroup.children[0];this.snowflakeGroup.remove(f),f.geometry&&f.geometry.dispose(),f.material&&f.material.dispose()}this._snowData=null;const e=700,t=new Float32Array(e*3),i=new Float32Array(e),a=new Float32Array(e),s=new Float32Array(e),o=new Float32Array(e),r=new Float32Array(e),n=new Float32Array(e);for(let f=0;f<e;f++){const c=f*3;t[c]=(Math.random()-.5)*80,t[c+1]=(Math.random()-.5)*60,t[c+2]=-40+Math.random()*35;const v=(t[c+2]+40)/35;i[f]=1.5+v*8,a[f]=.2+v*.6,s[f]=Math.random()*Math.PI*2,o[f]=.015+Math.random()*.04+v*.03,r[f]=.01+Math.random()*.02,n[f]=.4+Math.random()*.8}const d=new B;d.setAttribute("position",new E(t,3)),d.setAttribute("aSize",new E(i,1)),d.setAttribute("aOpacity",new E(a,1));const m=this.createSnowflakeTexture(),l=new O({uniforms:{uTexture:{value:m},uColor:{value:this.customColor?this.customColor.clone():new G(10875883)},uIntensity:{value:0},uSizeMultiplier:{value:1},uGlowAmount:{value:.5}},vertexShader:`
            ...
            `,transparent:!0,blending:T,depthWrite:!1,side:X}),i=new j(100,100),a=new P(i,t);a.position.z=-5,[{count:5,minSize:1,maxSize:1.5},{count:7,minSize:1.5,maxSize:2.2},{count:4,minSize:2.2,maxSize:3.5}].forEach(r=>{for(let n=0;n<r.count;n++){const d=r.minSize+Math.random()*(r.maxSize-r.minSize),m=["heating","rising","cooling","falling"],l=m[Math.floor(Math.random()*m.length)],x=-18+Math.random()*4,f=18+Math.random()*4;let c=0,v=.5;l==="heating"?(c=x,v=Math.random()*.5):l==="rising"?(c=x+Math.random()*(f-x),v=.8+Math.random()*.2):l==="cooling"?(c=f,v=1-Math.random()*.3):l==="falling"&&(c=f-Math.random()*(f-x),v=.2+Math.random()*.3),this.lavaBlobs.push({position:new Be((Math.random()-.5)*12,c,(Math.random()-.5)*6),material:{uniforms:{uTemp:{value:0},uIntensity:{value:0},uTime:{value:0}}},userData:{baseSize:d,state:l,temperature:v,floatMin:x,floatMax:f,heatRate:.15+Math.random()*.3,coolRate:.15+Math.random()*.3,riseSpeed:(.04+Math.random()*.05)/(d*.5),fallSpeed:(.05+Math.random()*.05)/(d*.5),driftPhase:Math.random()*Math.PI*2,driftSpeed:.01+Math.random()*.02}})}}),this.lavaGroup.add(a),this.lavaGroup.visible=!1}initFireplace(){const i=new j(100,80);this.fireMaterial=this.createFireShader(),this.fireMesh=new P(i,this.fireMaterial),this.fireMesh.position.set(0,0,-15),this.fireMesh.renderOrder=10,this.fireplaceGroup.add(this.fireMesh),this.fireLight=new ke(16737792,5,200),this.fireLight.position.set(0,0,30),this.fireplaceGroup.add(this.fireLight);const a=650,s=new B,o=[];for(let r=0;r<a;r++)o.push((Math.random()-.5)*100,-20+Math.random()*40,-15+(Math.random()-.5)*20);s.setAttribute("position",new N(o,3)),this.emberMat=new le({color:16763904,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.8,blending:T,depthWrite:!1}),this.embers=new k(s,this.emberMat),this.fireplaceGroup.add(this.embers),this.emberVelocities=new Float32Array(a);for(let r=0;r<a;r++)this.emberVelocities[r]=.02+Math.random()*.05;this.fireplaceGroup.position.set(0,0,0),this.fireplaceGroup.visible=!1}createFireShader(){return new O({uniforms:{uTime:{value:0},uColor:{value:new G(16729088)},uSpeed:{value:1},uIntensity:{value:1}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            ...
            `,transparent:!0,side:X,blending:T,depthWrite:!1})}initRainforest(){const e=this.batterySaver?400:1e3,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let s=0;s<e;s++){const o=s*3;i[o]=(Math.random()-.5)*60,i[o+1]=(Math.random()-.5)*40,i[o+2]=(Math.random()-.5)*40,a[o]=Math.random(),a[o+1]=Math.random(),a[o+2]=.08+Math.random()*.12}t.setAttribute("position",new E(i,3)),t.setAttribute("aRandom",new E(a,3)),this.rainMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new G(8965375)},uIntensity:{value:.6}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),this.raindrops=new k(t,this.rainMaterial),this.rainforestGroup.add(this.raindrops),this.rainforestGroup.visible=!1}initZenGarden(){const e=this.batterySaver?200:500,t=new B,i=new Float32Array(e*3),a=new Float32Array(e*3);for(let o=0;o<e;o++){const r=o*3;i[r]=(Math.random()-.5)*40,i[r+1]=(Math.random()-.5)*20,i[r+2]=(Math.random()-.5)*40,a[r]=Math.random(),a[r+1]=Math.random(),a[r+2]=Math.random()*Math.PI*2}t.setAttribute("position",new E(i,3)),t.setAttribute("aRandom",new E(a,3)),this.petalMaterial=new O({uniforms:{uTime:{value:0},uSpeed:{value:1},uColor:{value:new G(16758725)},uIntensity:{value:.8}},vertexShader:`
                attribute vec3 aRandom;
                uniform float uTime, uSpeed;
                varying float vAlpha;
                void main() {
            ...
            `,wireframe:!0,transparent:!0,side:X})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new B,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new N(a,3));const s=new le({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new k(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let d="🪷MINDWAVE";const m=e.logicMode,l=e.customText;m==="custom"||m==="txt"?d="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(m==="random"||m==="rnd"||m==="matrix"||m==="int"||m==="interstellar")&&(d="");const x=m==="matrix"||m==="int"||m==="interstellar"?[]:["LOGO",...d],f=x.length;for(let v=0;v<64;v++){const I=v%8,q=Math.floor(v/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(I*r,q*n,r,n),a.save(),a.translate(I*r+r/2,q*n+n/2);let A="",S=!1;if(v<f){const z=x[v];z==="LOGO"?S=!0:A=z}else{const w="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";A=w.charAt(Math.floor(Math.random()*w.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(A,0,0),a.restore(),A="")}if(A||S)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",S)if(this.logoImage){const D=document.createElement("canvas");D.width=100,D.height=100;const R=D.getContext("2d");R.imageSmoothingEnabled=!0,R.imageSmoothingQuality="high",R.drawImage(this.logoImage,0,0,100,100);const Y=R.getImageData(0,0,100,100),F=Y.data;for(let V=0;V<F.length;V+=4){const Q=180+(F[V]+F[V+1]+F[V+2])/3/255*75;F[V]=Q,F[V+1]=Q,F[V+2]=Q}R.putImageData(Y,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(D,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new Ne().load("./mindwave-logo-icon.png",w=>{if(this.logoImage=w,this.logoLoading=!1,this.cyberMaterial){const D=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=D}},void 0,w=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(A,0,0);a.restore()}const c=new J(i);return c.magFilter=ge,c.minFilter=ge,c}createCyberShader(e,t=this.matrixConfig){return new O({uniforms:{uTexture:{value:e},uColor:{value:new G(t.color||65345)},uHeadColor:{value:new G(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
