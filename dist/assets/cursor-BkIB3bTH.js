import{m as v}from"./bootloader-C4M82EBM.js";import"./preload-helper-CS1eXPs2.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";const d={sun:{name:"Sun",icon:"☀️",create:t=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="32" height="32">
            <g>
        ...
        </svg>`},sun2:{name:"Sun 2",icon:"🌞",create:()=>null},default:{name:"Default",icon:"↖",create:()=>null}};let n="lotus",s=null,u=1;function E(){v.currentTheme=localStorage.getItem("mindwave_theme")||"default";const t=localStorage.getItem("mindwave_cursor_shape");t&&d[t]?n=t:(n="default",localStorage.setItem("mindwave_cursor_shape","default"));const e=localStorage.getItem("mindwave_cursor_color");e&&(s=e),l(),window.addEventListener("themeChanged",r=>{v.currentTheme=r.detail.name,l()})}function m(){return s||getComputedStyle(document.documentElement).getPropertyValue("--accent").trim()||"#60a9ff"}function w(){return getComputedStyle(document.documentElement).getPropertyValue("--accent-secondary").trim()||"#fb7185"}function l(){let t=document.getElementById("dynamic-cursor-styles");if(t||(t=document.createElement("style"),t.id="dynamic-cursor-styles",document.head.appendChild(t)),n==="default"){t.textContent="";return}const e=m(),r=w();if(n==="sun2"){t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./tribal-sun-cursor.png') 16 16, auto !important; }`;return}if(n==="mindwave"){t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: url('./mindwave-cursor.png') 16 16, auto !important; }`;return}const a=d[n].create(e,r);if(!a){t.textContent="";return}let i=a;u!==1&&(i=i.replace("<svg ",`<svg opacity="${u}" `));const o=`url("data:image/svg+xml;utf8,${encodeURIComponent(i)}") 16 16, auto`;t.textContent=`body, a, button, [role="button"], input, select, textarea, .cursor-pointer { cursor: ${o} !important; }`}function p(t){d[t]&&(n=t,localStorage.setItem("mindwave_cursor_shape",t),l(),document.querySelectorAll(".cursor-option").forEach(e=>{const r=e.dataset.shape===t;e.classList.toggle("active",r),r?(e.classList.add("bg-[var(--accent)]/20","border-[var(--accent)]"),e.classList.remove("bg-white/5","border-white/10")):(e.classList.remove("bg-[var(--accent)]/20","border-[var(--accent)]"),e.classList.add("bg-white/5","border-white/10"))}))}function y(t){u=t,l()}function g(t){s=t,t?localStorage.setItem("mindwave_cursor_color",t):localStorage.removeItem("mindwave_cursor_color"),l();const e=document.getElementById("cursorColorPicker");e&&t&&e.value!==t&&(e.value=t);const r=document.getElementById("cursorColorPreview");r&&(r.style.backgroundColor=m())}function f(){g(null)}function C(){const t=document.getElementById("themeGrid");if(!t)return;const e=document.getElementById("cursorSection");e&&e.remove();const r=document.createElement("div");r.id="cursorSection",r.className="mt-6 pt-6 border-t border-white/10",r.innerHTML=`
        ...
        <div class="flex items-center gap-3 mb-4 p-3 rounded-xl bg-[var(--accent)]/10 border border-[var(--accent)]/20">
            ...
            <div class="relative group">
                <div id="cursorColorPreview" class="w-8 h-8 rounded-full border-2 border-[var(--accent)]/40 cursor-pointer shadow-lg transition-transform hover:scale-110" style="background-color: ${m()};"></div>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-5 gap-2" id="cursorShapeGrid">
            ${Object.entries(d).map(([o,c])=>{const h={sun:"☀️",moon:"🌙",heart:"❤️",mindwave:'<img src="./mindwave-cursor.png" width="24" height="24" style="display:inline-block;">',sun2:"🌞",default:"🖱️"}[o]||"🖱️";return`<button class="cursor-option p-3 rounded-xl text-center transition-all border ${o===n?"active bg-[var(--accent)]/20 border-[var(--accent)]":"bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20"}" data-shape="${o}" title="${c.name}"><span class="text-2xl block">${h}</span><div class="text-[10px] mt-1 font-semibold" style="color: var(--text-main);">${c.name}</div></button>`}).join("")}
        </div>`,t.parentElement.appendChild(r),r.querySelectorAll(".cursor-option").forEach(o=>o.addEventListener("click",()=>p(o.dataset.shape)));const a=r.querySelector("#cursorColorPicker");a&&["input","change"].forEach(o=>a.addEventListener(o,c=>g(c.target.value)));const i=r.querySelector("#resetCursorColor");i&&i.addEventListener("click",()=>{f();const o=getComputedStyle(document.documentElement).getPropertyValue("--accent").trim();r.querySelector("#cursorColorPreview")&&(r.querySelector("#cursorColorPreview").style.backgroundColor=o),a&&(a.value=o)})}window.setCursorShape=p;window.setCursorColor=g;window.setCursorOpacity=y;window.resetCursorColor=f;window.createCursorUIInThemeModal=C;window._setCursorShape=p;export{C as createCursorUIInThemeModal,E as initCursor,f as resetCursorColor,g as setCursorColor,y as setCursorOpacity,p as setCursorShape};
