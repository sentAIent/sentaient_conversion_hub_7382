...
`,ot=f=>f===1?`
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                if (uLoopActive > 0.5) {
                    ...
                    // Wave approaches from far distance (+150) to directly over the camera (-160)
                    wavePos = pos.y - 150.0 + (phase * 310.0);
                    
                    // Forming - wave grows as it approaches
                    float formProgress = smoothstep(0.0, 0.3, phase);
                    activeAmp = mix(uAmplitude * 0.1, uAmplitude, formProgress);
                    
                    // Pitching - barrel forms powerfully right as it gets near the camera
                    float pitchProgress = smoothstep(0.4, 0.85, phase);
                    activeCurl = mix(0.0, targetCurl, pitchProgress);
                    
                    // Crashing - wave collapses right as it passes the camera
                    float crashProgress = smoothstep(0.9, 0.98, phase);
                    activeAmp = mix(activeAmp, uAmplitude * 0.1, crashProgress);
                    activeCurl = mix(activeCurl, 0.0, crashProgress);
                    
                    // Washout
                    vPhaseWashout = smoothstep(0.95, 1.0, phase);
                }
                
                // Massive base height
                float baseHeight = 60.0 * activeAmp * (1.0 + react * 0.3);
                
                // Front/Back profile: Wave is moving towards -Y, so front cliff is wavePos < 0
                float envelope = 0.0;
                if (wavePos < 0.0) {
                    envelope = exp(-pow(wavePos * 0.04, 2.0)); // Front cliff (facing camera)
                } else {
                    envelope = exp(-pow(wavePos * 0.015, 2.0)); // Back of wave slope
                }
                
                // Wide wave along the X axis (huge beach break)
                float widthEnvelope = exp(-pow(pos.x * 0.012, 2.0));
                envelope *= widthEnvelope;
                ...
                if (uReactivity > 0.1) {
                    ...
                    float chaos = snoise(vec2(pos.x * 0.2, pos.y * 0.2 + time * 2.0));
                ...
                if (pos.z > curlStartHeight) {
                    ...
                    // The crest pushes FORWARD towards the camera (-Y) and curls down (Z) to engulf the user
                    pos.y = originalY - sin(bendAngle) * heightAbovePivot * 3.0;
        ...
        `,s=new O({vertexShader:i,fragmentShader:a,uniforms:t,transparent:!0,side:K,wireframe:!1});this.tsunamiMaterial=s;const o=new G(e,s);o.rotation.x=-Math.PI/2,o.position.y=-40,o.position.z=-150,o.position.x=0,o.frustumCulled=!1,this.tsunamiGroup.add(o);const n="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="512" height="512">
            <g>
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new X(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new $(40,40,32,32);this.zenWaterMaterial=new E({color:2245734,transparent:!0,opacity:.3,side:K}),this.zenWater=new G(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new $(300,100,128,64);this.oceanWave=new G(e,new O({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new A(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
