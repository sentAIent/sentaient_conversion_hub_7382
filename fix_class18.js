...
const optimizedClass18 = `    } else if (classId === 18) {
        return \`
        uniform float uTime;
...
// Extract everything before class 18, and everything after class 18's string return.
// The easiest way is to split on '    } else if (classId === 18) {'
const parts1 = code.split('    } else if (classId === 18) {');
if (parts1.length === 2) {
    // split the second part at the next '    } else {' which is the fallback block
    const parts2 = parts1[1].split('    } else {');
    
    // The second part of parts2 should be the fallback shader
    const newCode = parts1[0] + optimizedClass18 + '    } else {' + parts2.slice(1).join('    } else {');
    fs.writeFileSync(targetPath, newCode);
    console.log("Class 18 optimized and replaced successfully.");
} else {
    console.log("Error: Could not find class 18.");
}
