                    ...
                    float getDisplacementAt(vec2 uv, float t) {
                        ...
                        if (uType < 0.5) { // SACRED RESONANCE (Chladni standing waves)
                            ...
                            // Use low frequency for fragment displacement to match vertex and prevent normal aliasing
                        ...
                        if (uType > 5.5 && uType < 6.5) { // TYPE 6: PIXEL STARDUST (Colorful Fine-Grain Sand)
                            ...
                            // 2. Generate Pixel-by-Pixel Fine Grain (The "Sand" effect)
                            ...
                            // 3. Combine: Vivid Sand Grains on a dark plate
                            ...
                            // 1. Symmetrical Coordinate System
                            vec2 symUv = abs(uv);
                            float r = length(symUv);
                            float angle = atan(symUv.y, symUv.x);
                            
                            // 2. 5-Color Vivid Palette
                            vec3 p1 = vec3(1.0, 0.0, 0.5); // Hot Pink
                            vec3 p2 = vec3(0.0, 0.8, 1.0); // Bright Cyan
                            vec3 p3 = vec3(1.0, 0.8, 0.0); // Vibrant Gold
                            vec3 p4 = vec3(0.6, 0.0, 1.0); // Deep Violet
                            vec3 p5 = vec3(0.0, 1.0, 0.5); // Neon Spring Green
                            
                            // 3. Complex Symmetrical Mapping
                            float phase1 = sin(r * 12.0 - t * 2.0);
                            float phase2 = cos(angle * 8.0 + t * 1.5);
                            float phase3 = sin((r + angle) * 6.0);
                            
                            vec3 gradCol = mix(p4, p1, smoothstep(-1.0, 1.0, phase1));
                            gradCol = mix(gradCol, p2, smoothstep(-1.0, 1.0, phase2));
                            gradCol = mix(gradCol, p3, smoothstep(-1.0, 1.0, phase3));
                            gradCol = mix(gradCol, p5, smoothstep(0.8, 1.0, sin(r * 20.0)));
                            
                            // 4. Boost brightness and apply to base
                            baseCol = gradCol * (1.8 + uIntensity * 0.5);
                            
                            // 5. Add intense glowing highlights based on geometry peaks
                            float peaks = smoothstep(0.1, 0.0, abs(fCenter));
                            baseCol += p2 * peaks * 3.0; 
                        } else if (uType > 7.5 && uType < 8.5) { // TYPE 8: LEVEL 3 QUANTUM PLASMA (Customizable)
                            float r = length(uv);
