...
`,ot=f=>f===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class st{constructor(e){this.group=e,this.meshes=[],this.activeClassId=22,this.activeVariationId=0,this.materials={},this.rainbowMode=!1;const t=new ke(new Uint8Array([0,0,0,0]),1,1,Ee);t.needsUpdate=!0,this.dummyTexture=t;const i=new $(2,2);for(let a=1;a<=25;a++){let o=ot(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        ...
        void main() {`,o=o.replace("void main() {",h)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            void main() {
                ...
                // By adding pos.y, we slant the wave diagonally across the ocean!
                // Adding 100.0 ensures the wave forms to the left of the camera, so we look AT the front face instead of sitting behind it.
                float waveX = pos.x + (pos.y * 0.4) + 100.0; 
                
                if (uLoopActive > 0.5) {
                    float loopLen = 12.0; 
                    phase = fract(time / loopLen); 
                    
                    // Forming - never drop to 0.1, always maintain at least 40% of the wave so we never stare at a flat trough
                    float formProgress = smoothstep(0.0, 0.2, phase);
                    activeAmp = mix(uAmplitude * 0.4, uAmplitude, formProgress);
                    
                    // Pitching - barrel forms
                    float pitchProgress = smoothstep(0.2, 0.5, phase);
                    activeCurl = mix(0.0, targetCurl, pitchProgress);
                    
                    // Crashing - wave collapses but only down to 40% height, never flat
                    float crashProgress = smoothstep(0.8, 0.95, phase);
                    activeAmp = mix(activeAmp, uAmplitude * 0.4, crashProgress);
                    activeCurl = mix(activeCurl, 0.0, crashProgress);
                    
                    // Washout
                    vPhaseWashout = smoothstep(0.9, 1.0, phase);
                    
                    // Sweep the diagonal wave from left to right across the camera
                    waveX -= (phase * 200.0);
                }
                ...
                // Asymmetric bell curve - widened to make the trough (flat area) skinnier
                float envelope = 0.0;
                if (waveX < 0.0) {
                    envelope = exp(-pow(waveX * 0.008, 2.0)); // Wider back slope (gentle)
                } else {
                    envelope = exp(-pow(waveX * 0.025, 2.0)); // Wider front cliff (steep)
                }
                ...
                if (uReactivity > 0.1) {
                    // Flowing water texture up the face
                    float chaos = snoise(vec2(pos.x * 0.2 - time * 2.0, pos.y * 0.2 + time));
                    pos.z += (chaos * 20.0 * react * envelope); 
                }
                
                // Save the original, uncurled Z height for coloring and foam
                ...
                if (pos.z > curlStartHeight) {
                    ...
                    // We only want the front face of the wave to throw forward and curl.
                    // The back of the swell should remain a solid hill.
                    // This stretching creates a THICK, heavy lip instead of a paper-thin hollow ribbon.
                    float lipMask = smoothstep(-20.0, 10.0, waveX);
                    
                    // Max bend is ~1.15 PI (curls past vertical, pointing down into the trough)
                    float bendAngle = pow(normH, 1.5) * 3.14159 * 1.15 * normalizedCurl * envelope * lipMask; 
                    
                    // The lip physically throws out over the trough before dropping
                    float forwardThrow = pow(normH, 2.0) * 25.0 * normalizedCurl * envelope * lipMask;
                    
                    float originalX = pos.x;
                    
                    // x shift = forward throw + circular arc
                    // z shift = circular arc
                    pos.x = originalX + forwardThrow + sin(bendAngle) * h * 1.5;
                    pos.z = curlStartHeight + cos(bendAngle) * h;
                }
        ...
        `,a=`
            ...
            void main() {
                ...
                // Shifted the midWeight threshold to start at -10.0 to fill the trough completely with vibrant fcolors
                float midWeight = smoothstep(-10.0, 15.0, vElevation);
        ...
        `,s=new O({vertexShader:i,fragmentShader:a,uniforms:t,transparent:!0,side:K,wireframe:!1});this.tsunamiMaterial=s;const o=new G(e,s);o.rotation.x=-Math.PI/2,o.position.y=-50,o.position.z=-150,o.position.x=0,o.frustumCulled=!1,this.tsunamiGroup.add(o);const n="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="512" height="512">
            <g>
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new Y(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new $(40,40,32,32);this.zenWaterMaterial=new k({color:2245734,transparent:!0,opacity:.3,side:K}),this.zenWater=new G(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new $(300,100,128,64);this.oceanWave=new G(e,new O({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new A(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
            ...
            `,wireframe:!0,transparent:!0,side:K})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new B,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new j(a,3));const s=new me({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new Y(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let h="🪷MINDWAVE";const p=e.logicMode,l=e.customText;p==="custom"||p==="txt"?h="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(p==="random"||p==="rnd"||p==="matrix"||p==="int"||p==="interstellar")&&(h="");const g=p==="matrix"||p==="int"||p==="interstellar"?[]:["LOGO",...h],c=g.length;for(let x=0;x<64;x++){const I=x%8,R=Math.floor(x/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(I*r,R*n,r,n),a.save(),a.translate(I*r+r/2,R*n+n/2);let _="",M=!1;if(x<c){const F=g[x];F==="LOGO"?M=!0:_=F}else{const S="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";_=S.charAt(Math.floor(Math.random()*S.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(_,0,0),a.restore(),_="")}if(_||M)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",M)if(this.logoImage){const w=document.createElement("canvas");w.width=100,w.height=100;const U=w.getContext("2d");U.imageSmoothingEnabled=!0,U.imageSmoothingQuality="high",U.drawImage(this.logoImage,0,0,100,100);const Z=U.getImageData(0,0,100,100),q=Z.data;for(let L=0;L<q.length;L+=4){const Q=180+(q[L]+q[L+1]+q[L+2])/3/255*75;q[L]=Q,q[L+1]=Q,q[L+2]=Q}U.putImageData(Z,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(w,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new et().load("./mindwave-logo-icon.png",S=>{if(this.logoImage=S,this.logoLoading=!1,this.cyberMaterial){const w=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=w}},void 0,S=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(_,0,0);a.restore()}const u=new oe(i);return u.magFilter=Se,u.minFilter=Se,u}createCyberShader(e,t=this.matrixConfig){return new O({uniforms:{uTexture:{value:e},uColor:{value:new A(t.color||65345)},uHeadColor:{value:new A(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
