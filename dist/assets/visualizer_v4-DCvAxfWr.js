...
`,st=f=>f===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class rt{constructor(e){this.group=e,this.meshes=[],this.activeClassId=22,this.activeVariationId=0,this.materials={},this.rainbowMode=!1;const t=new Be(new Uint8Array([0,0,0,0]),1,1,We);t.needsUpdate=!0,this.dummyTexture=t;const i=new $(2,2);for(let a=1;a<=25;a++){let o=st(a);if(a<=16&&(o=o.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        uniform float uAudioHigh;`),o=o.replace("float intMod = max(0.1, uIntensity);","float intMod = max(0.1, uIntensity * (1.0 + uAudioLow * 3.0 + uAudioMid * 1.5));")),!o.includes("uniform float uMouseSync;")){let m="";/uniform\s+vec2\s+uMouse\s*;/.test(o)||(m+=`uniform vec2 uMouse;
        `),m+=`uniform float uMouseSync;
        void main() {`,o=o.replace("void main() {",m)}a<=21&&(o.includes("vec2 p = (vUv - 0.5) * 2.0;")?o=o.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            up -= uMouse * uMouseSync * 0.5;`)));const r=`
            ...
            `,transparent:!0,blending:T,depthWrite:!1}),this.particles=new j(t,this.particleMaterial),this.particleGroup.add(this.particles),this.particleGroup.visible=!1}initBox(){var p;this.boxOuter=new G;const e=new ce(new fe(3,3,3)),t=new ae({color:16777215,transparent:!0,opacity:.9,blending:T}),i=new ae({color:3900150,transparent:!0,opacity:.5,blending:T});this.boxOuter.add(new ie(e,t));for(let l=1;l<=3;l++){const g=new ie(e,i);g.scale.setScalar(1+l*.012),this.boxOuter.add(g)}const a=new ce(new fe(2,2,2)),s=new ae({color:14742270,transparent:!0,opacity:.8,blending:T}),o=new ae({color:6333946,transparent:!0,opacity:.4,blending:T});this.boxInner=new G,this.boxInner.add(new ie(a,s));for(let l=1;l<=2;l++){const g=new ie(a,o);g.scale.setScalar(1+l*.015),this.boxInner.add(g)}const r=new ce(new fe(3.05,3.05,3.05)),n=new ae({color:9684477,transparent:!0,opacity:.8,blending:T});this.boxEdges=new ie(r,n),this.boxGroup.add(this.boxOuter),this.boxGroup.add(this.boxInner),this.boxGroup.add(this.boxEdges),this.boxGroup.visible=!1;const m=((p=this.customColors)==null?void 0:p.box)||this.customColor;m&&(this.boxOuter.children.forEach(l=>l.material.color.copy(m)),this.boxInner.children.forEach(l=>l.material.color.copy(m)),this.boxEdges&&this.boxEdges.material&&this.boxEdges.material.color.copy(m))}initDragon(){var g;this.dragonDummy=new Ye,this.dragonLength=80;const e=new ne(.8,1),t=new U({color:15680580,wireframe:!0,transparent:!0,opacity:.8,blending:T}),i=new ne(.5,1),a=new U({color:16096779,wireframe:!1,transparent:!0,opacity:.6,blending:T});this.dragonBodyInstanced=new Me(e,t,this.dragonLength),this.dragonGlowInstanced=new Me(i,a,this.dragonLength);const s=new ue(1.5,3.5,5);s.rotateX(Math.PI/2);const o=new U({color:16638023,wireframe:!0,transparent:!0,opacity:.9,blending:T});this.dragonHead=new P(s,o),this.dragonPearlGroup=new G;const r=new le(1,16,16),n=new U({color:3718648,wireframe:!1,transparent:!0,opacity:.9,blending:T}),m=new le(1.3,16,16),p=new U({color:8246268,wireframe:!0,transparent:!0,opacity:.5,blending:T});this.dragonPearl=new P(r,n),this.dragonPearlHalo=new P(m,p),this.dragonPearlGroup.add(this.dragonPearl),this.dragonPearlGroup.add(this.dragonPearlHalo),this.dragonGroup.add(this.dragonBodyInstanced),this.dragonGroup.add(this.dragonGlowInstanced),this.dragonGroup.add(this.dragonHead),this.dragonGroup.add(this.dragonPearlGroup),this.dragonGroup.visible=!1;const l=((g=this.customColors)==null?void 0:g.dragon)||this.customColor;l&&this.updateDragonColor(l)}updateGalaxyColor(e){if(this.galaxySunMesh&&this.galaxySunMesh.traverse(t=>{t.isMesh&&t.material&&t.material.color.copy(e)}),this.galaxyStars&&this.galaxyStars.geometry){const t={};e.getHSL(t);const i=(t.h+.5)%1,a=this.galaxyStars.geometry.getAttribute("color");if(a){const s=a.count,o=this._tempColor;for(let r=0;r<s;r++){const n=r/s,m=n<.2?.8:n<.5?.6:.4+Math.random()*.15,p=.6+Math.random()*.3;o.setHSL(i,p,m),a.setXYZ(r,o.r,o.g,o.b)}a.needsUpdate=!0}}}updateDragonColor(e){if(!this.dragonBodyInstanced)return;this.dragonBodyInstanced.material.color.copy(e),this.dragonHead.material.color.copy(e);const t={};e.getHSL(t);const i=(t.h+.5)%1,a=new A().setHSL(i,t.s,t.l);this.dragonGlowInstanced.material.color.copy(a)}initGalaxy(){const e=this.batterySaver?500:1500,t=new B,i=[],a=[],s=[];for(let n=0;n<e;n++){const m=n/e*Math.PI*10,p=2.5+n/e*20+Math.random()*2,l=n%4*(Math.PI*2/4),g=Math.max(.5,n/e*4),c=Math.cos(m+l)*p+(Math.random()-.5)*g,u=(Math.random()-.5)*1.5,x=Math.sin(m+l)*p+(Math.random()-.5)*g;i.push(c,u,x);const I=n/e;I<.2?a.push(1,.95,.7):I<.5?a.push(.7+Math.random()*.3,.8,1):a.push(.4+Math.random()*.2,.3+Math.random()*.3,.8+Math.random()*.2),s.push(Math.random()<.05?.4+Math.random()*.3:.08+Math.random()*.15)}t.setAttribute("position",new K(i,3)),t.setAttribute("color",new K(a,3)),t.setAttribute("size",new K(s,1));const o=this.createStarTexture(),r=new me({size:.25,vertexColors:!0,map:o,transparent:!0,opacity:.9,blending:T,depthWrite:!1,sizeAttenuation:!0});this.galaxyStars=new j(t,r),this.galaxyGroup.add(this.galaxyStars),this.galaxySunStyle=this.galaxySunStyle||"sun",this.createGalaxySun(this.galaxySunStyle),this.galaxyGroup.visible=!1}createGalaxySun(e){this.galaxySunMesh&&(this.galaxyGroup.remove(this.galaxySunMesh),this.galaxySunMesh=null),this.galaxySunMesh=new G;const t=new U({color:4892415,transparent:!0,opacity:.85,blending:T,depthWrite:!1,side:N}),i=new G;if(e==="sun2"){const a=t.clone();a.side=je;const s=new Se(1.5,.25,16,64);i.add(new P(s,a));const o=8,r=new ue(.4,2.7,4);r.translate(0,2.7/2,0);const n=new ue(.2,1.7,4);n.translate(0,1.7/2,0);for(let m=0;m<o;m++){const p=m/o*Math.PI*2,l=new P(r,a);l.rotation.z=-p,l.position.set(Math.sin(p)*1.5,Math.cos(p)*1.5,0),i.add(l);const g=p+Math.PI/o,c=new P(n,a);c.rotation.z=-g,c.position.set(Math.sin(g)*1.5,Math.cos(g)*1.5,0),i.add(c)}}else if(e==="sun3"){const a={uTime:{value:0},uColor:{value:t.color},uBassIntt:{value:0}};this.galaxySunUniforms=a;const s=`
                varying vec2 vUv;
            ...
            `,r=new E({vertexShader:s,fragmentShader:o,uniforms:a,transparent:!0,blending:T,depthWrite:!1,side:N}),n=new le(2,64,64),m=new P(n,r);i.add(m);const p=new U({color:t.color,transparent:!0,opacity:.15,blending:T,depthWrite:!1,side:Ke}),l=new le(3,32,32);i.add(new P(l,p))}else{const a=new Se(1.5,.12,8,64);i.add(new P(a,t));const s=8;for(let o=0;o<s;o++){const r=o/s*Math.PI*2,n=new Te;n.moveTo(-.4,0),n.lineTo(.4,0),n.lineTo(0,2.7),n.lineTo(-.4,0);const m=new Ae(n),p=new P(m,t);p.rotation.z=-r,p.position.set(Math.sin(r)*1.5,Math.cos(r)*1.5,0),i.add(p);const l=r+Math.PI/s,g=new Te;g.moveTo(-.2,0),g.lineTo(.2,0),g.lineTo(0,1.7),g.lineTo(-.2,0);const c=new Ae(g),u=new P(c,t);u.rotation.z=-l,u.position.set(Math.sin(l)*1.5,Math.cos(l)*1.5,0),i.add(u)}}this.galaxySunMesh.add(i),this.galaxySunMesh.position.set(0,0,.1),this.galaxyGroup.add(this.galaxySunMesh)}setGalaxySunStyle(e){e!=="sun"&&e!=="sun2"&&e!=="sun3"||(this.galaxySunStyle=e,this.galaxyGroup&&this.galaxyGroup.children.length>0&&this.createGalaxySun(e))}setGalaxySunRotation(e,t){e==="x"&&(this.sunRotationSpeedX=parseFloat(t)),e==="y"&&(this.sunRotationSpeedY=parseFloat(t)),e==="z"&&(this.sunRotationSpeedZ=parseFloat(t))}createStarTexture(){if(this.textures.star)return this.textures.star;const e=64,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e);const o=i.createRadialGradient(a,s,0,a,s,e/2);return o.addColorStop(0,"rgba(255, 255, 255, 1.0)"),o.addColorStop(.1,"rgba(255, 255, 240, 0.8)"),o.addColorStop(.25,"rgba(255, 255, 200, 0.3)"),o.addColorStop(.5,"rgba(200, 200, 255, 0.1)"),o.addColorStop(1,"rgba(100, 100, 255, 0)"),i.fillStyle=o,i.fillRect(0,0,e,e),i.fillStyle="rgba(255, 255, 255, 0.6)",i.beginPath(),i.moveTo(0,s-1),i.lineTo(a,s-.5),i.lineTo(e,s-1),i.lineTo(e,s+1),i.lineTo(a,s+.5),i.lineTo(0,s+1),i.closePath(),i.fill(),i.beginPath(),i.moveTo(a-1,0),i.lineTo(a-.5,s),i.lineTo(a-1,e),i.lineTo(a+1,e),i.lineTo(a+.5,s),i.lineTo(a+1,0),i.closePath(),i.fill(),i.beginPath(),i.arc(a,s,2,0,Math.PI*2),i.fillStyle="rgba(255, 255, 255, 1.0)",i.fill(),this.textures.star=new oe(t),this.textures.star}initMandala(){var s;this.mandalaRings=[];const e=[2450411,15357964,3900150,16347926,12573694];for(let o=0;o<5;o++){const r=1.2+o*.8,n=6+o*6,m=new Qe(r-.05,r+.05,n),p=new U({color:e[o],side:N,transparent:!0,opacity:.4-o*.05,blending:T}),l=new P(m,p);l.userData={speed:(.01+o*.005)*(o%2===0?1:-1),segments:n},this.mandalaRings.push(l),this.mandalaGroup.add(l)}const t=new $e(.3,32),i=new U({color:16347926,transparent:!0,opacity:.6,blending:T});this.mandalaCenter=new P(t,i),this.mandalaGroup.add(this.mandalaCenter),this.mandalaGroup.visible=!1;const a=(s=this.customColors)==null?void 0:s.mandala;a&&(this.mandalaRings.forEach(o=>o.material.color.copy(a)),this.mandalaCenter.material.color.copy(a))}createSnowflakeTexture(){if(this.textures.snowflake)return this.textures.snowflake;const e=128,t=document.createElement("canvas");t.width=e,t.height=e;const i=t.getContext("2d"),a=e/2,s=e/2;i.clearRect(0,0,e,e),i.strokeStyle="rgba(220,240,255,1.0)",i.lineCap="round";for(let o=0;o<6;o++){const r=o/6*Math.PI*2;i.save(),i.translate(a,s),i.rotate(r),i.lineWidth=2.5,i.beginPath(),i.moveTo(0,0),i.lineTo(0,-52),i.stroke();const n=[{d:18,len:14,angle:Math.PI/4},{d:32,len:20,angle:Math.PI/4},{d:46,len:10,angle:Math.PI/5}];i.lineWidth=1.5,n.forEach(({d:m,len:p,angle:l})=>{[1,-1].forEach(g=>{i.beginPath(),i.moveTo(0,-m),i.lineTo(g*p*Math.cos(Math.PI/2-l),-m-p*Math.sin(Math.PI/2-l)),i.stroke()})}),i.restore()}i.beginPath();for(let o=0;o<6;o++){const r=o/6*Math.PI*2-Math.PI/6,n=a+Math.cos(r)*4,m=s+Math.sin(r)*4;o===0?i.moveTo(n,m):i.lineTo(n,m)}return i.closePath(),i.fillStyle="rgba(255, 255, 255, 0.8)",i.fill(),this.textures.snowflake=new oe(t),this.textures.snowflake}setTsunamiAmplitude(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uAmplitude.value=parseFloat(e))}setTsunamiCurl(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uCurl.value=parseFloat(e))}setTsunamiMist(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uMist.value=parseFloat(e)),this.tsunamiSun&&this.tsunamiSun.material&&(this.tsunamiSun.material.opacity=Math.max(parseFloat(e),.2),this.tsunamiSun.material.uniforms&&(this.tsunamiSun.material.uniforms.uMist.value=parseFloat(e)))}setTsunamiStyle(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uStyle.value=parseFloat(e))}setTsunamiReactivity(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uReactivity.value=parseFloat(e))}setTsunamiLoop(e){this.tsunamiMaterial&&this.tsunamiMaterial.uniforms&&(this.tsunamiMaterial.uniforms.uLoopActive.value=e?1:0)}initTsunami(){if(!this.tsunamiGroup)return;this.tsunamiGroup.clear();const e=new $(800,800,300,150),t={uTime:{value:0},uNormBass:{value:0},uColor:{value:new A(775123)},uSecondaryColor:{value:new A(27028)},uDeepColor:{value:new A(4907)},uFoamColor:{value:new A(16777215)},uAmplitude:{value:1},uCurl:{value:1},uMist:{value:1},uStyle:{value:1},uReactivity:{value:1},uLoopActive:{value:0}},i=`
            varying vec2 vUv;
            ...
            varying float vCrestDistToCamera;
            ...
            vec3 getWavePosition(vec3 p, float time, float activeAmp, float activeCurl, float phase, float react) {
                float waveX = p.x;
                if (uLoopActive > 0.5) {
                    waveX = p.x + 100.0 - (phase * 300.0); 
                }
                
                float baseHeight = 100.0 * activeAmp * (1.0 + react * 0.3);
                
                float envelope = 0.0;
                if (waveX < 0.0) {
                    envelope = exp(-pow(waveX * 0.06, 2.0)); 
                } else {
                    envelope = exp(-pow(waveX * 0.08, 2.0)); 
                }
                ...
                if (uReactivity > 0.1) {
                    ...
                    float edgeSmooth = smoothstep(150.0, 100.0, abs(p.y));
                    p.z += (chaos * 20.0 * react * envelope * edgeSmooth); 
                }
                
                if (activeCurl > 0.05) {
                    float crest_x = p.x - waveX;
                    vec2 pivot = vec2(crest_x + (baseHeight * 0.35), baseHeight * 0.45);
                    float curlFactor = envelope;
                    float maxAngle = 3.14159 * 1.25; 
                    float angle = -curlFactor * (activeCurl / 4.0) * maxAngle;
                    angle -= (react * 0.2 * curlFactor); 
            ...
            void main() {
                ...
                float react = max(uNormBass * uReactivity, snoise(vec2(time * 0.5)) * uReactivity * 0.2); 
                ...
                if (uLoopActive > 0.5) {
                    ...
                    float formProgress = smoothstep(0.0, 0.15, phase);
                    activeAmp = mix(0.1, uAmplitude, formProgress);
                    
                    float pitchProgress = smoothstep(0.15, 0.35, phase);
                    activeCurl = mix(0.0, targetCurl, pitchProgress);
                    
                    float crashProgress = smoothstep(0.7, 0.85, phase);
                    activeAmp = mix(activeAmp, 0.1, crashProgress);
                    activeCurl = mix(activeCurl, 0.0, crashProgress);
                    
                    vPhaseWashout = smoothstep(0.8, 0.95, phase);
                    
                    float crestX = -100.0 + phase * 300.0;
                    vCrestDistToCamera = 30.0 - crestX;
                } else {
                    vCrestDistToCamera = 1000.0;
                }
                ...
                float waveX = position.x;
                if (uLoopActive > 0.5) {
                    waveX = position.x + 100.0 - (phase * 300.0);
                }
                float envelope = 0.0;
                if (waveX < 0.0) {
                    envelope = exp(-pow(waveX * 0.06, 2.0));
                ...
                float preCurlHeight = position.z + baseHeight * envelope;
                
                vFoamFactor = smoothstep(baseHeight * 0.5, baseHeight, preCurlHeight) + foamNoise * 0.3;
        ...
        `,a=`
            ...
            void main() {
                ...
                // If we are looking at the underside of the wave, we shouldn't see foam
                if (!gl_FrontFacing) {
                    isFoam = 0.0;
                }
                ...
                if (uMist > 0.05) {
                    ...
                    finalColor += vec3(1.0, 0.9, 0.8) * glareMix * 2.0;
                }

                gl_FragColor = vec4(finalColor, alpha);
            }
        `;this.tsunamiMaterial=new E({uniforms:t,vertexShader:i,fragmentShader:a,transparent:!0,side:N}),this.tsunamiWave=new P(e,this.tsunamiMaterial),this.tsunamiWave.rotation.x=-Math.PI/2,this.tsunamiWave.position.y=-50,this.tsunamiGroup.add(this.tsunamiWave);const s=new $(3e3,3e3,10,10),o=new E({uniforms:t,vertexShader:`
                varying vec3 vWorldPosition;
                void main() {
            ...
            `,fragmentShader:`
                ...
                varying vec3 vWorldPosition;
                void main() {
                    // Deep water floor fading into the distance slightly to hide edge
                    float alpha = 1.0 - smoothstep(1000.0, 1500.0, length(vWorldPosition.xy));
                    gl_FragColor = vec4(uDeepColor, alpha * 0.95);
                }
            `,transparent:!0,side:N});this.tsunamiFloor=new P(s,o),this.tsunamiFloor.rotation.x=-Math.PI/2,this.tsunamiFloor.position.y=-50.1,this.tsunamiGroup.add(this.tsunamiFloor);const n="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="512" height="512">
            <g>
        ...
        </svg>`),m=new J().load(n),p=new we({map:m,color:16777215,transparent:!0,depthWrite:!1,fog:!1});this.tsunamiSun=new Ce(p),this.tsunamiSun.scale.set(60,60,1),this.tsunamiSun.position.set(70,50,-120),this.tsunamiSun.material.uniforms={uMist:{value:1}},this.tsunamiGroup.add(this.tsunamiSun)}initSnowflake(){for(;this.snowflakeGroup.children.length>0;){const c=this.snowflakeGroup.children[0];this.snowflakeGroup.remove(c),c.geometry&&c.geometry.dispose(),c.material&&c.material.dispose()}this._snowData=null;const e=this.snowCount||200,t=new Float32Array(e*3),i=new Float32Array(e),a=new Float32Array(e),s=new Float32Array(e),o=new Float32Array(e),r=new Float32Array(e),n=new Float32Array(e);for(let c=0;c<e;c++){const u=c*3;t[u]=(Math.random()-.5)*80,t[u+1]=(Math.random()-.5)*60,t[u+2]=-40+Math.random()*35;const x=(t[u+2]+40)/35;i[c]=1.5+x*8,a[c]=.2+x*.6,s[c]=Math.random()*Math.PI*2,o[c]=.015+Math.random()*.04+x*.03,r[c]=.01+Math.random()*.02,n[c]=.4+Math.random()*.8}const m=new B;m.setAttribute("position",new O(t,3)),m.setAttribute("aSize",new O(i,1)),m.setAttribute("aOpacity",new O(a,1));const p=this.createSnowflakeTexture(),l=new E({uniforms:{uTexture:{value:p},uColor:{value:this.customColor?this.customColor.clone():new A(10875883)},uIntensity:{value:0},uSizeMultiplier:{value:this.snowSizeOverride},uGlowAmount:{value:.5}},vertexShader:`
            ...
            `,transparent:!0,depthWrite:!1,blending:T}),g=new j(m,l);this.snowflakeGroup.add(g),this._snowData={count:e,positions:t,phases:s,speeds:o,drifts:r,driftFreqs:n,points:g,material:l,spinMeshes:[],spinSpeeds:[]}}setSnowSize(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uSizeMultiplier.value=Math.max(.2,Math.min(4,e)))}setSnowCount(e){this.snowCount=parseInt(e)||200,this.snowflakeGroup&&this.snowflakeGroup.visible&&this.initSnowflake()}setSnowDrift(e){this.snowDriftMultiplier=parseFloat(e)||1}setSnowGlow(e){var t;(t=this._snowData)!=null&&t.material&&(this._snowData.material.uniforms.uGlowAmount.value=Math.max(0,Math.min(1,e)))}initLava(){var o;this.lavaBlobs=[];const e=16;this.lavaUniforms={uBlobs:{value:[]},uColor:{value:new A(((o=v.visualColors)==null?void 0:o.lava)||16737792)},uSecondaryColor:{value:new A(16755200)},uTime:{value:0},uIntensity:{value:0},uResolution:{value:new pe(window.innerWidth,window.innerHeight)}};for(let r=0;r<e;r++)this.lavaUniforms.uBlobs.value.push(new Ze(0,-100,0,0));const t=new E({uniforms:this.lavaUniforms,vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
            ...
            `,transparent:!0,blending:T,depthWrite:!1,side:N}),i=new $(100,100),a=new P(i,t);a.position.z=-5,[{count:5,minSize:1,maxSize:1.5},{count:7,minSize:1.5,maxSize:2.2},{count:4,minSize:2.2,maxSize:3.5}].forEach(r=>{for(let n=0;n<r.count;n++){const m=r.minSize+Math.random()*(r.maxSize-r.minSize),p=["heating","rising","cooling","falling"],l=p[Math.floor(Math.random()*p.length)],g=-18+Math.random()*4,c=18+Math.random()*4;let u=0,x=.5;l==="heating"?(u=g,x=Math.random()*.5):l==="rising"?(u=g+Math.random()*(c-g),x=.8+Math.random()*.2):l==="cooling"?(u=c,x=1-Math.random()*.3):l==="falling"&&(u=c-Math.random()*(c-g),x=.2+Math.random()*.3),this.lavaBlobs.push({position:new Je((Math.random()-.5)*12,u,(Math.random()-.5)*6),material:{uniforms:{uTemp:{value:0},uIntensity:{value:0},uTime:{value:0}}},userData:{baseSize:m,state:l,temperature:x,floatMin:g,floatMax:c,heatRate:.15+Math.random()*.3,coolRate:.15+Math.random()*.3,riseSpeed:(.04+Math.random()*.05)/(m*.5),fallSpeed:(.05+Math.random()*.05)/(m*.5),driftPhase:Math.random()*Math.PI*2,driftSpeed:.01+Math.random()*.02}})}}),this.lavaGroup.add(a),this.lavaGroup.visible=!1}initFireplace(){const i=new $(100,80);this.fireMaterial=this.createFireShader(),this.fireMesh=new P(i,this.fireMaterial),this.fireMesh.position.set(0,0,-15),this.fireMesh.renderOrder=10,this.fireplaceGroup.add(this.fireMesh),this.fireLight=new et(16737792,5,200),this.fireLight.position.set(0,0,30),this.fireplaceGroup.add(this.fireLight);const a=650,s=new B,o=[];for(let r=0;r<a;r++)o.push((Math.random()-.5)*100,-20+Math.random()*40,-15+(Math.random()-.5)*20);s.setAttribute("position",new K(o,3)),this.emberMat=new me({color:16763904,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.8,blending:T,depthWrite:!1}),this.embers=new j(s,this.emberMat),this.fireplaceGroup.add(this.embers),this.emberVelocities=new Float32Array(a);for(let r=0;r<a;r++)this.emberVelocities[r]=.02+Math.random()*.05;this.fireplaceGroup.position.set(0,0,0),this.fireplaceGroup.visible=!1}createFireShader(){return new E({uniforms:{uTime:{value:0},uColor:{value:new A(16729088)},uSpeed:{value:1},uIntensity:{value:1}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new j(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const s=new $(40,40,32,32);this.zenWaterMaterial=new U({color:2245734,transparent:!0,opacity:.3,side:N}),this.zenWater=new P(s,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new $(300,100,128,64);this.oceanWave=new P(e,new E({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new A(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
            ...
            `,wireframe:!0,transparent:!0,side:N})),this.oceanWave.rotation.x=-Math.PI/3,this.oceanWave.position.y=-2,this.oceanGroup.add(this.oceanWave);const t=300,i=new B,a=[];for(let n=0;n<t;n++)a.push((Math.random()-.5)*50),a.push(-2.5+Math.random()*.5),a.push((Math.random()-.5)*40);i.setAttribute("position",new K(a,3));const s=new me({color:16777215,size:.15,map:this.createCircleTexture(),transparent:!0,opacity:.5,blending:T,depthWrite:!1});this.oceanFoam=new j(i,s),this.oceanGroup.add(this.oceanFoam),this.oceanGroup.visible=!1;const o=((r=this.customColors)==null?void 0:r.ocean)||this.customColor;o&&(this.oceanWave&&this.oceanWave.material.color.copy(o),this.oceanFoam&&this.oceanFoam.material.color.copy(o))}createCyberTexture(e=this.matrixConfig){const i=document.createElement("canvas");i.width=1024,i.height=1024;const a=i.getContext("2d");a.fillStyle="rgba(0,0,0,0)",a.fillRect(0,0,1024,1024),a.shadowBlur=12,a.shadowColor="rgba(255, 255, 255, 0.4)",a.font='bold 80px "Courier New", "MS Gothic", "Hiragino Kaku Gothic ProN", monospace',a.fillStyle="#ffffff",a.textAlign="center",a.textBaseline="middle";const s=8,o=8,r=1024/s,n=1024/o;let m="🪷MINDWAVE";const p=e.logicMode,l=e.customText;p==="custom"||p==="txt"?m="🪷"+(l&&l.length>0?l:"WELCOME TO MINDWAVE"):(p==="random"||p==="rnd"||p==="matrix"||p==="int"||p==="interstellar")&&(m="");const g=p==="matrix"||p==="int"||p==="interstellar"?[]:["LOGO",...m],c=g.length;for(let x=0;x<64;x++){const I=x%8,D=Math.floor(x/8);a.fillStyle="rgba(0,0,0,0)",a.fillRect(I*r,D*n,r,n),a.save(),a.translate(I*r+r/2,D*n+n/2);let z="",M=!1;if(x<c){const F=g[x];F==="LOGO"?M=!0:z=F}else{const S="ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ"+"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";z=S.charAt(Math.floor(Math.random()*S.length)),Math.random()>.5&&(a.save(),a.scale(-1,1),a.fillStyle="#ffffff",a.font="bold 80px monospace",a.fillText(z,0,0),a.restore(),z="")}if(z||M)if(a.fillStyle="#ffffff",a.font="bold 80px monospace",a.textAlign="center",a.textBaseline="middle",a.shadowBlur=16,a.shadowColor="rgba(255, 255, 255, 0.6)",M)if(this.logoImage){const w=document.createElement("canvas");w.width=100,w.height=100;const k=w.getContext("2d");k.imageSmoothingEnabled=!0,k.imageSmoothingQuality="high",k.drawImage(this.logoImage,0,0,100,100);const Q=k.getImageData(0,0,100,100),q=Q.data;for(let L=0;L<q.length;L+=4){const Z=180+(q[L]+q[L+1]+q[L+2])/3/255*75;q[L]=Z,q[L+1]=Z,q[L+2]=Z}k.putImageData(Q,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(w,-50,-50,100,100)}else!this.logoLoading&&!this.logoFailed&&(this.logoLoading=!0,new tt().load("./mindwave-logo-icon.png",S=>{if(this.logoImage=S,this.logoLoading=!1,this.cyberMaterial){const w=this.createCyberTexture();this.cyberMaterial.uniforms.uTexture.value=w}},void 0,S=>{this.logoFailed=!0,this.logoLoading=!1})),a.font="bold 80px monospace",a.fillStyle="#2dd4bf",a.fillText("🪷",0,0);else a.fillText(z,0,0);a.restore()}const u=new oe(i);return u.magFilter=Ie,u.minFilter=Ie,u}createCyberShader(e,t=this.matrixConfig){return new E({uniforms:{uTexture:{value:e},uColor:{value:new A(t.color||65345)},uHeadColor:{value:new A(15794160)},uTime:{value:0},uSpeed:{value:t.speed||1},uTailLength:{value:t.length||1},uRainbow:{value:t.rainbow?1:0},uBrightness:{value:this.brightnessMultiplier||1}},vertexShader:`
