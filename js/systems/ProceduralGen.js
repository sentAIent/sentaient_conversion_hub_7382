class ProceduralManager {
    ...
    generateSingleStyle(style) {
        console.log(`[Procedural] Nuclear Filter: Suppressing style generation for ${style}`);
        return [];
    }

    // --- PHASE 18 STUBS: Nuclear Filter Restoration ---
    generateStaticStars() { this.game.staticStars = []; }
    generateResourceDeposits() { this.game.resourceDeposits = []; }
    generateMissionBoard() { return []; }
    generateBlackHoleRings() { return []; }
    generateSupernovaParticles() { return []; }
    generateTunnelParticles() { return []; }
    generateSector(sx, sy) { return { x: sx, y: sy, minerals: [], deposits: [], structures: [], hazards: { mines: [], turrets: [], blackHoles: [], nebula: null } }; }
    ...
    generateDeepSpaceStyle() {
        ...
        // 2. Galaxies (SUPPRESSED)
        this.game.galaxies = [];

        // 3. Black Holes (SUPPRESSED)
        this.game.blackHoles = [];

        // 4. Nebulae (SUPPRESSED)
        this.game.nebulae = [];

        // 5. Planets (SUPPRESSED)
        this.game.planets = [];
        // 6. Shooting Stars (SUPPRESSED)
        // 6. Shooting Stars (SUPPRESSED)
