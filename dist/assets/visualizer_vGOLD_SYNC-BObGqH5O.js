...
`,We=h=>h===1?`
        ...
        `:"void main() { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); }";class ye{constructor(e){this.group=e,this.meshes=[],this.activeClassId=1,this.activeVariationId=0,this.materials={};const t=new Ae(new Uint8Array([0,0,0,0]),1,1,ze);t.needsUpdate=!0,this.dummyTexture=t;const i=new j(2,2);for(let a=1;a<=22;a++){let s=We(a);a<=16&&(s=s.replace("uniform float uIntensity;",`uniform float uIntensity;
        uniform float uAudioLow;
        uniform float uAudioMid;
        ...
        void main() {`)),a!==22&&(s.includes("vec2 p = (vUv - 0.5) * 2.0;")?s=s.replace("vec2 p = (vUv - 0.5) * 2.0;",`vec2 p = (vUv - 0.5) * 2.0;
            ...
            uv -= uMouse * uMouseSync * 0.5;`):s.includes("vec2 up = (vUv - 0.5) * 2.0;")&&(s=s.replace("vec2 up = (vUv - 0.5) * 2.0;",`vec2 up = (vUv - 0.5) * 2.0;
            ...
            `,transparent:!0,depthWrite:!1}),this.petals=new N(t,this.petalMaterial),this.zenGardenGroup.add(this.petals);const o=new j(40,40,32,32);this.zenWaterMaterial=new L({color:2245734,transparent:!0,opacity:.3,side:X}),this.zenWater=new _(o,this.zenWaterMaterial),this.zenWater.rotation.x=-Math.PI/2,this.zenWater.position.y=-5,this.zenGardenGroup.add(this.zenWater),this.zenGardenGroup.visible=!1}initOcean(){var r;const e=new j(300,100,128,64);this.oceanWave=new _(e,new E({uniforms:{uTime:{value:0},uColor:{value:this.customColor?this.customColor.clone():new G(43775)},uNormBass:{value:0},uBeatPulse:{value:0}},vertexShader:`
                varying float vDist;
