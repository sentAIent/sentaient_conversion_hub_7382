...
export class Visualizer3D {
    ...
    constructor(canvas, initialState = {}) {
        ...
        if (this.overlayCanvas) {
            ...
            this._boundResizeOverlayCanvas = () => this.resizeOverlayCanvas();
        ...
        try {
            ...
            this._boundResize = () => {
                if (resizeFrameId) return; // Debounce
            ...
            this._boundVisibilityChange = () => {
                if (document.hidden) {
                    ...
                    this.lastTime = (performance.now() * 0.001) % (2000.0 * Math.PI); // Prevent time-jump glitches
            ...
            this._boundContextRestored = () => {
                ...
                try {
                    ...
                    // Rebuild CymaticsCore safely after GPU crash
                    if (this.cymaticsCore) {
                        this.cymaticsCore.dispose();
                    }
                    while (this.cymaticsGroup.children.length > 0) {
                        this.cymaticsGroup.remove(this.cymaticsGroup.children[0]);
                    }
                    this.cymaticsCore = new CymaticsCore(this.cymaticsGroup);
                    if (this.currentCymaticData) {
                        this.setCymaticByName(this.currentCymaticData.name);
                    }

                    // Flag all procedural CanvasTextures for GPU re-upload to prevent blackouts
                    if (this.textures) {
                    ...
                    this.active = true;
                    this.lastTime = (performance.now() * 0.001) % (2000.0 * Math.PI);
    ...
    initSnowflake() {
        ...
        while (this.snowflakeGroup.children.length > 0) {
            const child = this.snowflakeGroup.children[0];
            this.snowflakeGroup.remove(child);
            child.traverse((c) => {
    ...
    render(analyserL, analyserR) {
        ...
        // Hardened mutex: absolutely prevent multiple concurrent loops by cancelling any pending frame
        if (state.animationId) {
            cancelAnimationFrame(state.animationId);
            state.animationId = null;
        }
        ...
        try {
            // Automatically capture audio state if not explicitly passed
            ...
            if (analyserL) {
                ...
                let bass = 0; for (let i = 0; i < 15; i++) bass += this._freqDataArray[i] || 0;
                normBass = Math.pow((bass / 15) / 255, 0.8);
                
                let mids = 0; for (let i = 10; i < 100; i++) mids += this._freqDataArray[i] || 0;
                normMids = (mids / 90) / 255;
                
                let highs = 0; for (let i = 100; i < 300; i++) highs += this._freqDataArray[i] || 0;
                normHighs = (highs / 200) / 255;
                
                // Fallbacks to prevent NaNs or non-finite values inside WebGL
                if (isNaN(normBass) || !isFinite(normBass)) normBass = 0;
                if (isNaN(normMids) || !isFinite(normMids)) normMids = 0;
                if (isNaN(normHighs) || !isFinite(normHighs)) normHighs = 0;
            }

            const multiplier = Math.max(0.001, this.speedMultiplier || 1.0);
            const now = (performance.now() * 0.001) % (2000.0 * Math.PI);
            if (this.activeModes.has('cymatics') && this.cymaticsCore) {
                // Ensure mouse position defaults to 0 if not interacting
                if (!this.mouseData) {
                    this.mouseData = { x: 0, y: 0 };
                    this._boundCymaticPointer = (e) => {
                        if (e.touches && e.touches.length > 0) {
                            this.mouseData.x = (e.touches[0].clientX / window.innerWidth) * 2 - 1;
                            this.mouseData.y = -(e.touches[0].clientY / window.innerHeight) * 2 + 1;
                        } else if (e.clientX !== undefined) {
                            this.mouseData.x = (e.clientX / window.innerWidth) * 2 - 1;
                            this.mouseData.y = -(e.clientY / window.innerHeight) * 2 + 1;
                        }
                    };
                    window.addEventListener('mousemove', this._boundCymaticPointer);
                    window.addEventListener('touchmove', this._boundCymaticPointer);
                }
                
                // Allow user to toggle audio sync. Cache DOM reference to prevent layout thrashing at 60fps
                if (!this.audioSyncToggleEl) {
                    this.audioSyncToggleEl = document.getElementById('audioSyncToggle');
                }
                let audioSyncEnabled = this.audioSyncToggleEl ? this.audioSyncToggleEl.checked : true;
                ...
                // Reuse audioData object to prevent garbage collection pressure in hot loop
                if (!this.reusableAudioData) {
                    this.reusableAudioData = { bass: 0, mids: 0, highs: 0 };
                }
                this.reusableAudioData.bass = normBass;
                this.reusableAudioData.mids = normMids;
                this.reusableAudioData.highs = normHighs;
                
                this.cymaticsCore.update(now * multiplier, this.reusableAudioData, this.mouseData);
            }
            if (!this.lastTime) this.lastTime = now;
            let dt = now - this.lastTime;
            if (dt < 0) dt = 0.016; // Fix catastrophic physics explosion when modulo wraps around
            dt = Math.min(0.1, dt);
            this.lastTime = now;
    ...
    applyCymatic(data) {
        if (!data || !this.cymaticsCore) return;
        ...
        const classId = data.classId || 1;
        const variationId = data.variationId || 0;
    ...
    // Adapt the UI color picker calls to the new CymaticsCore API
    ...
    if (viz3D && viz3D.cymaticsCore) {
        ...
        if (!viz3D.activeModes.has('cymatics')) {
            if (window.switchRightTab) {
                const cymaticsTab = document.querySelector('.tab-pill[title="Cymatics Engine"]');
                if (cymaticsTab) {
                    window.switchRightTab('cymatics', cymaticsTab);
                }
            }
